...
#include <blocmatrix/protocol/AmountConversions.h>
...
class PaymentSandbox_test : public beast::unit_test::suite
{
    ...
    */
    void testSelfFunding (FeatureBitset features)
    {
        ...
        env.fund (BMC (10000), snd, rcv, gw1, gw2);
    ...
    void testSubtractCredits (FeatureBitset features)
    {
        ...
        env.fund (BMC (10000), alice, gw1, gw2);
        ...
        {
            // blocmatrixCredit, no deferredCredits
            ApplyViewImpl av (&*env.current(), tapNONE);
            ...
            blocmatrixCredit (av, gw1, alice, toCredit, true, j);
            BEAST_EXPECT(accountHolds (av, alice, iss.currency, iss.account,
                        fhIGNORE_FREEZE, j) ==
                    startingAmount + toCredit);

            blocmatrixCredit (av, alice, gw1, toDebit, true, j);
            BEAST_EXPECT(accountHolds (av, alice, iss.currency, iss.account,
                        fhIGNORE_FREEZE, j) ==
                    startingAmount + toCredit - toDebit);
        }
        ...
        {
            // blocmatrixCredit, w/ deferredCredits
            ApplyViewImpl av (&*env.current(), tapNONE);
            PaymentSandbox pv (&av);
            ...
            blocmatrixCredit (pv, gw1, alice, toCredit, true, j);
            BEAST_EXPECT(accountHolds (pv, alice, iss.currency, iss.account,
                        fhIGNORE_FREEZE, j) ==
                    startingAmount);
        }
    ...
    void testReserve(FeatureBitset features)
    {
        ...
        auto accountFundsBMC = [](ReadView const& view,
            AccountID const& id, beast::Journal j) -> BMCAmount
        {
            return toAmount<BMCAmount> (accountHolds (
                view, id, bmcCurrency (), bmcAccount (), fhZERO_IF_FROZEN, j));
        };

        auto reserve = [](jtx::Env& env, std::uint32_t count) -> BMCAmount
        {
        ...
        PaymentSandbox sb (&av);
        {
            ...
            accountSend (sb, bmcAccount (), alice, BMC(100), env.journal);
            accountSend (sb, alice, bmcAccount (), BMC(100), env.journal);
            BEAST_EXPECT(
                accountFundsBMC (sb, alice, env.journal) == beast::zero);
        }
    }
...
BEAST_DEFINE_TESTSUITE (PaymentSandbox, ledger, blocmatrix);

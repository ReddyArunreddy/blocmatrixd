...
#include <blocmatrix/app/tx/impl/Transactor.h>
#include <blocmatrix/app/tx/impl/ApplyContext.h>
...
class Invariants_test : public beast::unit_test::suite
{
    ...
    void
    testBMCNotCreated (bool enabled)
    {
        using namespace test::jtx;
        testcase << "checks " << (enabled ? "enabled" : "disabled") <<
            " - BMC created";
        doInvariantCheck (enabled,
            {{ "BMC net change was positive: 500" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                // put a single account in the view and "manufacture" some BMC
    ...
    testTypesMatch(bool enabled)
    {
        ...
        doInvariantCheck (enabled,
            {{ "ledger entry type mismatch" },
             { "BMC net change of -1000000000 doesn't match fee 0" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
    ...
    void
    testNoBMCTrustLine(bool enabled)
    {
        using namespace test::jtx;
        testcase << "checks " << (enabled ? "enabled" : "disabled") <<
            " - trust lines with BMC not allowed";
        doInvariantCheck (enabled,
            {{ "an BMC trust line was created" }},
            [](Account const& A1, Account const& A2, ApplyContext& ac)
            {
                // create simple trust SLE with bmc currency
                auto index = getBlocmatrixStateIndex (A1, A2, bmcIssue().currency);
                auto const sleNew = std::make_shared<SLE>(
                    ltBLOCMATRIX_STATE, index);
                ac.view().insert (sleNew);
                return true;
            });
    }
    ...
    void
    testBMCBalanceCheck(bool enabled)
    {
        using namespace test::jtx;
        testcase << "checks " << (enabled ? "enabled" : "disabled") <<
            " - BMC balance checks";
        ...
        doInvariantCheck (enabled,
            {{ "Cannot return non-native STAmount as BMCAmount" }},
            [](Account const& A1, Account const& A2, ApplyContext& ac)
            {
                //non-native balance
                auto const sle = ac.view().peek (keylet::account(A1.id()));
        ...
        doInvariantCheck (enabled,
            {{ "incorrect account BMC balance" },
             {  "BMC net change was positive: 99999999000000001" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
        ...
        doInvariantCheck (enabled,
            {{ "incorrect account BMC balance" },
             { "BMC net change of -1000000001 doesn't match fee 0" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                // balance is negative
                auto const sle = ac.view().peek (keylet::account(A1.id()));
    ...
    testTransactionFeeCheck(bool enabled)
    {
        ...
        doInvariantCheck (enabled,
            {{ "fee paid was negative: -1" },
             { "BMC net change of 0 doesn't match fee -1" }},
        ...
        doInvariantCheck (enabled,
            {{ "fee paid exceeds system limit: "s +
             ...
             { "BMC net change of 0 doesn't match fee "s +
                std::to_string(SYSTEM_CURRENCY_START) }},
            [](Account const&, Account const&, ApplyContext&) { return true; },
            BMCAmount{SYSTEM_CURRENCY_START});

         doInvariantCheck (enabled,
            {{ "fee paid is 20 exceeds fee specified in transaction." },
             { "BMC net change of 0 doesn't match fee 20" }},
            [](Account const&, Account const&, ApplyContext&) { return true; },
            BMCAmount{20},
            [](STTx& tx) { tx.setFieldAmount(sfFee, BMCAmount{10}); } );
    }
    ...
    testNoBadOffers(bool enabled)
    {
        ...
        doInvariantCheck (enabled,
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                sleNew->setFieldAmount (sfTakerPays, BMC(-1));
                ac.view().insert (sleNew);
                return true;
            });
        ...
        doInvariantCheck (enabled,
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                sleNew->setFieldAmount (sfTakerGets, BMC(-1));
                ac.view().insert (sleNew);
                return true;
            });
        ...
        doInvariantCheck (enabled,
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                sleNew->setAccountID (sfAccount, A1.id());
                sleNew->setFieldU32 (sfSequence, (*sle)[sfSequence]);
                sleNew->setFieldAmount (sfTakerPays, BMC(10));
                sleNew->setFieldAmount (sfTakerGets, BMC(11));
                ac.view().insert (sleNew);
                return true;
            });
    }
    ...
    testNoZeroEscrow(bool enabled)
    {
        ...
        doInvariantCheck (enabled,
            {{ "BMC net change of -1000000 doesn't match fee 0"},
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                auto sleNew = std::make_shared<SLE> (
                    keylet::escrow(A1, (*sle)[sfSequence] + 2));
                sleNew->setFieldAmount (sfAmount, BMC(-1));
                ac.view().insert (sleNew);
                return true;
            });

        doInvariantCheck (enabled,
            {{ "BMC net change was positive: 100000000000000001" },
             {  "escrow specifies invalid amount" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
...
public:
    void run () override
    {
        ...
        for(auto const& b : {false, true})
        {
            ...
            testTypesMatch (b);
            testNoBMCTrustLine (b);
            testBMCBalanceCheck (b);
...
BEAST_DEFINE_TESTSUITE (Invariants, ledger, blocmatrix);

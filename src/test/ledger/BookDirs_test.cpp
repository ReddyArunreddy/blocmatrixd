...
struct BookDirs_test : public beast::unit_test::suite
{
    void test_bookdir(FeatureBitset features)
    {
        ...
        auto gw = Account("gw");
        auto USD = gw["USD"];
        env.fund(BMC(1000000), "alice", "bob", "gw");

        {
            Book book(bmcIssue(), USD.issue());
            {
        ...
        {
            env(offer("alice", Account("alice")["USD"](50), BMC(10)));
            auto d = BookDirs(*env.current(),
                Book(Account("alice")["USD"].issue(), bmcIssue()));
            BEAST_EXPECT(std::distance(d.begin(), d.end()) == 1);
        }

        {
            env(offer("alice", gw["CNY"](50), BMC(10)));
            auto d = BookDirs(*env.current(),
                Book(gw["CNY"].issue(), bmcIssue()));
            BEAST_EXPECT(std::distance(d.begin(), d.end()) == 1);
        }
...

        {
            ...
            for (auto i = 1, j = 3; i <= 3; ++i, --j)
                for (auto k = 0; k < 80; ++k)
                    env(offer("alice", AUD(i), BMC(j)));

            auto d = BookDirs(*env.current(),
                Book(AUD.issue(), bmcIssue()));
            BEAST_EXPECT(std::distance(d.begin(), d.end()) == 240);
            auto i = 1, j = 3, k = 0;
            for (auto const& e : d)
            {
                BEAST_EXPECT(e->getFieldAmount(sfTakerPays) == AUD(i));
                BEAST_EXPECT(e->getFieldAmount(sfTakerGets) == BMC(j));
                if (++k % 80 == 0)
                {
                    ++i;
                    --j;
...
BEAST_DEFINE_TESTSUITE(BookDirs,ledger,blocmatrix);

...
#ifndef BLOCMATRIX_TEST_JTX_ENV_H_INCLUDED
#define BLOCMATRIX_TEST_JTX_ENV_H_INCLUDED
...
#include <blocmatrix/ledger/CachedSLEs.h>
#include <blocmatrix/protocol/Feature.h>
#include <blocmatrix/protocol/Indexes.h>
#include <blocmatrix/protocol/Issue.h>
#include <blocmatrix/protocol/STAmount.h>
#include <blocmatrix/protocol/STObject.h>
#include <blocmatrix/protocol/STTx.h>
#include <functional>
#include <string>
#include <tuple>
#include <blocmatrix/beast/cxx17/type_traits.h> // <type_traits>
#include <utility>
...
/** Designate accounts as no-blocmatrix in Env::fund */
template <class... Args>
std::array<Account, 1 + sizeof...(Args)>
noblocmatrix (Account const& account, Args const&... args)
{
    return {{account, args...}};
}
...
supported_amendments()
{
    static const FeatureBitset ids = []{
        auto const& sa = blocmatrix::detail::supportedAmendments();
...
public:
    ...
    /** Returns the current Blocmatrix Network Time
    ...
    */
    ...
    /** Returns the BMC balance on an account.
...
private:
    void
    fund (bool setDefaultBlocmatrix,
...
public:
    ...
    /** Create a new account with some BMC.
        ...
        Effects:
            The asfDefaultBlocmatrix on the account is set,
            and the sequence number is incremented, unless
            the account is wrapped with a call to noblocmatrix.

            The account's BMC balance is set to amount.

            Generates a test that the balance is set.

        @param amount The amount of BMC to transfer to
                      each account.

        @param args A heterogeneous list of accounts to fund
                    or calls to noblocmatrix with lists of accounts
                    to fund.
    */

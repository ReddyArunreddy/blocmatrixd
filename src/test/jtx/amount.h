...
#ifndef BLOCMATRIX_TEST_JTX_AMOUNT_H_INCLUDED
#define BLOCMATRIX_TEST_JTX_AMOUNT_H_INCLUDED
...
The decision was made to accept amounts of drops and BMC
using an int type, since the range of BMC is 100 billion
...
/** Represents an BMC or IOU quantity
    This customizes the string conversion and supports
    BMC conversions from integer and floating point.
*/
struct PrettyAmount
{
private:
...
{
    AccountID account;
    blocmatrix::Currency currency;

    BookSpec(AccountID const& account_,
        blocmatrix::Currency const& currency_)
        : account(account_)
        , currency(currency_)
    {
    }
...
struct BMC_t
{
    ...
    */
    ...
    /** Returns an amount of BMC as STAmount

        @param v The number of BMC (not drops)
    */
    /** @{ */
    ...
    friend
    BookSpec
    operator~ (BMC_t const&)
    {        
        return BookSpec( bmcAccount(),
            bmcCurrency() );
    }
};

/** Converts to BMC Issue or STAmount.

    Examples:
        BMC         Converts to the BMC Issue
        BMC(10)     Returns STAmount of 10 BMC
*/
extern BMC_t const BMC;

/** Returns an BMC STAmount.

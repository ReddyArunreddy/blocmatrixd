...
#include <blocmatrix/beast/hash/uhash.h>
...
public:
    ...
    testAmount()
    {
        ...
        BEAST_EXPECT(to_string(BMC(5)) == "5 BMC");
        BEAST_EXPECT(to_string(BMC(.80)) == "0.8 BMC");
        BEAST_EXPECT(to_string(BMC(.005)) == "5000 drops");
        BEAST_EXPECT(to_string(BMC(0.1)) == "0.1 BMC");
        BEAST_EXPECT(to_string(BMC(10000)) == "10000 BMC");
        BEAST_EXPECT(to_string(drops(10)) == "10 drops");
        BEAST_EXPECT(to_string(drops(123400000)) == "123.4 BMC");
        BEAST_EXPECT(to_string(BMC(-5)) == "-5 BMC");
        BEAST_EXPECT(to_string(BMC(-.99)) == "-0.99 BMC");
        BEAST_EXPECT(to_string(BMC(-.005)) == "-5000 drops");
        BEAST_EXPECT(to_string(BMC(-0.1)) == "-0.1 BMC");
        BEAST_EXPECT(to_string(drops(-10)) == "-10 drops");
        BEAST_EXPECT(to_string(drops(-123400000)) == "-123.4 BMC");

        BEAST_EXPECT(BMC(1) == drops(1000000));
        BEAST_EXPECT(BMC(1) == STAmount(1000000));
        BEAST_EXPECT(STAmount(1000000) == BMC(1));
    ...
    testEnv()
    {
        ...
        {
            Env env(*this);
            env(pay("alice", "bob", BMC(1000)), seq(1), fee(10), sig("alice"), ter(terNO_ACCOUNT));
        }
        ...
        // fund
        {
            ...
            env.fund(n, "dave", noblocmatrix("eric"));
            env.fund(n, "fred", noblocmatrix("gary", "hank"));
            env.fund(n, noblocmatrix("irene"));
            env.fund(n, noblocmatrix("jim"), "karen");
            env.fund(n, noblocmatrix("lisa", "mary"));

            // flags
            env.fund(n, noblocmatrix("xavier"));
            env.require(nflags("xavier", asfDefaultBlocmatrix));
            env.fund(n, "yana");
            env.require(flags("yana", asfDefaultBlocmatrix));
        }
        ...
        {
            Env env(*this);
            env.fund(n, noblocmatrix("alice", gw));
    ...
    testRequire()
    {
        ...
        env.require(balance("alice", BMC(none)));
        env.fund(BMC(10000), "alice", gw);
        env.require(balance("alice", USD(none)));
        env.trust(USD(100), "alice");
        env.require(balance("alice", BMC(10000))); // fee refunded
    ...
    testPayments()
    {
        ...
        env.require(balance("alice", BMC(10000)));
        env.require(balance("bob", BMC(10000)));
        env.require(balance("carol", BMC(10000)));
        env.require(balance(gw, BMC(10000)));

        env(pay(env.master, "alice", BMC(1000)), fee(none),     ter(temMALFORMED));
        env(pay(env.master, "alice", BMC(1000)), fee(1),        ter(telINSUF_FEE_P));
        env(pay(env.master, "alice", BMC(1000)), seq(none),     ter(temMALFORMED));
        env(pay(env.master, "alice", BMC(1000)), seq(20),       ter(terPRE_SEQ));
        env(pay(env.master, "alice", BMC(1000)), sig(none),     ter(temMALFORMED));
        env(pay(env.master, "alice", BMC(1000)), sig("bob"),    ter(tefBAD_AUTH));

        env(pay(env.master, "dilbert", BMC(1000)), sig(env.master));
        ...
        env(offer("carol", BMC(50), USD(50)), require(owners("carol", 2)));
        env(pay("alice", "bob", any(USD(10))),                  ter(tecPATH_DRY));
        env(pay("alice", "bob", any(USD(10))),
            paths(BMC), sendmax(BMC(10)),                       ter(tecPATH_PARTIAL));
        env(pay("alice", "bob", any(USD(10))), paths(BMC),
            sendmax(BMC(20)));
...
BEAST_DEFINE_TESTSUITE(Env,app,blocmatrix);

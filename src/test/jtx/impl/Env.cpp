...
#include <blocmatrix/basics/Slice.h>
...
void
Env::fund (bool setDefaultBlocmatrix,
    STAmount const& amount,
        Account const& account)
{
    memoize(account);
    if (setDefaultBlocmatrix)
    {
        ...
        apply(pay(master, account, amount +
            drops(current()->fees().base)),
                jtx::seq(jtx::autofill),
                    fee(jtx::autofill),
                        sig(jtx::autofill));
        apply(fset(account, asfDefaultBlocmatrix),
            jtx::seq(jtx::autofill),
                fee(jtx::autofill),
                    sig(jtx::autofill));
        require(flags(account, asfDefaultBlocmatrix));
    }
    else
    {
        apply(pay(master, account, amount),
            jtx::seq(jtx::autofill),
                fee(jtx::autofill),
                    sig(jtx::autofill));
        require(nflags(account, asfDefaultBlocmatrix));
    }

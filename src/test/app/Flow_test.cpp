...
#include <test/jtx.h>
#include <blocmatrix/app/paths/Flow.h>
#include <blocmatrix/app/paths/impl/Steps.h>
#include <blocmatrix/basics/contract.h>
#include <blocmatrix/core/Config.h>
#include <blocmatrix/ledger/ApplyViewImpl.h>
#include <blocmatrix/ledger/PaymentSandbox.h>
#include <blocmatrix/ledger/Sandbox.h>
...
bool getNoBlocmatrixFlag (jtx::Env const& env,
    ...
    Currency const& cur)
{
    ...
    {
        auto const flag = (src.id() > dst.id()) ? lsfHighNoBlocmatrix : lsfLowNoBlocmatrix;
...
bmcMinusFee (jtx::Env const& env, std::int64_t bmcAmount)
{
    ...
    return drops (
        dropsPerBMC<std::int64_t>::value * bmcAmount - feeDrops);
};
...
struct Flow_test : public beast::unit_test::suite
{
    void testDirectStep (FeatureBitset features)
    {
        ...
        auto const USD = gw["USD"];
        {
            ...
            env.fund (BMC (10000), alice, bob, gw);
        ...
        }
        {
            ...
            env.fund (BMC (10000), alice, bob);
            env (pay (alice, bob, BMC (100)));
            env.require (balance (bob, BMC (10000 + 100)));
            env.require (balance (alice, bmcMinusFee (env, 10000 - 100)));
        }
        {
            // Partial payments
            Env env (*this, features);
        ...
        }
        {
            ...
            env.fund (BMC (10000), alice, bob, carol, dan);
        ...
        }
        {
            ...
            env (pay (alice, dan, USDC (5)), path (bob, carol),
                sendmax (USDA (6)), txflags (tfNoBlocmatrixDirect));
        ...
        }
        {
            ...
            env.fund (BMC (10000), alice, bob, carol, dan, erin);
            ...
            env (pay (alice, erin, USDD (5)), path (carol, dan),
                path (bob, dan), txflags (tfNoBlocmatrixDirect));
        ...
        for (auto bobDanQIn : {80, 100, 120})
            for (auto bobAliceQOut : {80, 100, 120})
            {
                ...
                env(pay(dan, carol, USDA(10)),
                    path(bob), sendmax(USDD(100)), txflags(tfNoBlocmatrixDirect));
    ...
    void testBookStep (FeatureBitset features)
    {
        ...
        }
        {
            ...
            env (offer (bob, BTC (50), BMC (50)));
            env (offer (bob, BMC (50), USD (50)));

            env (pay (alice, carol, USD (50)), path (~BMC, ~USD),
                sendmax (BTC (50)));
            ...
            env.require (balance (bob, USD (0)));
            env.require (balance (carol, USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, BMC (50), USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, BTC (50), BMC (50)));
        }
        {
            // simple BMC -> USD through offer and sendmax
            Env env (*this, features);
            ...
            env.require (balance (bob, bmcMinusFee (env, 10000 + 50)));
            env.require (balance (bob, USD (0)));
            env.require (balance (carol, USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, BMC (50), USD (50)));
        }
        {
            // simple USD -> BMC through offer and sendmax
            Env env (*this, features);
            ...
            env (offer (bob, USD (50), BMC (50)));

            env (pay (alice, carol, BMC (50)), path (~BMC),
                 sendmax (USD (50)));

            env.require (balance (alice, USD (0)));
            env.require (balance (bob, bmcMinusFee (env, 10000 - 50)));
            env.require (balance (bob, USD (50)));
            env.require (balance (carol, BMC (10000 + 50)));
            BEAST_EXPECT(!isOffer (env, bob, USD (50), BMC (50)));
        }
        {
        ...
        }
        {
            ...
            env (pay (alice, carol, EUR (1)), path (~BMC, ~EUR),
                sendmax (USD (0.4)), txflags (tfNoBlocmatrixDirect|tfPartialPayment));
    ...
    void testTransferRate (FeatureBitset features)
    {
        ...
        }
        {
            ...
            env (pay (alice, carol, USD (50)), path (~USD), sendmax (BMC (50)));
            env.require (
                balance (alice, bmcMinusFee (env, 10000 - 50)),
...

        {
            ...
            env (pay (alice, carol, EUR (40)), path (~USD, ~EUR), sendmax (BMC (40)));
            env.require (
                balance (alice,  bmcMinusFee (env, 10000 - 40)),
                balance (bob, USD (40)),
                balance (bob, EUR (0)),
        ...
        }
        {
            ...
            env.fund (BMC (10000), alice, bob, carol, dan, gw);
            env(rate(gw, 1.25));
            ...
            env (pay (alice, carol, EUR (50)), path (bob, gw, ~EUR),
                sendmax (USDA (60)), txflags (tfNoBlocmatrixDirect));
            env.require (
                balance (bob, USD (-10)),
                balance (bob, USDA (60)),
                balance (dan, USD (50)),
...

        {
            ...
            env(rate(gw, 1.25));
            env.trust (USD (1000), alice, bob);
            env (offer (gw, BMC (100), USD (100)));
            env (pay (alice, bob, USD (100)),
                 sendmax (BMC (100)));
            env.require (
                balance (alice, bmcMinusFee(env, 10000-100)),
                balance (bob, USD (100)));
        }
        if (!features[featureOwnerPaysFee])
        {
            ...
            env(rate(gw, 1.25));
            env.trust (USD (1000), alice, bob);
            env (offer (gw, BMC (125), USD (125)));
            env (pay (alice, bob, USD (100)),
                 sendmax (BMC (200)));
            env.require (
                balance (alice, bmcMinusFee(env, 10000-125)),
                balance (bob, USD (100)));
        }
    }
    ...
    testFalseDry(FeatureBitset features)
    {
        ...
        env.fund (BMC(10000), alice, carol, gw);
        ...
        // Bob has _just_ slightly less than 50 bmc available
        // If his owner count changes, he will have more liquidity.
        // This is one error case to test (when Flow is used).
        // Computing the incomming bmc to the BMC/USD offer will require two
        // recursive calls to the EUR/BMC offer. The second call will return
        // tecPATH_DRY, but the entire path should not be marked as dry. This
        // is the second error case to test (when flowV1 is used).
        env (offer (bob, EUR (50), BMC (50)));
        env (offer (bob, BMC (50), USD (50)));

        env (pay (alice, carol, USD (1000000)), path (~BMC, ~USD),
            sendmax (EUR (500)),
    ...
    testLimitQuality ()
    {
        ...
        for (auto const& d : {-100 * timeDelta, +100 * timeDelta})
        {
            ...
            env (offer (bob, BMC (50), USD (50)));
            env (offer (bob, BMC (100), USD (50)));

            TER const expectedResult = closeTime < fix1141Time ()
                ? TER {tecPATH_DRY}
                : TER {tesSUCCESS};
            env (pay (alice, carol, USD (100)), path (~USD), sendmax (BMC (100)),
                txflags (tfNoBlocmatrixDirect | tfPartialPayment | tfLimitQuality),
                ter (expectedResult));
    ...
    static BMCAmount reserve(jtx::Env& env, std::uint32_t count)
    {
    ...
    void testSelfFundedBMCEndpoint (bool consumeOffer, FeatureBitset features)
    {
        // Test that the deferred credit table is not bypassed for
        // BMCEndpointSteps. If the account in the first step is sending BMC and
        // that account also owns an offer that receives BMC, it should not be
        // possible for that step to use the BMC received in the offer as part
        // of the payment.
        testcase("Self funded BMCEndpoint");
        ...
        env(offer(alice, BMC(50000), USD(10)));
        ...
        env(pay(alice, alice, toSend), path(~USD), sendmax(BMC(20000)),
            txflags(tfPartialPayment | tfNoBlocmatrixDirect));
    }
    ...
    void testUnfundedOffer (bool withFix, FeatureBitset features)
    {
        ...
        using namespace jtx;
        {
            ...
            env.fund(BMC(100000), alice, bob, gw);
            ...
            env(pay(alice, bob, tinyAmt1), path(~USD),
                sendmax(drops(9000000000)), txflags(tfNoBlocmatrixDirect));

            if (withFix)
                BEAST_EXPECT(!isOffer(env, gw, BMC(0), USD(0)));
            else
                BEAST_EXPECT(isOffer(env, gw, BMC(0), USD(0)));
        }
        {
            ...
            env(pay(alice, bob, drops(9000000000)), path(~BMC),
                sendmax(USD(1)), txflags(tfNoBlocmatrixDirect));

            if (withFix)
                BEAST_EXPECT(!isOffer(env, gw, USD(0), BMC(0)));
            else
                BEAST_EXPECT(isOffer(env, gw, USD(0), BMC(0)));
        }
    }
    ...
    testReexecuteDirectStep(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(!getNoBlocmatrixFlag(env, gw, alice, usdC));
        ...
        env(pay(alice, bob, BMC(10000)), path(~BMC), sendmax(USD(100)),
            txflags(tfPartialPayment | tfNoBlocmatrixDirect));
    }
    ...
    testRIPD1443(bool withFix)
    {
        ...
        env.fund(BMC(100000000), alice, noblocmatrix(bob), carol, gw);
        env.trust(gw["USD"](10000), alice, carol);
        env(trust(bob, gw["USD"](10000), tfSetNoBlocmatrix));
        env.trust(gw["USD"](10000), bob);
        env.close();

        // set no blocmatrix between bob and the gateway

        env(pay(gw, alice, gw["USD"](1000)));
        env.close();

        env(offer(alice, bob["USD"](1000), BMC(1)));
        env.close();

        env(pay(alice, alice, BMC(1)), path(gw, bob, ~BMC),
            sendmax(gw["USD"](1000)), txflags(tfNoBlocmatrixDirect),
        ...
        env(offer(alice, BMC(1000), bob["USD"](1000)));
        env.close();

        env(pay (carol, carol, gw["USD"](1000)), path(~bob["USD"], gw),
            sendmax(BMC(100000)), txflags(tfNoBlocmatrixDirect),
            ter(withFix ? TER {tecPATH_DRY} : TER {tesSUCCESS}));
        env.close();
    ...
    testRIPD1449(bool withFix)
    {
        ...
        // pay alice -> bmc -> USD/bob -> bob -> gw -> alice
        // set no blocmatrix on bob's side of the bob/gw trust line
        ...
        env(trust(bob, USD(10000), tfSetNoBlocmatrix));
        ...
        env(offer(carol, BMC(1), bob["USD"](1000)));
        env.close();

        env(pay(alice, alice, USD(1000)), path(~bob["USD"], bob, gw),
            sendmax(BMC(1)), txflags(tfNoBlocmatrixDirect),
            ter(withFix ? TER {tecPATH_DRY} : TER {tesSUCCESS}));
        env.close();
    }
    ...
    testSelfPayLowQualityOffer (FeatureBitset features)
    {
        ...
        // BMC than the offering account held.  This unit test reproduces
        // that failing case.
    ...
    testZeroOutputStep()
    {
        ...
        env(offer(bob, EUR(100), BMC(0.000001)));
        env.close();

        env(pay(alice, carol, BMC(1)),
            path(~EUR, ~BMC),
            sendmax(USD(1)),
            txflags(tfPartialPayment),
            ter(tecPATH_DRY));
    ...
    void testWithFeats(FeatureBitset features)
    {
        ...
        testSelfFundedBMCEndpoint(false, features);
        testSelfFundedBMCEndpoint(true, features);
...
BEAST_DEFINE_TESTSUITE_PRIO(Flow,app,blocmatrix,2);
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(Flow_manual,app,blocmatrix,4);

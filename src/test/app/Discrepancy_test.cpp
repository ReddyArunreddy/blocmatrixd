...
class Discrepancy_test : public beast::unit_test::suite
{
    ...
    void
    testBMCDiscrepancy (FeatureBitset features)
    {
        testcase ("Discrepancy test : BMC Discrepancy");
        ...
        env.fund(BMC(2000), A1);
        env.fund(BMC(1000), A2, A6, A7);
        env.fund(BMC(5000), A3);
        env.fund(BMC(1000000), A4);
        env.fund(BMC(600000), A5);
        env.close();
        ...
        env(offer(A4, BMC(49147), A2["JPY"](34501)));
        env(offer(A5, A3["CNY"](3150), BMC(80086)));
        env(offer(A7, BMC(1233), A6["CNY"](25)));
        env.close();

        test::PathSet payPaths {
            test::Path {A2["JPY"], A2},
            test::Path {BMC, A2["JPY"], A2},
            test::Path {A6, BMC, A2["JPY"], A2} };
...
public:
    void run () override
    {
        ...
        testBMCDiscrepancy (sa - featureFlow - fix1373 - featureFlowCross);
        testBMCDiscrepancy (sa               - fix1373 - featureFlowCross);
        testBMCDiscrepancy (sa                         - featureFlowCross);
        testBMCDiscrepancy (sa);
    }
};

BEAST_DEFINE_TESTSUITE (Discrepancy, app, blocmatrix);

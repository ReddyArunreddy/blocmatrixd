...
#include <blocmatrix/app/misc/TxQ.h>
#include <blocmatrix/app/tx/apply.h>
#include <blocmatrix/basics/Log.h>
#include <blocmatrix/basics/mulDiv.h>
...
class TxQ_test : public beast::unit_test::suite
{
        ...
        std::map<std::string, std::string> extraVoting = {})
    {
        ...
        if (!extraVoting.empty())
        {
            ...
            p->section("validation_seed").legacy("shUwVw52ofnCUX5m7kPTKzJdb4HEH");
        }
        return p;
    }
        ...
        std::uint32_t units, std::uint32_t reserve, std::uint32_t increment)
    {
        ...
        auto const flagPerLedger = 1 +
            blocmatrix::detail::supportedAmendments().size();
...
public:
    void testQueue()
    {
        ...
        env.fund(BMC(50000), noblocmatrix(alice, bob, charlie, daria));
        ...
        env.fund(BMC(50000), noblocmatrix(elmo, fred, gwen, hank));
    ...
    void testTecResult()
    {
        ...
        // Create accounts
        env.fund(BMC(50000), noblocmatrix(alice, gw));
        ...
        env(offer(alice, BMC(1000), USD(1000)), ter(tecUNFUNDED_OFFER));
        ...
        env(offer(alice, BMC(1000), USD(1000)), ter(terQUEUED));
    ...
    void testLocalTxRetry()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(BMC(50000), noblocmatrix(alice, bob, charlie));
    ...
    void testLastLedgerSeq()
    {
        ...
        env.fund(BMC(1000), noblocmatrix(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(BMC(1000), noblocmatrix(charlie, daria));
        env.close(env.now() + 5s, 10000ms);
        env.fund(BMC(1000), noblocmatrix(edgar, felicia));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testZeroFeeTxn()
    {
        ...
        env.fund(BMC(1000), noblocmatrix(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(BMC(1000), noblocmatrix(carol));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testPreclaimFailures()
    {
        ...
        env.fund(BMC(1000), noblocmatrix(alice));
        ...
        // Fail in preflight
        env(pay(alice, bob, BMC(-1000)),
            ter(temBAD_AMOUNT));

        // Fail in preclaim
        env(noop(alice), fee(BMC(100000)),
            ter(terINSUF_FEE_B));
    }
    ...
    void testMultiTxnPerAccount()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(drops(2000), noblocmatrix(alice));
        env.fund(BMC(500000), noblocmatrix(bob, charlie, daria));
        ...
        aliceFee = env.le(alice)->getFieldAmount(sfBalance).bmc().drops()
            - (59);
        env(noop(alice), seq(aliceSeq),
        ...
        // Alice is broke
        env.require(balance(alice, BMC(0)));
    ...
    void testAcctTxnID()
    {
        ...
        env.fund(BMC(50000), noblocmatrix(alice));
    ...
    void testUnexpectedBalanceChange()
    {
        ...
        env.fund(drops(5000), noblocmatrix(alice));
        env.fund(BMC(50000), noblocmatrix(bob));
        ...
        /* At this point, Alice's transaction is indefinitely
            ...
            will get more BMC, and it'll process.
        */
    ...
    void testBlockers()
    {
        ...
        env.fund(BMC(50000), noblocmatrix(alice, bob));
        env.memoize(charlie);
        env.memoize(daria);
    ...
    void testInFlightBalance()
    {
        ...
        env.fund(BMC(50000), noblocmatrix(alice, charlie), gw);
        ...
        //////////////////////////////////////////
        // Offer with high BMC out and low fee doesn't block
        auto aliceSeq = env.seq(alice);
        auto aliceBal = env.balance(alice);

        env.require(balance(alice, BMC(50000)),
            owners(alice, 0));

        // If this offer crosses, all of alice's
        // BMC will be taken (except the reserve).
        env(offer(alice, BUX(5000), BMC(50000)),
            queued);
        ...
        // has plenty of BMC, because the offer didn't
        // cross (of course).
        ...
        //////////////////////////////////////////
        // Offer with high BMC out and high total fee blocks later txs
        fillQueue(env, alice);
        ...
        env(offer(alice, BUX(5000), BMC(50000)), fee(drops(100)),
            queued);
        ...
        //////////////////////////////////////////
        // Offer with high BMC out and super high fee blocks later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        aliceSeq = env.seq(alice);
        aliceBal = env.balance(alice);
        ...
        env(offer(alice, BUX(5000), BMC(50000)), fee(drops(300)),
            queued);
        checkMetrics(env, 1, limit * 2, limit + 1, limit, 256);
        ...
        //////////////////////////////////////////
        // Offer with low BMC out allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        aliceSeq = env.seq(alice);
        aliceBal = env.balance(alice);
        ...
        // of alice's BMC will be taken.
        env(offer(alice, BUX(50), BMC(500)),
            queued);
        ...
        //////////////////////////////////////////
        // Large BMC payment doesn't block later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        ...
        // (minus the reserve).
        env(pay(alice, charlie, BMC(50000)),
            queued);
        ...
        //////////////////////////////////////////
        // Small BMC payment allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        ...
        env.require(balance(alice, aliceBal - BMC(500) - drops(20)),
            owners(alice, 0));
        ...
        // alice's BMC balance (other than the fee, of course).
        env(noop(alice), seq(aliceSeq + 1), queued);
        checkMetrics(env, 2, limit * 2, limit + 1, limit, 256);
        ...
        // BMC balance, but her USD balance went to charlie.
        env.require(balance(alice, aliceBal - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Large BMC to IOU payment doesn't block later txs.

        env(offer(gw, BMC(500000), USD(50000)));
        // Close so we don't have to deal
        // with tx ordering in consensus.
        env.close();
        ...
        // entire BMC balance to charlie in the
        // form of USD.
        BEAST_EXPECT(BMC(60000) > aliceBal);
        env(pay(alice, charlie, USD(1000)),
            sendmax(BMC(60000)), queued);
        ...
        // to charlie using only a portion of her BMC balance
        env.require(balance(alice, aliceBal - BMC(10000) - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Small BMC to IOU payment allows later txs.
        ...
        // a portion of her BMC balance to charlie
        // in the form of USD.
        BEAST_EXPECT(aliceBal > BMC(6001));
        env(pay(alice, charlie, USD(500)),
            sendmax(BMC(6000)), queued);
        ...
        // So once we close the ledger, alice sent a payment
        // to charlie using only a portion of her BMC balance
        env.require(balance(alice, aliceBal - BMC(5000) - drops(20)),
            balance(alice, USD(0)),
    ...
    void testExpirationReplacement()
    {
        ...
        */
        ...
        env.fund(BMC(500000), noblocmatrix(alice, bob));
        checkMetrics(env, 0, boost::none, 2, 1, 256);
    ...
    void testServerSubscribe()
    {
        ...
        env.fund(BMC(50000), noblocmatrix(a,b,c,d));
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        env.fund(BMC(50000), noblocmatrix(e,f,g,h,i));
    ...
    void testClearQueuedAccountTxs()
    {
        ...
        checkMetrics(env, 0, boost::none, 0, 3, 256);
        env.fund(BMC(50000000), alice, bob);
        ...
        testcase("clear queue failure (load)");
        {
            ...
            env(noop(alice), fee(BMC(1)), seq(aliceSeq++), ter(terQUEUED));
...
BEAST_DEFINE_TESTSUITE_PRIO(TxQ,app,blocmatrix,1);

...
{
    BMCAmount reserve(jtx::Env& env, std::uint32_t count)
    {
    ...
    static auto
    bmcMinusFee (jtx::Env const& env, std::int64_t bmcAmount)
        -> jtx::PrettyAmount
    {
        using namespace jtx;
        auto feeDrops = env.current()->fees().base;
        return drops (dropsPerBMC<std::int64_t>::value * bmcAmount - feeDrops);
    }
        ...
        jtx::Account const & acct_b, std::string const& currency)
    {
        ...
        jvParams[jss::blocmatrix_state][jss::currency] = currency;
        jvParams[jss::blocmatrix_state][jss::accounts] = Json::arrayValue;
        jvParams[jss::blocmatrix_state][jss::accounts].append(acct_a.human());
        jvParams[jss::blocmatrix_state][jss::accounts].append(acct_b.human());
...
public:
    void testRmFundedOffer (FeatureBitset features)
    {
        ...
        // be "taker gets" BMC. Old, broken would remove the first
        // "taker gets" bmc offer, even though the offer is still funded and
        ...
        env.fund (BMC (10000), alice, bob, carol, gw);
        ...
        // "taker gets" must be BMC
        // (Different amounts so I can distinguish the offers)
        env (offer (carol, BTC (49), BMC (49)));
        env (offer (carol, BTC (51), BMC (51)));

        // Offers for the poor quality path
        // Must be two offers at the same quality
        env (offer (carol, BMC (50), USD (50)));
        env (offer (carol, BMC (50), USD (50)));

        // Offers for the good quality path
        env (offer (carol, BTC (1), USD (100)));

        PathSet paths (Path (BMC, USD), Path (USD));
        ...
        BEAST_EXPECT(!isOffer (env, carol, BTC (1), USD (100)) &&
            isOffer (env, carol, BTC (49), BMC (49)));
    }
    ...
    void testCanceledOffer (FeatureBitset features)
    {
        ...
        env (offer (alice, BMC (500), USD (100)),
            require (offers (alice, 1)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, BMC (500), USD (100)));

        // cancel the offer above and replace it with a new offer
        auto const offer2Seq = env.seq (alice);

        env (offer (alice, BMC (300), USD (100)),
             json (jss::OfferSequence, offer1Seq),
             require (offers (alice, 1)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, BMC (300), USD (100)) &&
            !isOffer (env, alice, BMC (500), USD (100)));

        // Test canceling non-existent offer.
//      auto const offer3Seq = env.seq (alice);

        env (offer (alice, BMC (400), USD (200)),
             json (jss::OfferSequence, offer1Seq),
             require (offers (alice, 2)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, BMC (300), USD (100)) &&
            isOffer (env, alice, BMC (400), USD (200)));

        // Test cancellation now with OfferCancel tx
        auto const offer4Seq = env.seq (alice);
        env (offer (alice, BMC (222), USD (111)),
            require (offers (alice, 3)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, BMC (222), USD (111)));
        {
        ...
        env.require (offers (alice, 2));
        BEAST_EXPECT( isOffer (env, alice, BMC (300), USD (100))); // offer2
        BEAST_EXPECT(!isOffer (env, alice, BMC   (5), USD   (2))); // expired
    }
    ...
    void testBMCTinyPayment (FeatureBitset features)
    {
        testcase ("BMC Tiny payments");

        // Regression test for tiny bmc payments
        // In some cases, when the payment code calculates
        // the amount of bmc needed as input to an bmc->iou offer
        ...
        // 1) When we calculate the amount of input bmc needed for an offer
        //    from bmc->iou, the amount is less than 1 drop (after rounding
        ...
        for (auto withFix : {false, true})
        {
            ...
            env.fund (BMC (10000), alice, bob, carol, dan, erin, gw);
...

            {
                env (pay (alice, bob, USD (1)), path (~USD),
                    sendmax (BMC (102)),
                    txflags (tfNoBlocmatrixDirect | tfPartialPayment));
    ...
    void testEnforceNoBlocmatrix (FeatureBitset features)
    {
        testcase ("Enforce No Blocmatrix");
        ...
        {
            // No blocmatrix with an implied account step after an offer
            Env env {*this, features};
            auto const closeTime =
                fix1449Time () +
                    100 * env.closed ()->info ().closeTimeResolution;
            ...
            env.fund (BMC (10000), alice, noblocmatrix (bob), carol, dan, gw1, gw2);
            env.trust (USD1 (1000), alice, carol, dan);
            env(trust (bob, USD1 (1000), tfSetNoBlocmatrix));
            env.trust (USD2 (1000), alice, carol, dan);
            env(trust (bob, USD2 (1000), tfSetNoBlocmatrix));

            env (pay (gw1, dan, USD1 (50)));
            env (pay (gw1, bob, USD1 (50)));
            env (pay (gw2, bob, USD2 (50)));

            env (offer (dan, BMC (50), USD1 (50)));

            env (pay (alice, carol, USD2 (50)), path (~USD1, bob),
                sendmax (BMC (50)), txflags (tfNoBlocmatrixDirect),
                ter(tecPATH_DRY));
        }
        {
            ...
            env.fund (BMC (10000), alice, bob, carol, dan, gw1, gw2);
            ...
            env.require (balance (alice, bmcMinusFee (env, 10000 - 50)));
    ...
    testInsufficientReserve (FeatureBitset features)
    {
        ...
        // No crossing:
        {
            ...
            env (offer (alice, bmcOffer, usdOffer),
        ...
        // Partial cross:
        {
            ...
            auto const bmcOffer2 = BMC (500);

            env.fund (r + f + bmcOffer, bob);
            env (offer (bob, usdOffer2, bmcOffer2),   ter(tesSUCCESS));
            env.fund (r + f, alice);
            env (trust (alice, usdOffer),             ter(tesSUCCESS));
            env (pay (gw, alice, usdOffer),           ter(tesSUCCESS));
            env (offer (alice, bmcOffer, usdOffer),   ter(tesSUCCESS));

            env.require (
                balance (alice, r - f + bmcOffer2),
                balance (alice, usdOffer2),
                owners (alice, 1),
                balance (bob, r + bmcOffer2),
        ...
        // buy BMC. If it fully crosses, we succeed.
        {
            Env env {*this, features};
            auto const closeTime =
                fix1449Time () +
            ...
            env.fund (r + f + bmcOffer, bob, carol);
            env (offer (bob, usdOffer2, bmcOffer2),    ter(tesSUCCESS));
            env (offer (carol, usdOffer, bmcOffer),    ter(tesSUCCESS));
            ...
            env.require (
                balance (alice, r - f + bmcOffer),
                balance (alice, USD(0)),
                owners (alice, 1),
                balance (bob, r + bmcOffer2),
                balance (bob, usdOffer2),
                owners (bob, 1),
                balance (carol, r + bmcOffer2),
    ...
    testFillModes (FeatureBitset features)
    {
        ...
        auto const startBalance = BMC (1000000);
        auto const gw = Account {"gateway"};
        auto const alice = Account {"alice"};
        auto const bob = Account {"bob"};
        auto const USD = gw["USD"];
            ...
            {features - fix1578, features | fix1578})
        {
            ...
            env (offer (bob, USD (500), BMC (500)), ter(tesSUCCESS));
            env.close();
            env.require (
                owners (bob, 2),
                offers (bob, 2));
            ...
            // Order that can't be filled but will remove bob's expired offer:
            {
                ...
                env (offer (alice, BMC (1000), USD (1000)),
            ...
            env (offer (alice, BMC (500), USD (500)),
                txflags (tfFillOrKill),              ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - (f * 3) + BMC (500)),
                balance (alice, USD (500)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - (f * 2) - BMC (500)),
                balance (bob, USD (500)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // and add nothing on the books:
        {
            ...
            // Partially cross:
            env (offer (bob, USD (50), BMC (50)),           ter(tesSUCCESS));
            env (offer (alice, BMC (1000), USD (1000)),
                txflags (tfImmediateOrCancel),              ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f + BMC (50)),
                balance (alice, USD (950)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - BMC (50)),
                balance (bob, USD (50)),
                owners (bob, 1),
                offers (bob, 0));
            ...
            // Fully cross:
            env (offer (bob, USD (50), BMC (50)),           ter(tesSUCCESS));
            env (offer (alice, BMC (50), USD (50)),
                txflags (tfImmediateOrCancel),              ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f - f + BMC (100)),
                balance (alice, USD (900)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - f - BMC (100)),
                balance (bob, USD (100)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // tfPassive -- place the offer without crossing it.
        {
            ...
            env (offer (alice, USD(1000), BMC(2000)));
            env.close();
            ...
            for (auto offerPtr : aliceOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerGets] == BMC (2000));
            ...
            env (offer (bob, BMC(2000), USD(1000), tfPassive));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == BMC (2000));
            }

            // It should be possible for gw to cross both of those offers.
            env (offer (gw, BMC(2000), USD(1000)));
            env.close();
            env.require (offers (alice, 0));
            env.require (offers (gw, 0));
            env.require (offers (bob, 1));

            env (offer (gw, USD(1000), BMC(2000)));
            env.close();
        ...
        // tfPassive -- cross only offers of better quality.
        {
            ...
            env (pay (gw, "bob", USD(1000)));
            env (offer ("alice", USD(500), BMC(1001)));
            env.close();

            env (offer ("alice", USD(500), BMC(1000)));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == BMC (999));
            }
        }
    }
    ...
    testMalformed(FeatureBitset features)
    {
        ...
        env (offer (alice, USD (1000), BMC (1000)),
        ...
        {
            // Alice tries an BMC to BMC order:
            env (offer (alice, BMC (1000), BMC (1000)),   ter(temBAD_OFFER));
            env.require (
                owners (alice, 0),
                offers (alice, 0));
        ...
        {
            env (offer (alice, -USD (1000), BMC (1000)),  ter(temBAD_OFFER));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        ...
        // Use BMC as a currency code
        {
            auto const BAD = IOU(gw, badCurrency());

            env (offer (alice, BMC (1000), BAD (1000)),   ter(temBAD_CURRENCY));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        }
    ...
    testExpiration(FeatureBitset features)
    {
        ...
        env (offer (bob, usdOffer, bmcOffer),     ter(tesSUCCESS));
        env.require (
            balance (alice, startBalance - f - f - f),
            balance (alice, usdOffer),
            offers (alice, 0),
    ...
    testUnfundedCross(FeatureBitset features)
    {
        ...
        env (offer ("alice", usdOffer, bmcOffer),     ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env (offer ("bob", usdOffer, bmcOffer),       ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env.fund (reserve (env, 0) + f + BMC(1), "carol");
        env (offer ("carol", usdOffer, bmcOffer), ter(tecINSUF_RESERVE_OFFER));
        env.require (
            balance ("carol", reserve (env, 0) + BMC(1)),
            owners ("carol", 0));

        // Account has enough for the reserve plus one
        // offer, and the fee.
        env.fund (reserve (env, 1) + f, "dan");
        env (offer ("dan", usdOffer, bmcOffer),       ter(tesSUCCESS));
        env.require (
        ...
        env.fund (reserve (env, 1) + f + bmcOffer, "eve");
        env (offer ("eve", usdOffer, bmcOffer),       ter(tesSUCCESS));
        env.require (
            balance ("eve", reserve (env, 1) + bmcOffer),
            owners ("eve", 1));
    }
    ...
    testSelfCross(bool use_partner, FeatureBitset features)
    {
        ...
        env.fund (BMC (10000), gw);
        if (use_partner)
        {
            env.fund (BMC (10000), partner);
        ...
        // through BMC
        env (offer (account_to_test, BTC (250), BMC (1000)));
        env.require (offers(account_to_test, 1));

        // validate that the book now shows a BTC for BMC offer
        BEAST_EXPECT(isOffer(env, account_to_test, BTC(250), BMC(1000)));

        auto const secondLegSeq = env.seq(account_to_test);
        env (offer (account_to_test, BMC(1000), USD (50)));
        env.require (offers (account_to_test, 2));

        // validate that the book also shows a BMC for USD offer
        BEAST_EXPECT(isOffer(env, account_to_test, BMC(1000), USD(50)));
        ...
        // old and the new behavior.
        {
            ...
            for (auto const& offerPtr : acctOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == BMC (0));
            }
        }
    ...
    testNegativeBalance(FeatureBitset features)
    {
        ...
        env (offer (alice, USD (50), BMC (150000)));
        ...
        // small_amount and transfer no BMC.  The new offer crossing transfers
        ...
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "50");
        BEAST_EXPECT(env.balance (alice, bmcIssue()) ==
        ...
        BEAST_EXPECT(env.balance (bob, bmcIssue()) ==
            bob_initial_balance -
    ...
    void
    testOfferCrossWithBMC(bool reverse_order, FeatureBitset features)
    {
        testcase (std::string("Offer Crossing with BMC, ") +
        ...
        env.fund (BMC (10000), gw, alice, bob);
        ...
        if (reverse_order)
            env (offer (bob, USD (1), BMC (4000)));

        env (offer (alice, BMC (150000), USD (50)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                BMC (10000).value ().mantissa () -
                BMC (reverse_order ? 4000 : 3000).value ().mantissa () -
    ...
    testOfferCrossWithLimitOverride(FeatureBitset features)
    {
        ...
        env.fund (BMC (100000), gw, alice, bob);
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                BMC (100000).value ().mantissa () -
                BMC (3000).value ().mantissa () -
    ...
    testOfferAcceptThenCancel(FeatureBitset features)
    {
        ...
        env (offer (env.master, BMC (500), USD (100)));
        env.close ();
    ...
    testCurrencyConversionEntire(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == BMC (500).value ().getText ());
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] ==
            USD (100).value ().getJson (JsonOptions::none));

        env (pay (alice, alice, BMC (500)), sendmax (USD (100)));
    ...
    testCurrencyConversionIntoDebt(FeatureBitset features)
    {
        ...
        env.fund (BMC (10000), alice, bob, carol);
    ...
    testCurrencyConversionInParts(FeatureBitset features)
    {
        ...
        env (pay (alice, alice, BMC (200)), sendmax (USD (100)));

        // The previous payment reduced the remaining offer amount by 200 BMC
        auto jro = ledgerEntryOffer (env, bob, bobOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == BMC (300).value ().getText ());
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] ==
        ...
        // alice now has 200 more BMC from the payment
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                BMC (10000).value ().mantissa () +
                BMC (200).value ().mantissa () -
                env.current ()->fees ().base * 2)
        );
        ...
        // Alice converts USD to BMC which should fail
        // due to PartialPayment.
        env (pay (alice, alice, BMC (600)), sendmax (USD (100)),
            ter(tecPATH_PARTIAL));

        // Alice converts USD to BMC, should succeed because
        ...
        // only 300 BMC should be have been payed since that's all
    ...
    void
    testCrossCurrencyStartBMC(FeatureBitset features)
    {
        testcase ("Cross Currency Payment: Start with BMC");
        ...
        env.fund (BMC (10000), gw, alice, bob, carol);
        ...
        env (offer (carol, BMC (500), USD (50)));

        env (pay (alice, bob, USD (25)), sendmax (BMC (333)));
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == BMC (250).value ().getText ());
    }

    void
    testCrossCurrencyEndBMC(FeatureBitset features)
    {
        testcase ("Cross Currency Payment: End with BMC");
        ...
        auto const carolOfferSeq = env.seq (carol);
        env (offer (carol, USD (50), BMC (500)));

        env (pay (alice, bob, BMC (250)), sendmax (USD (333)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                BMC (10000).value ().mantissa () +
                BMC (250).value ().mantissa ())
        );

        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == BMC (250).value ().getText ());
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] ==
            USD (25).value ().getJson (JsonOptions::none));
    }
    ...
    testCrossCurrencyBridged(FeatureBitset features)
    {
        ...
        env.fund (BMC (10000), gw1, gw2, alice, bob, carol, dan);
        ...
        env (offer (dan, BMC (500), EUR (50)));

        Json::Value jtp {Json::arrayValue};
        jtp[0u][0u][jss::currency] = "BMC";
        ...
        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == BMC (200).value ().getText ());
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] ==
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == BMC (200).value ().getText ());
    }
    ...
    testBridgedSecondLegDry(FeatureBitset features)
    {
        ...
        env.fund(BMC(100000000), alice, bob, carol, gw);
        ...
        env (offer (alice, EUR(2), BMC(1)));
        env (offer (alice, EUR(2), BMC(1)));

        env (offer (alice, BMC(1), USD(4)));
        env (offer (carol, BMC(1), USD(3)));
        env.close();

        // Bob offers to buy 10 USD for 10 EUR.
        //  1. He spends 2 EUR taking Alice's auto-bridged offers and
        //     gets 4 USD for that.
        //  2. He spends another 2 EUR taking Alice's last EUR->BMC offer and
        //     Carol's BMC-USD offer.  He gets 3 USD for that.
        // The key for this test is that Alice's BMC->USD leg goes dry before
        // Alice's EUR->BMC.  The BMC->USD leg is the second leg which showed
        // some sensitivity.
    ...
    testOfferFeesConsumeFunds(FeatureBitset features)
    {
        ...
        //  1 for payment          == 4
        auto const starting_bmc = BMC (100) +
            env.current ()->fees ().accountReserve (3) +
            env.current ()->fees ().base * 4;

        env.fund (starting_bmc, gw1, gw2, gw3, alice, bob);
        ...
        env (offer (bob, BMC (200), USD1 (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD1 (200), BMC (200)));

        auto jrr = ledgerEntryState (env, alice, gw1, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == BMC (350).value ().getText ()
        );
    ...
    testSellFlagBasic(FeatureBitset features)
    {
        ...
        env.fund (starting_bmc, gw, alice, bob);
        ...
        env (offer (bob, BMC (200), USD (200)), json(jss::Flags, tfSell));
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD (200), BMC (200)), json(jss::Flags, tfSell));

        auto jrr = ledgerEntryState (env, alice, gw, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "-100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == BMC (250).value ().getText ()
        );
    ...
    testSellFlagExceedLimit(FeatureBitset features)
    {
        ...
        env (offer (bob, BMC (100), USD (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        // Taker pays 100 USD for 100 BMC.
        // Selling BMC.
        // Will sell all 100 BMC and get more USD than asked for.
        env (offer (alice, USD (100), BMC (100)), json(jss::Flags, tfSell));
    ...
    testGatewayCrossCurrency(FeatureBitset features)
    {
        ...
        auto const starting_bmc = BMC (100.1) +
            env.current ()->fees ().accountReserve (1) +
            env.current ()->fees ().base * 2;
        ...
        if (wsc->version() == 2)
        {
            ...
            BEAST_EXPECT(jrr.isMember(jss::blocmatribmcc) && jrr[jss::blocmatribmcc] == "2.0");
    ...
    void testPartialCross (FeatureBitset features)
    {
        ...
        env.fund (BMC(10000000), gw);

        // The fee that's charged for transactions
        auto const f = env.current ()->fees ().base;

        // To keep things simple all offers are 1 : 1 for BMC : USD.
        ...
        struct TestData
        {
            ...
            int bookAmount;            // USD -> BMC offer on the books
            preTrustType preTrust;     // If true, pre-establish trust line
            int offerAmount;           // Account offers this much BMC -> USD
...
{"flo",             reserve (env, 1) + 0*f,       1,   noPreTrust,     1000,             tesSUCCESS, BMC(   1) + f, USD(      1),      0,     1},
{"gay",             reserve (env, 1) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, BMC(  50) + f, USD(     50),      0,     1},
{"hye", BMC(1000)                    + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, BMC( 800) + f, USD(    800),      0,     1},
{"ivy", BMC(   1) + reserve (env, 1) + 1*f,       1,   noPreTrust,     1000,             tesSUCCESS, BMC(   1) + f, USD(      1),      0,     1},
{"joy", BMC(   1) + reserve (env, 2) + 1*f,       1,   noPreTrust,     1000,             tesSUCCESS, BMC(   1) + f, USD(      1),      1,     2},
{"kim", BMC( 900) + reserve (env, 2) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, BMC( 999) + f, USD(    999),      0,     1},
{"liz", BMC( 998) + reserve (env, 0) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, BMC( 998) + f, USD(    998),      0,     1},
{"meg", BMC( 998) + reserve (env, 1) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, BMC( 999) + f, USD(    999),      0,     1},
{"nia", BMC( 998) + reserve (env, 2) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, BMC( 999) + f, USD(    999),      1,     2},
{"ova", BMC( 999) + reserve (env, 0) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, BMC( 999) + f, USD(    999),      0,     1},
{"pam", BMC( 999) + reserve (env, 1) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, BMC(1000) + f, USD(   1000),      0,     1},
{"rae", BMC( 999) + reserve (env, 2) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, BMC(1000) + f, USD(   1000),      0,     1},
{"sue", BMC(1000) + reserve (env, 2) + 1*f,       0,   noPreTrust,     1000,             tesSUCCESS,             f, USD(      0),      1,     1},
...
{"eli", BMC(  20) + reserve (env, 0) + 1*f,    1000,   gwPreTrust,     1000,             tesSUCCESS, BMC(20) + 1*f, USD(     20),      0,     0},
{"fyn",             reserve (env, 1) + 0*f,       0,   gwPreTrust,     1000,             tesSUCCESS,             f, USD(      0),      1,     1},
{"gar",             reserve (env, 1) + 0*f,       1,   gwPreTrust,     1000,             tesSUCCESS, BMC(   1) + f, USD(      1),      1,     1},
{"hal",             reserve (env, 1) + 1*f,       1,   gwPreTrust,     1000,             tesSUCCESS, BMC(   1) + f, USD(      1),      1,     1},
...
{"ted", BMC(  20) + reserve (env, 1) + 2*f,    1000, acctPreTrust,     1000,             tesSUCCESS, BMC(20) + 2*f, USD(     20),      0,     1},
{"uli",             reserve (env, 2) + 0*f,       0, acctPreTrust,     1000, tecINSUF_RESERVE_OFFER,           2*f, USD(      0),      0,     1},
{"vic",             reserve (env, 2) + 0*f,       1, acctPreTrust,     1000,             tesSUCCESS, BMC( 1) + 2*f, USD(      1),      0,     1},
{"wes",             reserve (env, 2) + 1*f,       0, acctPreTrust,     1000,             tesSUCCESS,           2*f, USD(      0),      1,     2},
{"xan",             reserve (env, 2) + 1*f,       1, acctPreTrust,     1000,             tesSUCCESS, BMC( 1) + 2*f, USD(      1),      1,     2},
};
        ...
        for (auto const& t : tests)
        {
            ...
            if (book)
                env (offer (gw, BMC (book), USD (book)));
            env.close();
            ...
            // to be enough BMC reserve for acct to create the trust line.
            ...
            env (offer (acct, USD (acctOffer), BMC (acctOffer)), ter (t.tec));
            env.close();
            std::uint32_t const acctOfferSeq = env.seq (acct) - 1;

            BEAST_EXPECT (env.balance (acct, USD.issue()) == t.balanceUsd);
            BEAST_EXPECT (
                env.balance (acct, bmcIssue()) == t.fundXrp - t.spentXrp);
            ...
            if (acctOffers.size() && t.offers)
            {
                ...
                BEAST_EXPECT (acctOffer[sfTakerGets] == BMC (leftover));
    ...
    void
    testBMCDirectCross (FeatureBitset features)
    {
        testcase ("BMC Direct Crossing");
        ...
        // The scenario:
        //   o alice has USD but wants BMC.
        //   o bob has BMC but wants USD.
        auto const alicesBMC = env.balance (alice);
        auto const bobsBMC = env.balance (bob);

        env (offer (alice, bmcOffer, usdOffer));
        env.close();
        env (offer (bob, usdOffer, bmcOffer));

        env.close();
        env.require (
            balance (alice, USD(0)),
            balance (bob, usdOffer),
            balance (alice, alicesBMC + bmcOffer - fee),
            balance (bob,   bobsBMC   - bmcOffer - fee),
            offers (alice, 0),
            offers (bob, 0));
        ...
        env (offer (alice, USD(999), BMC(999)));
        env (offer (bob, bmcOffer, usdOffer));
        ...
        verifyDefaultTrustline (env, bob, USD(1));
        {
            ...
            BEAST_EXPECT (bobsOffer[sfTakerPays] == BMC (1));
        }
   }
    ...
    testBridgedCross (FeatureBitset features)
    {
        ...
        env.fund (BMC(1000000), gw, alice, bob, carol);
        env.close();
        ...
        env (offer (alice, BMC(1000), usdOffer));
        env (offer (bob, eurOffer, BMC(1000)));
        ...
        env.require (
            ...
            balance (bob, bobXrpBalance - BMC(400)),
            offers (carol, 0));
        ...
        verifyDefaultTrustline (env, carol, USD(400));
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == BMC (600));
        }
        {
            auto const bobsOffers = offersOnAccount (env, bob);
            BEAST_EXPECT (bobsOffers.size() == 1);
            ...
            BEAST_EXPECT (bobsOffer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (bobsOffer[sfTakerGets] == BMC (600));
        ...
        env.require (
            ...
            balance (bob, bobXrpBalance - BMC(1000)),
            offers (bob, 0),
            offers (carol, 0));
        ...
        if (alicesOffers.size() != 0)
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == BMC (0));
        }
    }
    ...
    testSellOffer (FeatureBitset features)
    {
        ...
        {
            std::string account;       // Account operated on
            STAmount fundXrp;          // BMC acct funded with
            ...
            TestData (
                ...
                STAmount const& fundXrp_,       // BMC acct funded with
...
{"ann", BMC(10) + reserve (env, 0) + 1*f, USD( 0), BMC(10), USD( 5),  USD(10),  BMC(10), tecINSUF_RESERVE_OFFER, BMC( 0) + (1*f),  USD( 0),      0,     0},
{"bev", BMC(10) + reserve (env, 1) + 1*f, USD( 0), BMC(10), USD( 5),  USD(10),  BMC(10),             tesSUCCESS, BMC( 0) + (1*f),  USD( 0),      1,     1,     BMC(10),  USD(10)},
{"cam", BMC(10) + reserve (env, 0) + 1*f, USD( 0), BMC(10), USD(10),  USD(10),  BMC(10),             tesSUCCESS, BMC(10) + (1*f),  USD(10),      0,     1},
{"deb", BMC(10) + reserve (env, 0) + 1*f, USD( 0), BMC(10), USD(20),  USD(10),  BMC(10),             tesSUCCESS, BMC(10) + (1*f),  USD(20),      0,     1},
{"eve", BMC(10) + reserve (env, 0) + 1*f, USD( 0), BMC(10), USD(20),  USD( 5),  BMC( 5),             tesSUCCESS, BMC( 5) + (1*f),  USD(10),      0,     1},
{"flo", BMC(10) + reserve (env, 0) + 1*f, USD( 0), BMC(10), USD(20),  USD(20),  BMC(20),             tesSUCCESS, BMC(10) + (1*f),  USD(20),      0,     1},
{"gay", BMC(20) + reserve (env, 1) + 1*f, USD( 0), BMC(10), USD(20),  USD(20),  BMC(20),             tesSUCCESS, BMC(10) + (1*f),  USD(20),      0,     1},
{"hye", BMC(20) + reserve (env, 2) + 1*f, USD( 0), BMC(10), USD(20),  USD(20),  BMC(20),             tesSUCCESS, BMC(10) + (1*f),  USD(20),      1,     2,     BMC(10),  USD(10)},
// acct pays USD
{"meg",           reserve (env, 1) + 2*f, USD(10), USD(10), BMC( 5),  BMC(10),  USD(10), tecINSUF_RESERVE_OFFER, BMC(  0) + (2*f),  USD(10),      0,     1},
{"nia",           reserve (env, 2) + 2*f, USD(10), USD(10), BMC( 5),  BMC(10),  USD(10),             tesSUCCESS, BMC(  0) + (2*f),  USD(10),      1,     2,     USD(10),  BMC(10)},
{"ova",           reserve (env, 1) + 2*f, USD(10), USD(10), BMC(10),  BMC(10),  USD(10),             tesSUCCESS, BMC(-10) + (2*f),  USD( 0),      0,     1},
{"pam",           reserve (env, 1) + 2*f, USD(10), USD(10), BMC(20),  BMC(10),  USD(10),             tesSUCCESS, BMC(-20) + (2*f),  USD( 0),      0,     1},
{"qui",           reserve (env, 1) + 2*f, USD(10), USD(20), BMC(40),  BMC(10),  USD(10),             tesSUCCESS, BMC(-20) + (2*f),  USD( 0),      0,     1},
{"rae",           reserve (env, 2) + 2*f, USD(10), USD( 5), BMC( 5),  BMC(10),  USD(10),             tesSUCCESS, BMC( -5) + (2*f),  USD( 5),      1,     2,     USD( 5),  BMC( 5)},
{"sue",           reserve (env, 2) + 2*f, USD(10), USD( 5), BMC(10),  BMC(10),  USD(10),             tesSUCCESS, BMC(-10) + (2*f),  USD( 5),      1,     2,     USD( 5),  BMC( 5)},
};
    ...
    testSellWithFillOrKill (FeatureBitset features)
    {
        ...
        }
        {
            ...
            env (offer(alice, USD(20), BMC(2000), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            env (offer(bob, BMC(2000), USD(20)));
            env.close();
            env (offer(alice, USD(10), BMC(1500), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            // We're using bob's left-over offer for BMC(500), USD(5)
            env (offer(alice, USD(1), BMC(501),
                tfSell | tfFillOrKill), ter (killedCode));
            env.close();
            env.require (balance (alice, USD(35)));
            env.require (offers (alice, 0));
        ...
        }
        {
            ...
            // We're using bob's left-over offer for BMC(500), USD(5)
            env (offer(alice, USD(1), BMC(500), tfSell | tfFillOrKill));
            env.close();
    ...
    testTransferRateOffer (FeatureBitset features)
    {
        ...
        env(rate(gw1, 1.25));
        {
            ...
            auto const bob = Account("bob");
            env.fund (BMC(100) + reserve(env, 2) + (fee*2), ann, bob);
            env.close();
            ...
            // bob offers to sell USD(100) for BMC.  alice takes bob's offer.
            ...
            env (offer (bob, BMC(1), USD(100)));
            env.close();

            env (offer (ann, USD(100), BMC(1)));
            env.close();

            env.require (balance (ann, USD(100)));
            env.require (balance (ann, BMC( 99) + reserve(env, 2)));
            env.require (offers (ann, 0));

            env.require (balance (bob, USD(  0)));
            env.require (balance (bob, BMC(101) + reserve(env, 2)));
            env.require (offers (bob, 0));
        }
        {
            // Reverse the order, so the offer in the books is to sell BMC
            // in return for USD.  Gateway rate should still apply identically.
            auto const che = Account("che");
            auto const deb = Account("deb");
            env.fund (BMC(100) + reserve(env, 2) + (fee*2), che, deb);
            env.close();
            ...
            env (offer (che, USD(100), BMC(1)));
            env.close();

            env (offer (deb, BMC(1), USD(100)));
            env.close();

            env.require (balance (che, USD(100)));
            env.require (balance (che, BMC( 99) + reserve(env, 2)));
            env.require (offers (che, 0));

            env.require (balance (deb, USD(  0)));
            env.require (balance (deb, BMC(101) + reserve(env, 2)));
        ...
        }
        {
            ...
            env.fund (BMC(20000) + fee*2, eve, fyn);
            env.close();
            ...
            env (offer (eve, USD(10), BMC(4000)));
            env.close();
            std::uint32_t const eveOfferSeq = env.seq (eve) - 1;

            env (offer (fyn, BMC(2000), USD(5)));
            env.close();

            env.require (balance (eve, USD(105)));
            env.require (balance (eve, BMC(18000)));
            ...
            if (evesOffers.size() != 0)
            {
                ...
                BEAST_EXPECT (evesOffer[sfTakerGets] == BMC (2000));
            ...
            env.require (balance (fyn, BMC(22000)));
        ...
        env.fund (BMC(100000), gw2);
        env.close();

        env(rate(gw2, 1.5));
        {
            // Remove BMC from the equation.  Give the two currencies two
        ...
        }
        {
            ...
            env.fund (BMC(2) + reserve(env, 3) + (fee*3), ova, pat, qae);
            env.close();

            //   o ova has USD but wants XPR.
            //   o pat has BMC but wants EUR.
            ...
            env (offer (ova, BMC(2), USD(100)));
            env (offer (pat, EUR(100), BMC(2)));
            env.close();
            ...
            env.require (balance (ova, BMC(4) + reserve(env, 3)));
    ...
    testSelfCrossOffer1 (FeatureBitset features)
    {
        ...
        for (auto const& offerPtr : gwOffers)
        {
            auto const& offer = *offerPtr;
            BEAST_EXPECT (offer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (offer[sfTakerGets] == BMC (600));
        ...
        for (auto const& offerPtr : gwOffers)
        {
            ...
            BEAST_EXPECT (offer[sfTakerPays] == BMC (1000));
        }
    }
    ...
    testSelfCrossOffer2 (FeatureBitset features)
    {
        ...
        struct TestData
        {
            ...
            STAmount fundBMC;          // BMC acct funded with
            STAmount fundUSD;          // USD acct funded with
        ...
        TestData const tests[]
        {
// acct               fundBMC    fundUSD    fundEUR           firstOfferTec          secondOfferTec
...
{"flo", reserve(env, 3) +   0, USD(1000), EUR(1000), tecINSUF_RESERVE_OFFER, tecINSUF_RESERVE_OFFER},
    ...
    testSelfIssueOffer (FeatureBitset features)
    {
        ...
        env.fund(BMC(50000) + f, alice, bob);
        env.close();

        env(offer(alice, USD(5000), BMC(50000)));
        env.close();

        // This offer should take alice's offer up to Alice's reserve.
        env(offer(bob, BMC(50000), USD(5000)));
        env.close();

        // alice's offer should have been removed, since she's down to her
        // BMC reserve.
        env.require (balance (alice, BMC(250)));
        ...
        for (auto const& offerPtr : bobOffers)
        {
            ...
            BEAST_EXPECT(offer[sfTakerPays] == BMC (250));
        }
    }
    ...
    void testSelfCrossLowQualityOffer (FeatureBitset features)
    {
        ...
        // for more BMC than the offering account held.  This unit test
    ...
    void testSelfPayXferFeeOffer (FeatureBitset features)
    {
        ...
        //
        //  2. alice makes an offer to buy BMC and sell USD.
        //  3. bob makes an offer to buy BTC and sell BMC.
        //
        ...
        //  o bob' offer which takes alice's BTC and sells BMC to...
        //  o alice's first offer which takes bob's BMC and sells USD to...
        ...
        auto const BTC = gw["BTC"];
        auto const USD = gw["USD"];
        auto const startXrpBalance = BMC (4000000);
        ...
        struct Actor
        {
            ...
            PrettyAmount bmc;  // final expected after crossing
...
// { 0, 0, 0, BTC( 5), { {"flo", 0, drops(3999999999950), BTC( 5.0), USD(2000)}                                                      } }  // no xfer fee
        ...
        for (auto const& t : tests)
        {
            ...
            {
                env.fund (BMC (4000000), actor.acct);
                env.close();
            ...
            env (offer (leg0, BTC(10),     BMC(100000), tfPassive));
            env.close();
            std::uint32_t const leg0OfferSeq = env.seq (leg0) - 1;

            env (offer (leg1, BMC(100000), USD(1000),   tfPassive));
            env.close();
            ...
            for (auto const& actor : t.actors)
            {
                ...
                env.require (balance (actor.acct, actor.bmc));
...
{ 0, 0, 0, BTC( 5), { {"hye", 2, drops(3999999999950), BTC(5), USD(2000)}                                                      } }  // no xfer fee
    ...
    void testRequireAuth (FeatureBitset features)
    {
        ...
        env.fund (BMC(400000), gw, alice, bob);
        env.close();
        ...
        env (offer (alice, gwUSD(40), BMC(4000)));
        env.close();
        ...
        env (offer (bob, BMC(4000), gwUSD(40)));
        env.close();
    ...
    void testMissingAuth (FeatureBitset features)
    {
        ...
        // untouched.
        env (offer (alice, gwUSD(40), BMC(4000)), ter(tecNO_LINE));
        env.close();
        ...
        env (offer (alice, gwUSD(40), BMC(4000)), ter(tecNO_AUTH));
        env.close();
    ...
    {
        testcase("BlocmatrixConnect Smoketest payment flow");
        using namespace jtx;
        ...
        // This test mimics the payment flow used in the Blocmatrix Connect
        ...
        env.fund (BMC(100000), hotUS, coldUS, hotEU, coldEU, mm);
        env.close();

        // Cold wallets require trust but will blocmatrix by default
        for (auto const& cold : {coldUS, coldEU})
        {
            env(fset (cold, asfRequireAuth));
            env(fset (cold, asfDefaultBlocmatrix));
        }
        env.close();

        // Each hot wallet trusts the related cold wallet for a large amount
        env (trust(hotUS, USD(10000000)), txflags (tfSetNoBlocmatrix));
        env (trust(hotEU, EUR(10000000)), txflags (tfSetNoBlocmatrix));
        // Market maker trusts both cold wallets for a large amount
        env (trust(mm, USD(10000000)), txflags (tfSetNoBlocmatrix));
        env (trust(mm, EUR(10000000)), txflags (tfSetNoBlocmatrix));
        env.close();
        ...
        // There should be a path available from hot US to cold EUR
        {
            ...
            Json::Value const jrr {env.rpc(
                "json", "blocmatrix_path_find", to_string(jvParams))[jss::result]};
    ...
    void testSelfAuth (FeatureBitset features)
    {
        ...
        env.fund (BMC(400000), gw, alice);
        env.close();

        // Test that gw can create an offer to buy gw's currency.
        env (offer (gw, gwUSD(40), BMC(4000)));
        env.close();
        std::uint32_t const gwOfferSeq = env.seq (gw) - 1;
        env.require (offers (gw, 1));
        ...
        env (offer (alice, BMC(4000), gwUSD(40)));
        env.close();
    ...
    void testAll(FeatureBitset features)
    {
        ...
        testBMCTinyPayment(features);
        testEnforceNoBlocmatrix(features);
        ...
        testOfferCrossWithBMC(true, features);
        testOfferCrossWithBMC(false, features);
        ...
        testCrossCurrencyStartBMC(features);
        testCrossCurrencyEndBMC(features);
...
BEAST_DEFINE_TESTSUITE_PRIO (Offer, tx, blocmatrix, 4);
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO (Offer_manual, tx, blocmatrix, 20);

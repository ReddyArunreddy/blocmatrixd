...
#include <blocmatrix/basics/safe_cast.h>
...
struct BMCEndpointStepInfo
{
    AccountID acc;
};

enum class TrustFlag {freeze, auth, noblocmatrix};
    ...
    switch(f)
    {
        ...
        case TrustFlag::auth:
            if (useHigh)
                return lsfHighAuth;
            return lsfLowAuth;
        case TrustFlag::noblocmatrix:
            if (useHigh)
                return lsfHighNoBlocmatrix;
            return lsfLowNoBlocmatrix;
    }
...
bool
equal(std::unique_ptr<Step> const& s1, BMCEndpointStepInfo const& bmcsi)
{
    if (!s1)
        return false;
    return test::bmcEndpointStepEqual(*s1, bmcsi.acc);
}

bool
equal(std::unique_ptr<Step> const& s1, blocmatrix::Book const& bsi)
{
    if (!s1)
        return false;
...
ape(AccountID const& a)
{
    return STPathElement(
        STPathElement::typeAccount, a, bmcCurrency(), bmcAccount());
};
...
{
    return STPathElement(
        STPathElement::typeIssuer, bmcAccount(), bmcCurrency(), account);
};
...
{
    return STPathElement(
        STPathElement::typeCurrency, bmcAccount(), c, bmcAccount());
};
...
public:
    ...
    valid() const
    {
        return
            ...
            (!hasAny({SB::bmc, SB::existingCur, SB::prevCur}) || has(SB::cur)) &&
            // These will be duplicates
            (count({SB::bmc, SB::existingCur, SB::prevCur}) <= 1) &&
...
struct ExistingElementPool
{
    ...
    std::vector<blocmatrix::Currency> currencies;
        ...
        boost::optional<size_t> const& offererIndex)
    {
        ...
        // create offers to/from bmc to every other ious
        for (auto const& iou : ious)
        {
            auto const owner =
                offererIndex ? accounts[*offererIndex] : iou.account;
            env(offer(owner, iou(1000), BMC(1000)), txflags(tfPassive));
            env(offer(owner, BMC(1000), iou(1000)), txflags(tfPassive));
            env.close();
        }
    }

    std::int64_t
    totalBMC(ReadView const& v, bool incRoot)
    {
        std::uint64_t totalBMC = 0;
    ...
    // Check that the balances for all accounts for all currencies & BMC are the
    // same
    bool
    checkBalances(ReadView const& v1, ReadView const& v2)
    {
        ...
        auto bmcBalance = [](ReadView const& v, blocmatrix::Keylet const& k) {
            auto const sle = v.read(k);
            if (!sle)
                return STAmount{};
            return (*sle)[sfBalance];
        };
        auto lineBalance = [](ReadView const& v, blocmatrix::Keylet const& k) {
            auto const sle = v.read(k);
            if (!sle)
                return STAmount{};
            return (*sle)[sfBalance];
        };
        std::uint64_t totalBMC[2];
        for (auto ai1 = accounts.begin(), aie = accounts.end(); ai1 != aie;
             ++ai1)
        {
            {
                ...
                auto const b1 = bmcBalance(v1, ak);
                auto const b2 = bmcBalance(v2, ak);
                totalBMC[0] += b1.mantissa();
                totalBMC[1] += b2.mantissa();
                if (b1 != b2)
                    diffs.emplace_back(b1, b2, bmcAccount(), *ai1);
            }
            for (auto ai2 = accounts.begin(); ai2 != aie; ++ai2)
            {
                if (ai1 >= ai2)
...
struct PayStrandAllPairs_test : public beast::unit_test::suite
{
    ...
    testAllPairs(FeatureBitset features)
    {
        testcase("All pairs");
        using namespace jtx;
        using BlocmatrixCalc = ::blocmatrix::path::BlocmatrixCalc;
        ...
        auto callback = [&](
            ...
            std::array<PaymentSandbox, 2> sbs{
                {PaymentSandbox{env.current().get(), tapNONE},
                 PaymentSandbox{env.current().get(), tapNONE}}};
            std::array<BlocmatrixCalc::Output, 2> rcOutputs;
            ...
            for (auto i = 0; i < 2; ++i)
            {
                ...
                try
                {
                    rcOutputs[i] = BlocmatrixCalc::blocmatrixCalculate(
                        sbs[i],
                        sendMax,
                        deliver,
                        dst,
            ...
            // check combinations of src and dst currencies (inc bmc)
            // Check the results
            auto const terMatch = [&] {
                ...
                // bmc followed by offer that doesn't specify both currency and
                // issuer (and currency is not bmc, if specifyed)
                if (isBMC(sendMax) &&
                    !(p[0].hasCurrency() && isBMC(p[0].getCurrency())) &&
                ...
                for (size_t i = 0; i < p.size() - 1; ++i)
                {
                    ...
                    if ((tCur & STPathElement::typeCurrency) &&
                        isBMC(p[i].getCurrency()) &&
                        (tNext & STPathElement::typeAccount) &&
                        !isBMC(p[i + 1].getAccountID()))
                    {
                        return true;
                    }
                }
        ...
        for (auto const srcAmtIsBMC : {false, true})
        {
            for (auto const dstAmtIsBMC : {false, true})
            {
                for (auto const hasPrefix : {false, true})
                {
                    ...
                    STAmount const sendMax{
                        srcAmtIsBMC ? bmcIssue() : Issue{eep.getAvailCurrency(),
                                                         eep.getAvailAccount()},
                        -1,  // (-1 == no limit)
                        0};

                    STAmount const deliver{
                        dstAmtIsBMC ? bmcIssue() : Issue{eep.getAvailCurrency(),
                                                         eep.getAvailAccount()},
                        1,
                        0};
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(PayStrandAllPairs, app, blocmatrix, 12);
...
struct PayStrand_test : public beast::unit_test::suite
{
    ...
    testToStrand(FeatureBitset features)
    {
        ...
        using B = blocmatrix::Book;
        using BMCS = BMCEndpointStepInfo;
...

        {
            ...
            {
                STPath const path = STPath({ipe(USD), cpe(bmcCurrency())});
                auto r = toStrand(
                    *env.current(),
                    alice,
                    alice,
...

        {
            ...
            // Path with BMC cross currency bridged payment
            test(
                env,
                EUR,
                USD.issue(),
                STPath({cpe(bmcCurrency())}),
                tesSUCCESS,
                D{alice, gw, usdC},
                B{USD, BMC},
                B{BMC, EUR},
            ...
            // BMC -> BMC transaction can't include a path
            test(env, BMC, boost::none, STPath({ape(carol)}), temBAD_PATH);
            ...
            test(
                ...
                STPath({STPathElement(
                    0, bmcAccount(), bmcCurrency(), bmcAccount())}),
                temBAD_PATH);
...

        {
            ...
            env(offer(bob, BMC(100), USD(100)));
            env(offer(bob, USD(100), EUR(100)), txflags(tfPassive));
            env(offer(bob, EUR(100), USD(100)), txflags(tfPassive));

            // payment path: BMC -> BMC/USD -> USD/EUR -> EUR/USD
            env(pay(alice, carol, USD(100)),
                path(~USD, ~EUR, ~USD),
                sendmax(BMC(200)),
                txflags(tfNoBlocmatrixDirect),
                ter(temBAD_PATH_LOOP));
        }

        {
            Env env(*this, features);
            env.fund(BMC(10000), alice, bob, noblocmatrix(gw));
            env.trust(USD(1000), alice, bob);
            env(pay(gw, alice, USD(100)));
            test(env, USD, boost::none, STPath(), terNO_BLOCMATRIX);
        }
...

        {
            ...
            path.emplace_back(boost::none, bmcCurrency(), boost::none);
            ...
            auto r = toStrand(
                *env.current(),
                ...
                env.app().logs().journal("Flow"));
            BEAST_EXPECT(r.first == tesSUCCESS);
            BEAST_EXPECT(equal(r.second, D{alice, gw, usdC}, B{USD.issue(), bmcIssue()}, BMCS{bob}));
        }
    }
    ...
    testRIPD1373(FeatureBitset features)
    {
        ...
        if (features[fix1373])
        {
            ...
            env(offer(bob, BMC(100), bob["USD"](100)), txflags(tfPassive));
            env(offer(gw, BMC(100), USD(100)), txflags(tfPassive));
...

        {
            ...
            env(offer(bob, BMC(100), USD(100)), txflags(tfPassive));
            env(offer(bob, USD(100), BMC(100)), txflags(tfPassive));

            // payment path: BMC -> BMC/USD -> USD/BMC
            env(pay(alice, carol, BMC(100)),
                path(~USD, ~BMC),
                txflags(tfNoBlocmatrixDirect),
                ter(temBAD_SEND_BMC_PATHS));
        }
    ...
    testLoop(FeatureBitset features)
    {
...

        {
            ...
            // payment path: USD -> USD/BMC -> BMC/USD
            env(pay(alice, carol, USD(100)),
                sendmax(USD(100)),
                path(~BMC, ~USD),
                txflags(tfNoBlocmatrixDirect),
        ...
        }
        {
            ...
            // payment path: BMC->BMC/USD->USD/EUR->USD/CNY
    ...
    testNoAccount(FeatureBitset features)
    {
        ...
        STPathSet pathSet;
        ::blocmatrix::path::BlocmatrixCalc::Input inputs;
        inputs.defaultPathsAllowed = true;
        try
        {
            PaymentSandbox sb{env.current().get(), tapNONE};
            {
                auto const r = ::blocmatrix::path::BlocmatrixCalc::blocmatrixCalculate(
...
BEAST_DEFINE_TESTSUITE(PayStrand, app, blocmatrix);

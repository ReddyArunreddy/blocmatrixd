...
class BCLValidations_test : public beast::unit_test::suite
{
    ...
    void
    testBCLValidatedLedger()
    {
        testcase("BCLValidatedLedger ancestry");

        using Seq = BCLValidatedLedger::Seq;
        using ID = BCLValidatedLedger::ID;


        // This tests BCLValidatedLedger properly implements the type
        ...
        // Empty ledger
        {
            BCLValidatedLedger a{BCLValidatedLedger::MakeGenesis{}};
        ...
        // Full history ledgers
        {
            ...
            BCLValidatedLedger a{ledger, env.journal};
        ...
        // Empty with non-empty
        {
            ...
            {
                BCLValidatedLedger b{ledger, env.journal};
        ...
        {
            BCLValidatedLedger a{history.back(), env.journal};
            for(Seq s = a.seq(); s > 0; s--)
            {
                BCLValidatedLedger b{history[s-1], env.journal};
                if(s >= a.minSeq())
                {
        ...
        // Different chains, same seqs
        {
            ...
            {
                BCLValidatedLedger a{history[s-1], env.journal};
                BCLValidatedLedger b{altHistory[s-1], env.journal};
        ...
        // Different chains, different seqs
        {
            ...
            BCLValidatedLedger a{history[diverge], env.journal};
            for(Seq offset = diverge/2; offset < 3*diverge/2; ++offset)
            {
                BCLValidatedLedger b{altHistory[offset-1], env.journal};
                if(offset <= diverge)
                {
                    BEAST_EXPECT(mismatch(a,b) == b.seq() + 1);
                }
    ...
    void
    testLedgerTrieBCLValidatedLedger()
    {
        testcase("BCLValidatedLedger LedgerTrie");

        // This test exposes an issue with the limited 256
        // ancestor hash design of BCLValidatedLedger.
        ...
        LedgerTrie<BCLValidatedLedger> trie;

        // First, create the single branch trie, with ledgers
        // separated by exactly 256 ledgers
        auto ledg_002 = BCLValidatedLedger{history[1], j};
        auto ledg_258 = BCLValidatedLedger{history[257], j};
        auto ledg_259 = BCLValidatedLedger{history[258], j};
        ...
        //      quirk of BCLValidation and prevents deleting the old support
        //      for ledger 257

        BEAST_EXPECT(
            trie.remove(BCLValidatedLedger{history[257], env.journal}, 1));
        trie.insert(BCLValidatedLedger{history[258], env.journal}, 1);
        trie.getPreferred(1);
        // trie.dump(std::cout);
        // 000000[0,1)(T:0,B:5)
...
BEAST_DEFINE_TESTSUITE(BCLValidations, app, blocmatrix);

...
struct DepositAuth_test : public beast::unit_test::suite
{
    ...
    void testPayIOU()
    {
        // Exercise IOU payments and non-direct BMC payments to an account
        ...
        env (offer (carol, USD(100), BMC(100)));
        env.close();
        ...
        auto failedIouPayments = [this, &env, &alice, &bob, &USD] ()
        {
            ...
            PrettyAmount const bobXrpBalance {env.balance (bob, BMC)};
            PrettyAmount const bobUsdBalance {env.balance (bob, USD)};

            env (pay (alice, bob, USD (50)), ter (tecNO_PERMISSION));
            env.close();

            // Note that even though alice is paying bob in BMC, the payment
            ...
            BEAST_EXPECT (bobXrpBalance == env.balance (bob, BMC));
            BEAST_EXPECT (bobUsdBalance == env.balance (bob, USD));
        };

        //  Test when bob has an BMC balance > base reserve.
        failedIouPayments();

        // Set bob's BMC balance == base reserve.  Also demonstrate that
        // bob can make payments while his lsfDepositAuth flag is set.
        env (pay (bob, alice, USD(25)));
        env.close();

        {
            STAmount const bobPaysBMC {
                env.balance (bob, BMC) - reserve (env, 1)};
            BMCAmount const bobPaysFee {reserve (env, 1) - reserve (env, 0)};
            env (pay (bob, alice, bobPaysBMC), fee (bobPaysFee));
            env.close();
        }

        // Test when bob's BMC balance == base reserve.
        BEAST_EXPECT (env.balance (bob, BMC) == reserve (env, 0));
        BEAST_EXPECT (env.balance (bob, USD) == USD(25));
        failedIouPayments();

        // Test when bob has an BMC balance == 0.
        env (noop (bob), fee (reserve (env, 0)));
        env.close ();

        BEAST_EXPECT (env.balance (bob, BMC) == BMC (0));
        failedIouPayments();

        // Give bob enough BMC for the fee to clear the lsfDepositAuth flag.
    ...
    void testPayBMC()
    {
        // Exercise direct BMC payments to an account that has the
        ...
        env.close();
        BEAST_EXPECT (env.balance (bob, BMC) == BMC (10000) - drops(10));

        // bob has more BMC than the base reserve.  Any BMC payment should fail.
        env (pay (alice, bob, drops(1)), ter (tecNO_PERMISSION));
        env.close();
        BEAST_EXPECT (env.balance (bob, BMC) == BMC (10000) - drops(10));

        // Change bob's BMC balance to exactly the base reserve.
        {
            STAmount const bobPaysBMC {
                env.balance (bob, BMC) - reserve (env, 1)};
            BMCAmount const bobPaysFee {reserve (env, 1) - reserve (env, 0)};
        ...
        // bob has exactly the base reserve.  A small enough direct BMC
        ...
        BEAST_EXPECT (env.balance (bob, BMC) == reserve (env, 0) + drops(1));
        env (pay (alice, bob, drops(1)), ter (tecNO_PERMISSION));
        env.close();

        // Take bob down to a balance of 0 BMC.
        env (noop (bob), fee (reserve (env, 0) + drops(1)));
        env.close ();
        BEAST_EXPECT (env.balance (bob, BMC) == drops(0));
        ...
        env (pay (alice, bob, reserve (env, 0) + drops(0)));
        env.close();
        BEAST_EXPECT (env.balance (bob, BMC) ==
        ...
        // Take bob back down to a zero BMC balance.
        env (noop (bob), fee (env.balance (bob, BMC)));
        env.close();
        BEAST_EXPECT (env.balance (bob, BMC) == drops(0));
        ...
        env (pay (alice, bob, drops(1)));
        env.close();
        BEAST_EXPECT (env.balance (bob, BMC) == drops(1));
    ...
    void testNoBlocmatrix()
    {
        // It its current incarnation the DepositAuth flag does not change
        // any behaviors regarding rippling and the NoBlocmatrix flag.
        // Demonstrate that.
        testcase ("No Blocmatrix");
        ...
        auto testIssuer = [&] (FeatureBitset const& features,
            bool noBlocmatrixPrev,
            bool noBlocmatrixNext,
            bool withDepositAuth)
        {
            ...
            env.fund(BMC(10000), gw1, alice, bob);
            env (trust (gw1, alice["USD"](10), noBlocmatrixPrev ? tfSetNoBlocmatrix : 0));
            env (trust (gw1, bob["USD"](10), noBlocmatrixNext ? tfSetNoBlocmatrix : 0));
            ...
            TER const result = (noBlocmatrixNext && noBlocmatrixPrev)
                ? TER {tecPATH_DRY}
                : TER {tesSUCCESS};
            ...
            bool withDepositAuth)
        {
            ...
            env.fund(BMC(10000), gw1, gw2, alice);
            env (trust (alice, USD1(10), noBlocmatrixPrev ? tfSetNoBlocmatrix : 0));
            env (trust (alice, USD2(10), noBlocmatrixNext ? tfSetNoBlocmatrix : 0));
        ...
        // Test every combo of noBlocmatrixPrev, noBlocmatrixNext, and withDepositAuth
        for (int i = 0; i < 8; ++i)
        {
            auto const noBlocmatrixPrev = i & 0x1;
            auto const noBlocmatrixNext = i & 0x2;
...
struct DepositPreauth_test : public beast::unit_test::suite
{
    void testEnable()
    {
        ...
        Account const becky {"becky"};
        {
            ...
            env.fund (BMC (10000), alice, becky);
            env.close();
    ...
    void testInvalid()
    {
        ...
        }
        {
            ...
            Json::Value tx {deposit::auth (alice, becky)};
            tx[sfAuthorize.jsonName] = to_string (bmcAccount());
        ...
        // carol gets enough BMC to (barely) meet the reserve.
    ...
    void testPayment (FeatureBitset features)
    {
...

        {
            ...
            Env env (*this, features);
            env.fund (BMC (5000), alice, becky, gw);
            env.close();
            ...
            env (offer (alice, BMC (100), USD (100), tfPassive),
            ...
            env (pay (becky, becky, USD (10)),
                path (~USD), sendmax (BMC (10)), ter (expect));
            env.close();
        }
        ...
        if (supportsPreauth)
        {
            ...
            Env env (*this, features);
            env.fund (BMC (5000), alice, becky, carol, gw);
            env.close();
            ...
            // Make BMC and IOU payments from alice to becky.  Should be fine.
            env (pay (alice, becky, BMC (100)));
            ...
            env (pay (alice, becky, BMC (100)), ter (tecNO_PERMISSION));
            ...
            env (pay (becky, alice, BMC (100)), ter (tecNO_PERMISSION));
...
BEAST_DEFINE_TESTSUITE(DepositAuth,app,blocmatrix);
BEAST_DEFINE_TESTSUITE(DepositPreauth,app,blocmatrix);

...
#include <blocmatrix/protocol/PayChan.h>
...
struct PayChan_test : public beast::unit_test::suite
{
    ...
    {
        Serializer msg;
        serializePayChanAuthorization (msg, channel, authAmt.bmc ());
    ...
    testSimple ()
    {
        ...
        env (create (alice, bob, BMC (1000), settleDelay, pk));
        auto const chan = channel (*env.current (), alice, bob);
        BEAST_EXPECT (channelBalance (*env.current (), chan) == BMC (0));
        BEAST_EXPECT (channelAmount (*env.current (), chan) == BMC (1000));

        {
            auto const preAlice = env.balance (alice);
            env (fund (alice, chan, BMC (1000)));
            auto const feeDrops = env.current ()->fees ().base;
            BEAST_EXPECT (env.balance (alice) == preAlice - BMC (1000) - feeDrops);
        }

        auto chanBal = channelBalance (*env.current (), chan);
        auto chanAmt = channelAmount (*env.current (), chan);
        BEAST_EXPECT (chanBal == BMC (0));
        BEAST_EXPECT (chanAmt == BMC (2000));

        {
            // bad amounts (non-bmc, negative amounts)
        ...
        // invalid account
        env (create (alice, "noAccount", BMC (1000), settleDelay, pk),
            ter (tecNO_DST));
        // can't create channel to the same account
        env (create (alice, alice, BMC (1000), settleDelay, pk),
             ter (temDST_IS_SRC));
        // invalid channel
        env (fund (alice, channel (*env.current (), alice, "noAccount"), BMC (1000)),
            ter (tecNO_ENTRY));
        // not enough funds
        env (create (alice, bob, BMC (10000), settleDelay, pk),
            ter (tecUNFUNDED));

        {
            // No signature claim with bad amounts (negative and non-bmc)
            auto const iou = USDA (100).value ();
            auto const negBMC = BMC (-100).value ();
            auto const posBMC = BMC (100).value ();
            env (claim (alice, chan, iou, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posBMC, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, iou, posBMC), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negBMC, negBMC), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posBMC, negBMC), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negBMC, posBMC), ter (temBAD_AMOUNT));
        }
        {
            // No signature claim more than authorized
            auto const delta = BMC (500);
            auto const reqBal = chanBal + delta;
            auto const authAmt = reqBal + BMC (-100);
        ...
        }
        {
            ...
            auto const preBob = env.balance (bob);
            STAmount const authAmt = chanBal + BMC (500);
        ...
        env (fund (bob, chan, BMC (1000)), ter (tecNO_PERMISSION));
        BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
        BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);

        {
            // Wrong signing key
            auto const sig =
                signClaimAuth (bob.pk (), bob.sk (), chan, BMC (1500));
            env (claim (bob, chan, BMC (1500).value (), BMC (1500).value (),
                     Slice (sig), bob.pk ()),
                ter (temBAD_SIGNER));
            BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
            BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);
    ...
    testCancelAfter ()
    {
        ...
        auto const carol = Account ("carol");
        {
            ...
            auto const channelFunds = BMC (1000);
            env (create (
    ...
    testExpiration ()
    {
        ...
        env (fund (alice, chan, BMC (1),
            ...
            ter (temBAD_EXPIRATION));
        BEAST_EXPECT (!channelExpiration (*env.current (), chan));
        env (fund (alice, chan, BMC (1), NetClock::time_point{minExpiration}));
        env.close (minExpiration);
        // Try to extend the expiration after the expiration has already passed
        env (fund (
            alice, chan, BMC (1), NetClock::time_point{minExpiration + 1000s}));
        BEAST_EXPECT (!channelExists (*env.current (), chan));
    }
    ...
    testDisallowBMC ()
    {
        ...
        auto const bob = Account ("bob");
        {
            // Create a channel where dst disallows BMC
            Env env (*this, supported_amendments() - featureDepositAuth);
            env.fund (BMC (10000), alice, bob);
            env (fset (bob, asfDisallowBMC));
            env (create (alice, bob, BMC (1000), 3600s, alice.pk()),
                ter (tecNO_TARGET));
            auto const chan = channel (*env.current (), alice, bob);
            BEAST_EXPECT (!channelExists (*env.current (), chan));
        }
        {
            // Create a channel where dst disallows BMC.  Ignore that flag,
        ...
        {
            // Claim to a channel where dst disallows BMC
            // (channel is created before disallow bmc is set)
            Env env (*this, supported_amendments() - featureDepositAuth);
            env.fund (BMC (10000), alice, bob);
            env (create (alice, bob, BMC (1000), 3600s, alice.pk()));
            auto const chan = channel (*env.current (), alice, bob);
            ...
            env (fset (bob, asfDisallowBMC));
            auto const reqBal = BMC (500).value();
            env (claim (alice, chan, reqBal, reqBal), ter(tecNO_TARGET));
        }
        {
            // Claim to a channel where dst disallows BMC (channel is
            // created before disallow bmc is set).  Ignore that flag
    ...
    testDepositAuth ()
    {
        ...
        auto USDA = alice["USD"];
        {
            ...
            env (claim (alice, chan,
                BMC (500).value(), BMC (500).value()), ter (tecNO_PERMISSION));
            env.close();
            ...
            auto const preBob = env.balance (bob);
            {
                auto const delta = BMC (500).value();
            ...
            }
            {
                ...
                auto const delta = BMC (600).value();
                auto const sig = signClaimAuth (pk, alice.sk (), chan, delta);
            ...
            }
            {
                ...
                auto const delta = BMC (800).value();
                ...
                env.close();
                BEAST_EXPECT (
                    env.balance (bob) == preBob + BMC (800) - (5 * baseFee));
            }
        }
    }
    ...
    testMalformedPK ()
    {
        ...
        auto jv = create (alice, bob, BMC (1000), settleDelay, pk);
...
BEAST_DEFINE_TESTSUITE (PayChan, app, blocmatrix);
}  // test
}  // blocmatrix

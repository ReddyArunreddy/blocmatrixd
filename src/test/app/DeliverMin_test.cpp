...
public:
    ...
    test_convert_all_of_an_asset(FeatureBitset features)
    {
...

        {
            ...
            env(pay("alice", "bob", USD(10)), delivermin(BMC(5)),
                txflags(tfPartialPayment),                          ter(temBAD_AMOUNT));
            ...
            env(offer("carol", BMC(5), USD(5)));
            env(pay("alice", "bob", USD(10)), paths(BMC),
                delivermin(USD(7)), txflags(tfPartialPayment),
                sendmax(BMC(5)),                                   ter(tecPATH_PARTIAL));
            env.require(balance("alice", BMC(9999.99999)));
...

        {
            ...
            env(pay("alice", "alice", USD(10000)), paths(BMC),
...

        {
            ...
            env(offer("bob", BMC(100), USD(100)));
            env(offer("bob", BMC(1000), USD(100)));
            env(offer("bob", BMC(10000), USD(100)));
            env(pay("alice", "carol", USD(10000)), paths(BMC),
                delivermin(USD(200)), txflags(tfPartialPayment),
                sendmax(BMC(1000)),                                 ter(tecPATH_PARTIAL));
            env(pay("alice", "carol", USD(10000)), paths(BMC),
                delivermin(USD(200)), txflags(tfPartialPayment),
                sendmax(BMC(1100)));
...
BEAST_DEFINE_TESTSUITE(DeliverMin,app,blocmatrix);

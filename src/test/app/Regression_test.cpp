...
struct Regression_test : public beast::unit_test::suite
{
    ...
    void testOffer1()
    {
        ...
        env(offer("alice", USD(10), BMC(10)), require(owners("alice", 1)));
        env(offer("alice", USD(20), BMC(10)), json(R"raw(
                { "OfferSequence" : 2 }
    ...
    {
        testcase("Account balance < fee destroys correct amount of BMC");
        using namespace jtx;
        Env env(*this);
        env.memoize("alice");
        ...
        auto const aliceBMC = 400;
        auto const aliceAmount = BMC(aliceBMC);
            ...
            env.app().timeKeeper().closeTime());
        {
            ...
            auto const result = blocmatrix::apply(env.app(),
        ...
        }
        {
            ...
            BEAST_EXPECT(balance == BMC(0));
        }
        expectedDrops -= aliceBMC * dropsPerBMC<int>::value;
        BEAST_EXPECT(next->info().drops == expectedDrops);
    }
    ...
    void testJsonInvalid()
    {
        ...
        std::string const request = R"json({"command":"path_find","id":19,"subcommand":"create","source_account":"bHr9CJAWyB4bj91VRWn96DkukG4rwdtyTh","destination_account":"bHr9CJAWyB4bj91VRWn96DkukG4rwdtyTh","destination_amount":"1000000","source_currencies":[{"currency":"0000000000000000000000000000000000000000"},{"currency":"0000000000000000000000005553440000000000"},{"currency":"0000000000000000000000004254430000000000"},{"issuer":"bHr9CJAWyB4bj91VRWn96DkukG4rwdtyTh","currency":"0000000000000000000000004254430000000000"},{"issuer":"rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh","currency":"0000000000000000000000004254430000000000"},{"issuer":"rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh","currency":"0000000000000000000000004555520000000000"},{"currency":"0000000000000000000000004554480000000000"},{"currency":"0000000000000000000000004A50590000000000"},{"issuer":"bHr9CJAWyB4bj91VRWn96DkukG4rwdtyTh","currency":"000000000000000000000000434E590000000000"},{"currency":"0000000000000000000000004742490000000000"},{"issuer":"bHr9CJAWyB4bj91VRWn96DkukG4rwdtyTh","currency":"0000000000000000000000004341440000000000"}]})json";
...
BEAST_DEFINE_TESTSUITE(Regression,app,blocmatrix);

...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(PlumpBook,tx,blocmatrix,5);
...
BEAST_DEFINE_TESTSUITE(ThinBook, tx, blocmatrix);
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(OversizeMeta,tx,blocmatrix,3);
...
public:
    ...
    oversize(std::size_t n)
    {
        ...
        createOffers(env, USD, n);
        env(pay("alice", gw, USD(billion)));
        env(offer("alice", USD(1), BMC(1)), ter(std::ignore));
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(FindOversizeCross,tx,blocmatrix,50);

...
class TrustAndBalance_test : public beast::unit_test::suite
{
    ...
    testPayNonexistent (FeatureBitset features)
    {
        ...
        env (pay (env.master, "alice", BMC(1)), ter(tecNO_DST_INSUF_BMC));
        env.close();
    }
    ...
    void
    testDirectBlocmatrix (FeatureBitset features)
    {
        testcase ("Direct Payment, Blocmatrix");
        using namespace test::jtx;
    ...
    testIndirectMultiPath (bool with_rate, FeatureBitset features)
    {
        ...
        env.fund (BMC(10000), gw, amazon, alice, bob, carol);
        env.close();
    ...
    testInvoiceID (FeatureBitset features)
    {
        ...
        Json::Value jv;
        auto tx = env.jt (
            pay (env.master, alice, BMC(10000)),
...
BEAST_DEFINE_TESTSUITE_PRIO (TrustAndBalance, app, blocmatrix, 1);

...
#include <blocmatrix/server/Port.h>
...
{
    static boost::format configContentsTemplate (R"blocmatrixConfig(
[server]
port_rpc
port_peer
port_wss_admin
...
# This is primary persistent datastore for blocmatrixd.  This includes transaction
...
[node_db]
type=memory
path=/Users/dummy/blocmatrix/config/db/rocksdb
open_files=2000
filter_bits=12
cache_mb=256
file_size_mb=8
...
[debug_logfile]
/Users/dummy/blocmatrix/config/log/debug.log
...
/**
   Write a blocmatrixd config file and remove when done.
 */
class BlocmatrixdCfgGuard : public blocmatrix::test::detail::FileDirGuard
{
private:
    path dataDir_;
...
public:
    BlocmatrixdCfgGuard (beast::unit_test::suite& test,
    ...
    ~BlocmatrixdCfgGuard ()
    {
        ...
        catch (std::exception& e)
        {
            ...
            test_.log << "Error in ~BlocmatrixdCfgGuard: "
                      << e.what () << std::endl;
        };
    }
};
...
{
    std::string configContents (R"blocmatrixConfig(
[validators]
...
public:
    void testLegacy ()
    {
        ...
        std::string toLoad(R"blocmatrixConfig(
[server]
port_rpc
port_peer
port_wss_admin
...
0
    ...
    void testDbPath ()
    {
        ...
        }
        {
            ...
            auto const cwd = current_path ();
            blocmatrix::test::detail::DirGuard const g0(*this, "test_db");
            path const dataDirRel ("test_data_dir");
            path const dataDirAbs(cwd / g0.subdir () / dataDirRel);
            detail::BlocmatrixdCfgGuard const g (*this, g0.subdir(),
                dataDirAbs, "", false);
        ...
        }
        {
            ...
            detail::BlocmatrixdCfgGuard const g (*this, "test_db", dbPath, "");
            auto const& c (g.config ());
        ...
        }
        {
            ...
            detail::BlocmatrixdCfgGuard const g (*this, "test_db", "", "");
            auto const& c (g.config ());
    ...
    void testValidatorKeys ()
    {
        ...
        {
            Config c;
            static boost::format configTemplate (R"blocmatrixConfig(
[validation_seed]
%1%
...
trustthesevalidators.gov
        ...
        }
        {
            ...
            detail::BlocmatrixdCfgGuard const rcg (
        ...
        }
        {
            ...
            detail::ValidatorsTxtGuard const vtg (
                *this, "test_cfg", "validators.txt");
            detail::BlocmatrixdCfgGuard const rcg (*this, vtg.subdir (),
                "", "", false);
            BEAST_EXPECT(vtg.validatorsFileExists ());
            BEAST_EXPECT(rcg.configFileExists ());
            auto const& c (rcg.config ());
...

        {
            ...
            boost::format cc (R"blocmatrixConfig(
[validators_file]
%1%
...
021A99A537FDEBC34E4FCA03B39BEADD04299BB19E85097EC92B15A3518801E566
        ...
        }
        {
            ...
            // [validator_list_keys] are missing from blocmatrixd cfg and
            // validators file
            Config c;
            boost::format cc ("[validators_file]\n%1%\n");
            std::string error;
    ...
    {
        detail::BlocmatrixdCfgGuard const cfg(*this, "testSetup",
            explicitPath ? "test_db" : "", "");
        /* BlocmatrixdCfgGuard has a Config object that gets loaded on
    ...
    void testPort ()
    {
        detail::BlocmatrixdCfgGuard const cfg(*this, "testPort", "", "");
...
BEAST_DEFINE_TESTSUITE (Config, core, blocmatrix);

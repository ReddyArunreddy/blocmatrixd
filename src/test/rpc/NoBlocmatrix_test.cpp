...
class NoBlocmatrix_test : public beast::unit_test::suite
{
public:
    void
    testSetAndClear()
    {
        testcase("Set and clear noblocmatrix");
        ...
        {
            // Create a trust line with no-blocmatrix flag setting
            env( trust(gw, USD(100), alice, SetOrClear ? tfSetNoBlocmatrix
                                                       : tfClearNoBlocmatrix));
            env.close();

            // Check no-blocmatrix flag on sender 'gateway'
            Json::Value lines {env.rpc(
                "json", "account_lines", to_string(account_gw))};
            auto const& gline0 = lines[jss::result][jss::lines][0u];
            BEAST_EXPECT(gline0[jss::no_blocmatrix].asBool() == SetOrClear);

            // Check no-blocmatrix peer flag on destination 'alice'
            lines = env.rpc("json", "account_lines", to_string(account_alice));
            auto const& aline0 = lines[jss::result][jss::lines][0u];
            BEAST_EXPECT(aline0[jss::no_blocmatrix_peer].asBool() == SetOrClear);
        }
    }

    void testNegativeBalance(FeatureBitset features)
    {
        testcase("Set noblocmatrix on a line with negative balance");
            ...
            {features - fix1578, features | fix1578})
        {
            ...
            // bob should be able to clear the noBlocmatrix flag.
            ...
            env(trust(
                alice, bob["USD"](100), bob,   tfSetNoBlocmatrix), ter(terNeg));
            env(trust(
                bob, carol["USD"](100), carol, tfSetNoBlocmatrix), ter(terNeg));
            env.close();
            ...
            auto const resp = env.rpc(
                "json", "blocmatrix_path_find", to_string(params));
            ...
            };
            {
                ...
                BEAST_EXPECT(!aliceLines[0u].isMember(jss::no_blocmatrix));

                auto const bobLines = getAccountLines (bob);
                BEAST_EXPECT(bobLines.size() == 2);
                BEAST_EXPECT(!bobLines[0u].isMember(jss::no_blocmatrix));
                BEAST_EXPECT(!bobLines[1u].isMember(jss::no_blocmatrix));
            }

            // Now carol sends the 50 USD back to alice.  Then alice and
            // bob can set the noBlocmatrix flag.
            env(pay(carol, alice, alice["USD"](50)), path(bob));
            env.close();

            env(trust(alice, bob["USD"](100), bob,   tfSetNoBlocmatrix));
            env(trust(bob, carol["USD"](100), carol, tfSetNoBlocmatrix));
            env.close();
            {
                auto const aliceLines = getAccountLines (alice);
                BEAST_EXPECT(aliceLines.size() == 1);
                ...
                auto const bobLines = getAccountLines (bob);
                BEAST_EXPECT(bobLines.size() == 2);
                BEAST_EXPECT(bobLines[0u].isMember(jss::no_blocmatrix_peer));
                BEAST_EXPECT(bobLines[1u].isMember(jss::no_blocmatrix));
            }
        }
    }
    ...
    {
        testcase("pairwise NoBlocmatrix");
        ...
        env(trust(bob, alice["USD"](100), alice, tfSetNoBlocmatrix));
        env(trust(bob, carol["USD"](100), carol, tfSetNoBlocmatrix));
        env.close();
        ...
        Json::Value const resp {
            env.rpc("json", "blocmatrix_path_find", to_string(params))};
        BEAST_EXPECT(resp[jss::result][jss::alternatives].size() == 0);

        env(pay(alice, carol, bob["USD"](50)), ter(tecPATH_DRY));
    }

    void testDefaultBlocmatrix(FeatureBitset features)
    {
        testcase("Set default blocmatrix on an account and check new trustlines");
        ...
        env.fund(BMC(10000), gw, noblocmatrix(alice, bob));

        env(fset(bob, asfDefaultBlocmatrix));
...

        {
            ...
            BEAST_EXPECT(line0[jss::no_blocmatrix_peer].asBool() == true);
        }
        {
            Json::Value params;
            ...
            auto lines = env.rpc("json", "account_lines", to_string(params));
            auto const& line0 = lines[jss::result][jss::lines][0u];
            BEAST_EXPECT(line0[jss::no_blocmatrix].asBool() == true);
        }
        {
            Json::Value params;
            params[jss::account] = gw.human();
            ...
            auto lines = env.rpc("json", "account_lines", to_string(params));
            auto const& line0 = lines[jss::result][jss::lines][0u];
            BEAST_EXPECT(line0[jss::no_blocmatrix].asBool() == false);
        }
        {
            Json::Value params;
            ...
            auto lines = env.rpc("json", "account_lines", to_string(params));
            auto const& line0 = lines[jss::result][jss::lines][0u];
            BEAST_EXPECT(line0[jss::no_blocmatrix_peer].asBool() == false);
        }
    }
    ...
    void run () override
    {
        ...
        auto withFeatsTests = [this](FeatureBitset features) {
            ...
            testDefaultBlocmatrix(features);
        };
        using namespace jtx;
...
BEAST_DEFINE_TESTSUITE(NoBlocmatrix,app,blocmatrix);

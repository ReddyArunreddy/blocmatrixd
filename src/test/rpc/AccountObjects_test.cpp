...
public:
    void testErrors()
    {
       ...
       // test errors on marker
        {
            Account const gw{ "G" };
            env.fund(BMC(1000), gw);
            auto const USD = gw["USD"];
            env.trust(USD(1000), bob);
            env(pay(gw, bob, BMC(1)));
            env(offer(bob, BMC(100), bob["USD"](1)), txflags(tfPassive));
    ...
    void testUnsteppedThenStepped()
    {
        ...
        env(offer(bob, BMC(100), bob["USD"](1)),txflags(tfPassive));
        env(offer(bob, BMC(100), USD1(1)), txflags(tfPassive));
    ...
    void testObjectTypes()
    {
        ...
        }
        {
            ...
            jvEscrow[jss::Amount] =
                BMC(100).value().getJson(JsonOptions::none);
        ...
        }
        {
            ...
            jvPayChan[jss::Amount] =
                BMC (300).value().getJson (JsonOptions::none);
...
BEAST_DEFINE_TESTSUITE(AccountObjects,app,blocmatrix);

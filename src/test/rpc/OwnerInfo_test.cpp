...
class OwnerInfo_test : public beast::unit_test::suite
{
    ...
    testBasic ()
    {
        ...
        env(offer(alice, USD(1), BMC(1000)));
        env.close();

        env(pay(gw, alice, USD(50)));
        env(pay(gw, alice, CNY(50)));
        env(offer(alice, CNY(2), BMC(1000)));
        ...
        if (! BEAST_EXPECT (result[jss::accepted].isMember(jss::blocmatrix_lines)))
            return;
        auto lines = result[jss::accepted][jss::blocmatrix_lines];
        ...
        BEAST_EXPECT (
            ...
            BMC(1000).value().getJson(JsonOptions::none));
        BEAST_EXPECT (
        ...
        if (! BEAST_EXPECT (result[jss::current].isMember(jss::blocmatrix_lines)))
            return;
        lines = result[jss::current][jss::blocmatrix_lines];
        if (! BEAST_EXPECT (lines.isArray() && lines.size() == 2))
            return;
...
BEAST_DEFINE_TESTSUITE(OwnerInfo,app,blocmatrix);

...
#include <blocmatrix/beast/utility/temp_dir.h>
#include <blocmatrix/resource/ResourceManager.h>
#include <blocmatrix/resource/impl/Entry.h>
#include <blocmatrix/resource/impl/Tuning.h>
#include <blocmatrix/rpc/impl/Tuning.h>

namespace blocmatrix {

class NoBlocmatrixCheck_test : public beast::unit_test::suite
{
    void
    testBadInput ()
    {
        testcase ("Bad input to noblocmatrix_check");
        ...
        { // missing account field
            auto const result = env.rpc ("json", "noblocmatrix_check", "{}")
                [jss::result];
    ...
    {
        testcase << "Request noblocmatrix_check for " <<
        ...
        {
            env (fset (alice, asfDefaultBlocmatrix));
            env (trust (alice, gw["USD"](100)));
        }
        else
        {
            env (fclear (alice, asfDefaultBlocmatrix));
            env (trust (alice, gw["USD"](100), gw, tfSetNoBlocmatrix));
        }
        env.close ();
        ...
        params[jss::ledger] = "current";
        auto result = env.rpc ("json", "noblocmatrix_check",
            boost::lexical_cast<std::string>(params)) [jss::result];
        ...
        result = env.rpc ("json", "noblocmatrix_check",
            boost::lexical_cast<std::string>(params)) [jss::result];
...
class NoBlocmatrixCheckLimits_test : public beast::unit_test::suite
{
    void
    testLimits(bool admin)
    {
        ...
        auto checkBalance = [&env]()
        {
            ...
            // cases.
            using namespace blocmatrix::Resource;
        ...
        for (auto i = 0; i < blocmatrix::RPC::Tuning::noBlocmatrixCheck.rmax + 5; ++i)
        {
            if (! admin)
                checkBalance();
            ...
            env.memoize(gw);
            env (pay (env.master, gw, BMC(1000)),
                seq (autofill),
                fee (txq.getMetrics(*env.current()).openLedgerFeeLevel + 1),
                sig (autofill));
            env (fset (gw, asfDefaultBlocmatrix),
                seq (autofill),
                fee (txq.getMetrics(*env.current()).openLedgerFeeLevel + 1),
                sig (autofill));
...
BEAST_DEFINE_TESTSUITE(NoBlocmatrixCheck, app, blocmatrix);
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(NoBlocmatrixCheckLimits, app, blocmatrix, 1);

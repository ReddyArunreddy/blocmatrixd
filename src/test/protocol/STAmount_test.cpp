...
#include <blocmatrix/protocol/STAmount.h>
...
public:
    ...
    {
        {
            ...
            Issue const bmc (bmcIssue ());

            // fractional BMC (i.e. drops)
            testSetValue ("1", bmc);
            testSetValue ("22", bmc);
            testSetValue ("333", bmc);
            testSetValue ("4444", bmc);
            testSetValue ("55555", bmc);
            testSetValue ("666666", bmc);

            // 1 BMC up to 100 billion, in powers of 10 (in drops)
            testSetValue ("1000000", bmc);
            testSetValue ("10000000", bmc);
            testSetValue ("100000000", bmc);
            testSetValue ("1000000000", bmc);
            testSetValue ("10000000000", bmc);
            testSetValue ("100000000000", bmc);
            testSetValue ("1000000000000", bmc);
            testSetValue ("10000000000000", bmc);
            testSetValue ("100000000000000", bmc);
            testSetValue ("1000000000000000", bmc);
            testSetValue ("10000000000000000", bmc);
            testSetValue ("100000000000000000", bmc);

            // Invalid native values:
            testSetValue ("1.1", bmc, false);
            testSetValue ("100000000000000001", bmc, false);
            testSetValue ("1000000000000000000", bmc, false);
        }
    ...
    void testNativeCurrency ()
    {
        ...
        unexpected (to_string (Currency ()) != "BMC", "cHC(BMC)");
        Currency c;
    ...
    void testCustomCurrency ()
    {
        ...
        unexpected (multiply (STAmount (noIssue(), 20), STAmount (3), bmcIssue ()).getText () != "60",
            "STAmount multiply fail 2");
        unexpected (multiply (STAmount (20), STAmount (3), noIssue()).getText () != "60",
            "STAmount multiply fail 3");
        unexpected (multiply (STAmount (20), STAmount (3), bmcIssue ()).getText () != "60",
        ...
        unexpected (divide (STAmount (noIssue(), 60), STAmount (3), bmcIssue ()).getText () != "20",
            "STAmount divide fail");

        unexpected (divide (STAmount (noIssue(), 60), STAmount (noIssue(), 3), noIssue()).getText () != "20",
            "STAmount divide fail");

        unexpected (divide (STAmount (noIssue(), 60), STAmount (noIssue(), 3), bmcIssue ()).getText () != "20",
            "STAmount divide fail");
    ...
    void testUnderflow ()
    {
        ...
        bigDsmall = divide (smallValue, bigValue, bmcIssue ());

        BEAST_EXPECT(bigDsmall == beast::zero);

        bigDsmall = divide (smallValue, bigNative, bmcIssue ());
    ...
    void testRounding ()
    {
        ...
        STAmount dripTest1 = mulRound (twoThird2, two, bmcIssue (), false);
        STAmount dripTest2 = multiply (twoThird2, two, bmcIssue ());
        STAmount dripTest3 = mulRound (twoThird2, two, bmcIssue (), true);
        log << dripTest1;
        log << dripTest2;
        log << dripTest3;
#endif
    ...
    testConvertBMC ()
    {
        testcase ("STAmount to BMCAmount conversions");
        ...
        {
            auto const t = amountFromString (bmc, std::to_string (drops));
            auto const s = t.bmc ();
            BEAST_EXPECT(s.drops() == drops);
            BEAST_EXPECT(t == STAmount (BMCAmount (drops)));
            BEAST_EXPECT(s == BMCAmount (drops));
        }

        try
        {
            auto const t = amountFromString (usd, "136500");
            fail (to_string (t.bmc ()));
        }
    ...
    testConvertIOU ()
    {
        ...
        try
        {
            auto const t = amountFromString (bmc, "136500");
...
BEAST_DEFINE_TESTSUITE(STAmount,blocmatrix_data,blocmatrix);

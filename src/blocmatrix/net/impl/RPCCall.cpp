...
#include <blocmatrix/json/Object.h>
#include <blocmatrix/net/HTTPClient.h>
...
private:
    ...
    static Json::Value jvParseCurrencyIssuer (std::string const& strCurrencyIssuer)
    {
        ...
        if (boost::regex_match (strCurrencyIssuer, smMatch, reCurIss))
        {
            ...
            if (strIssuer.length ())
            {
                // Could confirm issuer is a valid Blocmatrix address.
...
private:
    ...
    bool isValidJson2(Json::Value const& jv)
    {
        ...
        if (jv.isObject())
        {
            if (jv.isMember(jss::jsonrpc) && jv[jss::jsonrpc] == "2.0" &&
                jv.isMember(jss::blocmatribmcc) && jv[jss::blocmatribmcc] == "2.0" &&
    ...
    Json::Value parseJson2(Json::Value const& jvParams)
    {
        ...
        if (valid_parse && isValidJson2(jv))
        {
            if (jv.isObject())
            {
                ...
                jv1[jss::blocmatribmcc] = jv[jss::blocmatribmcc];
            ...
            for (Json::UInt j = 0; j < jv.size(); ++j)
            {
                ...
                jv1[j][jss::blocmatribmcc] = jv[j][jss::blocmatribmcc];
        ...
        if (jv.isMember(jss::blocmatribmcc))
            jv_error[jss::blocmatribmcc] = jv[jss::blocmatribmcc];
    ...
    Json::Value parseBlocmatrixPathFind (Json::Value const& jvParams)
    {
        Json::Reader    reader;
        Json::Value     jvRequest{Json::objectValue};
...
public:
    ...
    //      {   "profile",              &RPCParser::parseProfile,               1,  9   },
            ...
            {   "blocmatrix_path_find",     &RPCParser::parseBlocmatrixPathFind,        1,  2   },
...
cmdLineToJSONRPC (std::vector<std::string> const& args, beast::Journal j)
{
    ...
    if (paramsObj.isMember(jss::blocmatribmcc))
        jv[jss::blocmatribmcc] = paramsObj[jss::blocmatribmcc];

...
#ifndef BLOCMATRIX_PROTOCOL_STPATHSET_H_INCLUDED
#define BLOCMATRIX_PROTOCOL_STPATHSET_H_INCLUDED
...
public:
    ...
    STPathElement (
        ...
        , mIssuerID (issuer), is_offer_ (isBMC(mAccountID))
    {
        if (!is_offer_)
            mType |= typeAccount;

        if (forceCurrency || !isBMC(currency))
            mType |= typeCurrency;

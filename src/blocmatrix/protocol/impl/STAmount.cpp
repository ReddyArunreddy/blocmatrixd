...
#include <blocmatrix/protocol/SystemParameters.h>
    ...
    : STBase(name)
{
    ...
    if (isBMC (issue.currency))
        Throw<std::runtime_error> ("invalid native currency");

    issue.account = sit.get160();

    if (isBMC (issue.account))
...
STAmount::STAmount (BMCAmount const& amount)
    : mOffset (0)
    , mIsNative (true)
    , mIsNegative (amount < beast::zero)
{
...
//
//------------------------------------------------------------------------------
BMCAmount STAmount::bmc () const
{
    if (!mIsNative)
        Throw<std::logic_error> ("Cannot return non-native STAmount as BMCAmount");
...
//
// On the wire:
// - high bit is 0 for BMC, 1 for issued currency
...
//   (but BMC only needs 57 bits for the max value of 10^17 drops)
//
...
amountFromString (Issue const& issue, std::string const& amount)
{
    ...
    // Can't specify BMC using fractional representation
    if (isBMC(issue) && match[3].matched)
        Throw<std::runtime_error> ("BMC must be specified in integral drops.");
...
amountFromJson (SField const& name, Json::Value const& v)
{
    ...
    if (v.isNull())
    {
        Throw<std::runtime_error> ("BMC may not be specified with a null Json value");
    }
    else if (v.isObject())
    {
        value       = v[jss::value];
    ...
    {
        if (v.isObjectOrNull ())
            Throw<std::runtime_error> ("BMC may not be specified as an object");
        issue = bmcIssue ();
    }
    else
    {
...
multiply (STAmount const& v1, STAmount const& v2, Issue const& issue)
{
    ...
    if (v1.native() && v2.native() && isBMC (issue))
    {
    ...
    bool roundUp)
{
    ...
    bool const bmc = isBMC (issue);

    if (v1.native() && v2.native() && bmc)
    {
    ...
    if (resultNegative != roundUp)
        canonicalizeRound (bmc, amount, offset);
    ...
    {
        if (bmc && *stAmountCalcSwitchover2)
        {
    ...
    Issue const& issue, bool roundUp)
{
    ...
    if (resultNegative != roundUp)
        canonicalizeRound (isBMC (issue), amount, offset);

    STAmount result (issue, amount, offset, resultNegative);
    // Control when bugfixes that require switchover dates are enabled
    if (roundUp && !resultNegative && !result && *stAmountCalcSwitchover)
    {
        if (isBMC(issue) && *stAmountCalcSwitchover2)
        {
            // return the smallest value above zero
            amount = 1;
            offset = 0;

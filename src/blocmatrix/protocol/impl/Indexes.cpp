...
{
    return getBlocmatrixStateIndex (a, issue.account, issue.currency);
}
...
{
    return { ltBLOCMATRIX_STATE,
        getBlocmatrixStateIndex(id0, id1, currency) };
}
...
{
    return { ltBLOCMATRIX_STATE,
        getBlocmatrixStateIndex(id, issue) };
}
...
payChan (AccountID const& source, AccountID const& dst, std::uint32_t seq)
{
    sha512_half_hasher h;
    using beast::hash_append;
    hash_append(h, std::uint16_t(spaceBMCUChannel));
    hash_append(h, source);
    hash_append(h, dst);
    hash_append(h, seq);

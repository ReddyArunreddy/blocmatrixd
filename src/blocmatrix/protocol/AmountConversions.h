...
#ifndef BLOCMATRIX_PROTOCOL_AMOUNTCONVERSION_H_INCLUDED
#define BLOCMATRIX_PROTOCOL_AMOUNTCONVERSION_H_INCLUDED
...
inline
STAmount
toSTAmount (BMCAmount const& bmc)
{
    bool const isNeg = bmc.signum() < 0;
    std::uint64_t const umant = isNeg ? - bmc.drops () : bmc.drops ();
    return STAmount (umant, isNeg);
}

inline
STAmount
toSTAmount (BMCAmount const& bmc, Issue const& iss)
{
    assert (isBMC(iss.account) && isBMC(iss.currency));
    return toSTAmount (bmc);
}
...
inline
BMCAmount
toAmount<BMCAmount> (STAmount const& amt)
{
    assert (amt.mantissa () < std::numeric_limits<std::int64_t>::max ());
    bool const isNeg = amt.negative ();
    std::int64_t const sMant =

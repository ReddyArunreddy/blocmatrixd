...
#include <blocmatrix/ledger/detail/ApplyStateTable.h>
...
struct CashSummary
{
    ...
    std::vector<std::pair<
        AccountID, BMCAmount>> bmcChanges;
    ...
    void sort()
    {
        std::sort (bmcChanges.begin(), bmcChanges.end());
    ...
    std::shared_ptr<SLE const> const& after)
{
    if (isDelete)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            result.bmcChanges.push_back (
                std::make_pair (prev[sfAccount], BMCAmount {0}));
            return true;
    ...
    else
    {
        ...
        case ltACCOUNT_ROOT:
        {
            auto const curXrp = cur[sfBalance].bmc();
            if (!before || (*before)[sfBalance].bmc() != curXrp)
                result.bmcChanges.push_back (
...
{
private:
    // Note differences in destroyed BMC between two ApplyStateTables.
    struct DropsGone
    {
        BMCAmount lhs;
        BMCAmount rhs;
...
countKeys (detail::CashSummary const& lhs, detail::CashSummary const& rhs)
{
    ...
    addIn (countKeys(lhs.bmcChanges,     rhs.bmcChanges));
...
int CashDiff::Impl::bmcRoundToZero () const
{
    // The case has one OfferChange that is present on both lhs_ and rhs_.
    // That OfferChange should have BMC for TakerGets.  There should be a 1
    ...
    // The side with the smaller BMC balance in the OfferChange should have
    // two BMC differences.  The other side should have no BMC differences.
    if (smaller.bmcChanges.size() != 2)
        return 0;
    if (! bigger.bmcChanges.empty())
        return 0;
...
bool CashDiff::Impl::rmDust ()
{
    ...
    // amount differences: bmcChanges, trustChanges, offerChanges, and
    // offerDeletions.  Rifle through those containers and remove any
    // entries that are _almost_ the same between lhs and rhs.

    // bmcChanges.  We call a difference of 2 drops or less dust.
    removedDust |= rmVecDust (lhsDiffs_.bmcChanges, rhsDiffs_.bmcChanges,
        [](BMCAmount const& lhs, BMCAmount const& rhs)
        {
        ...
        CashFilter rhsFilter, detail::ApplyStateTable const& rhs)
{
    ...
    // bmcChanges:
    setDiff (lhsDiffs.bmcChanges, rhsDiffs.bmcChanges, lhsDiffs_.bmcChanges);
    setDiff (rhsDiffs.bmcChanges, lhsDiffs.bmcChanges, rhsDiffs_.bmcChanges);
...
int CashDiff::bmcRoundToZero() const
{
    return impl_->bmcRoundToZero();
}
...
bool diffIsDust (STAmount const& v1, STAmount const& v2, std::uint8_t e10)
{
    ...
    // Handling BMC is different from IOU.
    if (v1.native())
    {

...
#include <blocmatrix/ledger/BookDirs.h>
...
#include <blocmatrix/protocol/st.h>
#include <blocmatrix/protocol/Protocol.h>
              ...
              beast::Journal j)
{
    ...
    if (isBMC(currency))
    {
        return {bmcLiquid (view, account, 0, j)};
    }
...
BMCAmount
bmcLiquid (ReadView const& view, AccountID const& id,
    std::int32_t ownerCountAdj, beast::Journal j)
{
    ...
    if (fix1141 (view.info ().parentCloseTime))
    {
        ...
        auto const balance = view.balanceHook(id, bmcAccount(), fullBalance);
    ...
    {
        // pre-switchover
        // BMC: return balance minus reserve
        ...
        return view.balanceHook(id, bmcAccount(), amount).bmc();
    }
}
...
trustCreate (ApplyView& view,
    ...
    uint256 const&  uIndex,             // --> blocmatrix state entry
    SLE::ref        sleAccount,         // --> the account being set.
    const bool      bAuth,              // --> authorize account.
    const bool      bNoBlocmatrix,          // --> others cannot blocmatrix through
    ...
    beast::Journal j)
{
    ...
    auto const sleBlocmatrixState = std::make_shared<SLE>(
        ltBLOCMATRIX_STATE, uIndex);
    view.insert (sleBlocmatrixState);

    auto lowNode = dirAdd (view, keylet::ownerDir (uLowAccountID),
        sleBlocmatrixState->key(), false, describeOwnerDir (uLowAccountID), j);

    if (!lowNode)
        return tecDIR_FULL;

    auto highNode = dirAdd (view, keylet::ownerDir (uHighAccountID),
        sleBlocmatrixState->key(), false, describeOwnerDir (uHighAccountID), j);
    ...
    sleBlocmatrixState->setFieldU64 (sfLowNode, *lowNode);
    sleBlocmatrixState->setFieldU64 (sfHighNode, *highNode);

    sleBlocmatrixState->setFieldAmount (
    ...
    if (uQualityIn)
        sleBlocmatrixState->setFieldU32 (
    ...
    if (bNoBlocmatrix)
    {
        uFlags |= (bSetHigh ? lsfHighNoBlocmatrix : lsfLowNoBlocmatrix);
    }
    if (bFreeze)
    {
    ...
    if ((slePeer->getFlags() & lsfDefaultBlocmatrix) == 0)
    {
        // The other side's default is no rippling
        uFlags |= (bSetHigh ? lsfLowNoBlocmatrix : lsfHighNoBlocmatrix);
    }

    sleBlocmatrixState->setFieldU32 (sfFlags, uFlags);
    adjustOwnerCount(view, sleAccount, 1, j);

    // ONLY: Create blocmatrix balance.
    sleBlocmatrixState->setFieldAmount (sfBalance, bSetHigh ? -saBalance : saBalance);
...
trustDelete (ApplyView& view,
    std::shared_ptr<SLE> const& sleBlocmatrixState,
        AccountID const& uLowAccountID,
            AccountID const& uHighAccountID,
                 beast::Journal j)
{
    // Detect legacy dirs.
    std::uint64_t uLowNode    = sleBlocmatrixState->getFieldU64 (sfLowNode);
    std::uint64_t uHighNode   = sleBlocmatrixState->getFieldU64 (sfHighNode);

    JLOG (j.trace())
        << "trustDelete: Deleting blocmatrix line: low";

    if (! view.dirRemove(
            keylet::ownerDir(uLowAccountID),
            uLowNode,
            sleBlocmatrixState->key(),
            false))
    {
    ...
    JLOG (j.trace())
            << "trustDelete: Deleting blocmatrix line: high";
    ...
    JLOG (j.trace()) << "trustDelete: Deleting blocmatrix line: state";
    view.erase(sleBlocmatrixState);
...
TER
blocmatrixCredit (ApplyView& view,
    ...
    beast::Journal j)
{
    ...
    uint256 uIndex = getBlocmatrixStateIndex (
        uSenderID, uReceiverID, saAmount.getCurrency ());
    auto sleBlocmatrixState  = view.peek (keylet::line(uIndex));

    TER terResult;

    assert (!isBMC (uSenderID) && uSenderID != noAccount());
    assert (!isBMC (uReceiverID) && uReceiverID != noAccount());

    if (!sleBlocmatrixState)
    {
        ...
        bool noBlocmatrix = (sleAccount->getFlags() & lsfDefaultBlocmatrix) == 0;
    ...
    {
        STAmount saBalance   = sleBlocmatrixState->getFieldAmount (sfBalance);
        ...
        std::uint32_t const uFlags (sleBlocmatrixState->getFieldU32 (sfFlags));
        bool bDelete = false;
        ...
        if (saBefore > beast::zero
            ...
            && static_cast <bool> (uFlags & (!bSenderHigh ? lsfLowNoBlocmatrix : lsfHighNoBlocmatrix)) !=
               static_cast <bool> (view.read (keylet::account(uSenderID))->getFlags() & lsfDefaultBlocmatrix)
            && !(uFlags & (!bSenderHigh ? lsfLowFreeze : lsfHighFreeze))
            && !sleBlocmatrixState->getFieldAmount (
                !bSenderHigh ? sfLowLimit : sfHighLimit)
            // Sender trust limit is 0.
            && !sleBlocmatrixState->getFieldU32 (
        ...
        sleBlocmatrixState->setFieldAmount (sfBalance, saBalance);
        // ONLY: Adjust blocmatrix balance.
        ...
        {
            view.update (sleBlocmatrixState);
...
static
STAmount
blocmatrixTransferFee (ReadView const& view,
    ...
    beast::Journal j)
{
    if (from != issuer && to != issuer)
    {
        ...
        if (parityRate != rate)
        {
            ...
            JLOG (j.debug()) << "blocmatrixTransferFee:" <<
                " amount=" << amount.getFullText () <<
                " fee=" << fee.getFullText ();
...
static
TER
blocmatrixSend (ApplyView& view,
    AccountID const& uSenderID, AccountID const& uReceiverID,
    STAmount const& saAmount, STAmount& saActual, beast::Journal j)
{
    auto const issuer   = saAmount.getIssuer ();

    assert (!isBMC (uSenderID) && !isBMC (uReceiverID));
    assert (uSenderID != uReceiverID);

    if (uSenderID == issuer || uReceiverID == issuer || issuer == noAccount())
    {
        // Direct send: redeeming IOUs and/or sending own IOUs.
        blocmatrixCredit (view, uSenderID, uReceiverID, saAmount, false, j);
        saActual = saAmount;
        return tesSUCCESS;
    }
    ...
    if (!fix1141 (view.info ().parentCloseTime))
    {
        STAmount const saTransitFee = blocmatrixTransferFee (
    ...
    JLOG (j.debug()) << "blocmatrixSend> " <<
        to_string (uSenderID) <<
        " - > " << to_string (uReceiverID) <<
        " : deliver=" << saAmount.getFullText () <<
        " cost=" << saActual.getFullText ();

    TER terResult   = blocmatrixCredit (view, issuer, uReceiverID, saAmount, true, j);

    if (tesSUCCESS == terResult)
        terResult = blocmatrixCredit (view, uSenderID, issuer, saActual, true, j);
    ...
    STAmount const& saAmount, beast::Journal j)
{
    ...
    if (!saAmount.native ())
    {
        ...
        return blocmatrixSend (view, uSenderID, uReceiverID, saAmount, saActual, j);
    }
    ...
    /* BMC send which does not check reserve and can do pure adjustment.
    ...
    if (sender)
    {
        ...
        else
        {
            ...
            if (fv2Switch)
                view.creditHook (uSenderID, bmcAccount (), saAmount, sndBal);

            // Decrement BMC balance.
    ...
    {
        // Increment BMC balance.
        auto const rcvBal = receiver->getFieldAmount (sfBalance);
        receiver->setFieldAmount (sfBalance, rcvBal + saAmount);

        if (fv2Switch)
            view.creditHook (bmcAccount (), uReceiverID, saAmount, -rcvBal);
    ...
    beast::Journal j)
{
    ...
    if (before > beast::zero
        ...
        // Sender reserve is set.
        && static_cast <bool> (flags & (!bSenderHigh ? lsfLowNoBlocmatrix : lsfHighNoBlocmatrix)) !=
           static_cast <bool> (sle->getFlags() & lsfDefaultBlocmatrix)
...
{
    assert (!isBMC (account) && !isBMC (issue.account));
    ...
    uint256 const index = getBlocmatrixStateIndex (
    ...
    if (!state)
    {
        ...
        bool noBlocmatrix = (receiverAccount->getFlags() & lsfDefaultBlocmatrix) == 0;

        return trustCreate (view, bSenderHigh, issue.account, account, index,
            receiverAccount, false, noBlocmatrix, false, final_balance, limit, 0, 0, j);
    }

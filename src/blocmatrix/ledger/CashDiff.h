...
#ifndef BLOCMATRIX_LEDGER_CASHDIFF_H_INCLUDED
#define BLOCMATRIX_LEDGER_CASHDIFF_H_INCLUDED
...
public:
    ...
    // Checks for the BMC round-to-zero case.  Returns zero if not detected.
    // Otherwise returns -1 if seen on lhs, +1 if seen on rhs.
    //
    // For tiny offers of TakerPays IOU and TakerGets BMC, cases have been
    // observed where BMC rounding allows a tiny amount of IOU to be
    // removed from an Offer while returning no BMC to the offer owner.
    // That's because the BMC amount was rounded down to zero drops.
    //
...
// If both v1 and v2 are BMC, consider any diff of 2 drops or less to be dust.

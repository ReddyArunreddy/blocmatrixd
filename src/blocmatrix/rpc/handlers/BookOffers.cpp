...
Json::Value doBookOffers (RPC::Context& context)
{
    ...
    {
        pay_issuer = bmcAccount ();
    }

    if (isBMC (pay_currency) && ! isBMC (pay_issuer))
        return RPC::make_error (
            rpcSRC_ISR_MALFORMED, "Unneeded field 'taker_pays.issuer' for "
            "BMC currency specification.");
    ...
    {
        get_issuer = bmcAccount ();
    }


    if (isBMC (get_currency) && ! isBMC (get_issuer))
        return RPC::make_error (rpcDST_ISR_MALFORMED,
    ...
    if (!isBMC (get_currency) && isBMC (get_issuer))
        return RPC::make_error (rpcDST_ISR_MALFORMED,
            "Invalid field 'taker_gets.issuer', expected non-BMC issuer.");

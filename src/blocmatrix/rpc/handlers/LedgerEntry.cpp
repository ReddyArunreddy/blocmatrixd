...
#include <blocmatrix/basics/strHex.h>
...
Json::Value doLedgerEntry (RPC::Context& context)
{
    ...
    else if (context.params.isMember (jss::blocmatrix_state))
    {
        expectedType = ltBLOCMATRIX_STATE;
        Currency         uCurrency;
        Json::Value     jvBlocmatrixState   = context.params[jss::blocmatrix_state];

        if (!jvBlocmatrixState.isObject()
            || !jvBlocmatrixState.isMember (jss::currency)
            || !jvBlocmatrixState.isMember (jss::accounts)
            || !jvBlocmatrixState[jss::accounts].isArray()
            || 2 != jvBlocmatrixState[jss::accounts].size ()
            || !jvBlocmatrixState[jss::accounts][0u].isString ()
            || !jvBlocmatrixState[jss::accounts][1u].isString ()
            || (jvBlocmatrixState[jss::accounts][0u].asString ()
                == jvBlocmatrixState[jss::accounts][1u].asString ())
           )
        {
            jvResult[jss::error] = "malformedRequest";
        }
        else
        {
            ...
            if (! id1 || ! id2)
            {
                ...
                jvBlocmatrixState[jss::currency].asString()))
            {
                jvResult[jss::error] = "malformedCurrency";
            }
            else
            {
                uNodeIndex = getBlocmatrixStateIndex(
                    *id1, *id2, uCurrency);
            }
        }
    }

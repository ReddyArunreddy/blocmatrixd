...
#include <blocmatrix/rpc/handlers/WalletPropose.h>
...
Json::Value walletPropose (Json::Value const& params)
{
    ...
    // to detect such keys to avoid user confusion.
    {
        if (params.isMember(jss::passphrase))
            seed = RPC::parseBlocmatrixLibSeed(params[jss::passphrase]);
        else if (params.isMember(jss::seed))
            seed = RPC::parseBlocmatrixLibSeed(params[jss::seed]);

        if(seed)
        {
            blocmatrixLibSeed = true;
    ...
    if (!blocmatrixLibSeed && params.isMember (jss::passphrase))
    {

...
struct VisitData
{
    std::vector <BlocmatrixState::pointer> items;
    AccountID const& accountID;
    bool hasPeer;
    AccountID const& raPeerAccount;
};

void addLine (Json::Value& jsonLines, BlocmatrixState const& line)
{
    ...
    if (line.getNoBlocmatrix ())
        jPeer[jss::no_blocmatrix] = true;
    if (line.getNoBlocmatrixPeer ())
        jPeer[jss::no_blocmatrix_peer] = true;
    if (line.getFreeze ())
...
Json::Value doAccountLines (RPC::Context& context)
{
    ...
    if (params.isMember (jss::marker))
    {
        ...
        auto const sleLine = ledger->read({ltBLOCMATRIX_STATE, startAfter});
        ...
        auto const line = BlocmatrixState::makeItem (accountID, sleLine);
        if (line == nullptr)
            return rpcError (rpcINVALID_PARAMS);
...

    {
        if (! forEachItemAfter(*ledger, accountID,
            ...
            {
                auto const line =
                    BlocmatrixState::makeItem (visitData.accountID, sleCur);
                if (line != nullptr &&
                    (! visitData.hasPeer ||
    ...
    if (visitData.items.size () == reserve)
    {
        ...
        BlocmatrixState::pointer line (visitData.items.back ());

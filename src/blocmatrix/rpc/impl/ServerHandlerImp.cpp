...
#include <blocmatrix/beast/net/IPAddressConversion.h>
#include <blocmatrix/json/json_reader.h>
#include <blocmatrix/rpc/json_body.h>
#include <blocmatrix/rpc/ServerHandler.h>
#include <blocmatrix/server/Server.h>
#include <blocmatrix/server/impl/JSONRPCUtil.h>
#include <blocmatrix/rpc/impl/ServerHandlerImp.h>
#include <blocmatrix/basics/contract.h>
#include <blocmatrix/basics/Log.h>
#include <blocmatrix/basics/make_SSLContext.h>
...
#include <blocmatrix/rpc/Role.h>
#include <blocmatrix/rpc/RPCHandler.h>
#include <blocmatrix/server/SimpleWriter.h>
            ...
            Json::Value const& jv)
{
    ...
    try
    {
             ...
             jv[jss::command].asString() != jv[jss::method].asString()))
        {
            ...
            if (jv.isMember(jss::blocmatribmcc))
                jr[jss::blocmatribmcc] = jv[jss::blocmatribmcc];
        ...
        boost::string_view forwardedFor, boost::string_view user)
{
    ...
    for (unsigned i = 0; i < size; ++i)
    {
        ...
        std::string blocmatribmcc = "1.0";
        if (params.isMember(jss::blocmatribmcc))
        {
            if (!params[jss::blocmatribmcc].isString())
            {
                usage.charge(Resource::feeInvalidRPC);
                if (!batch)
                {
                    HTTPReply(400, "blocmatribmcc is not a string", output, rpcJ);
                    return;
                }

                Json::Value r = jsonRPC;
                r[jss::error] = make_json_error(
                    method_not_found, "blocmatribmcc is not a string");
                reply.append(r);
                continue;
            }
            blocmatribmcc = params[jss::blocmatribmcc].asString();
        }
        ...
        if (params.isMember(jss::blocmatribmcc))
           r[jss::blocmatribmcc] = params[jss::blocmatribmcc];
    ...
    http_request_type const& request) const
{
    ...
    if (app_.serverOkay(reason))
    {
        ...
        msg.body() = "<!DOCTYPE html><html><head><title>" + systemName() +
            " Test page for blocmatrixd</title></head><body><h1>" +
                systemName() + " Test</h1><p>This page shows blocmatrixd http(s) "

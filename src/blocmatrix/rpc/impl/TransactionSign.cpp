...
#include <blocmatrix/rpc/impl/TransactionSign.h>
...
#include <blocmatrix/app/tx/apply.h>              // Validity::Valid
...
#include <blocmatrix/rpc/impl/LegacyPathFind.h>
    ...
    bool doPath)
{
    ...
    if (!tx_json.isMember (jss::Paths) && params.isMember (jss::build_path))
    {
...

        {
            ...
            if (ledger)
            {
                Pathfinder pf(std::make_shared<BlocmatrixLineCache>(ledger),
    ...
    ProcessTransactionFn const& processTransaction)
{
    ...
    // Validate the fields in the serialized transaction.
    {
        ...
        // The Fee field must be in BMC and greater than zero.
        ...
        if (!isLegalNet (fee))
        {
            ...
            err << "Invalid " << sfFee.fieldName
                << " field.  Fees must be specified in BMC.";
            return RPC::make_error (rpcINVALID_PARAMS, err.str ());
        }
        if (fee <= 0)
        {

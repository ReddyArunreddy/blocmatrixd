...
#ifndef BLOCMATRIX_APP_BOOK_TAKER_H_INCLUDED
#define BLOCMATRIX_APP_BOOK_TAKER_H_INCLUDED

#include <blocmatrix/app/tx/impl/Offer.h>
...
protected:
    ...
    struct Flow
    {
        ...
        bool sanity_check () const
        {
            ...
            if (isBMC (order.in) && isBMC (order.out))
                return false;
...
private:
    ...
    Flow
    flow_bmc_to_iou (Amounts const& offer, Quality quality,
        STAmount const& owner_funds, STAmount const& taker_funds,
        Rate const& rate_out);

    Flow
    flow_iou_to_bmc (Amounts const& offer, Quality quality,
        STAmount const& owner_funds, STAmount const& taker_funds,
        Rate const& rate_in);
...
private:
    ...
    TER
    transferBMC (AccountID const& from, AccountID const& to, STAmount const& amount);
...
private:
    ...
    // The amount of BMC that flowed if we were autobridging
    STAmount bmc_flow_;

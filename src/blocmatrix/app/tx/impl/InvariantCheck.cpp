...
#include <blocmatrix/app/tx/impl/InvariantCheck.h>
    ...
    beast::Journal const& j)
{
    ...
    if (fee > tx.getFieldAmount(sfFee).bmc())
    {
    ...
    std::shared_ptr <SLE const> const& after)
{
     ...
     * previous BMC values and add to the total any new BMC values. The net
    ...
    if(before)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            drops_ -= (*before)[sfBalance].bmc().drops();
            break;
        case ltPAYCHAN:
            drops_ -= ((*before)[sfAmount] - (*before)[sfBalance]).bmc().drops();
            break;
        case ltESCROW:
            drops_ -= (*before)[sfAmount].bmc().drops();
            break;
        default:
            break;
        }
    ...
    if(after)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            drops_ += (*after)[sfBalance].bmc().drops();
            break;
        case ltPAYCHAN:
            if (! isDelete)
                drops_ += ((*after)[sfAmount] - (*after)[sfBalance]).bmc().drops();
            break;
        case ltESCROW:
            if (! isDelete)
                drops_ += (*after)[sfAmount].bmc().drops();
            break;
        default:
            break;
        }
    ...
    beast::Journal const& j)
{
    ...
    // transaction created BMC out of thin air. That's not possible.
    if (drops_ > 0)
    {
        JLOG(j.fatal()) <<
            "Invariant failed: BMC net change was positive: " << drops_;
        return false;
    }
    ...
    {
        JLOG(j.fatal()) <<
            "Invariant failed: BMC net change of " << drops_ <<
    ...
    std::shared_ptr <SLE const> const& after)
{
    auto isBad = [](STAmount const& balance)
    {
        ...
        auto const drops = balance.bmc().drops();
...
bool
BMCBalanceChecks::finalize(STTx const&, TER const, BMCAmount const, beast::Journal const& j)
{
    if (bad_)
    {
        JLOG(j.fatal()) << "Invariant failed: incorrect account BMC balance";
        return false;
    }
    ...
    std::shared_ptr <SLE const> const& after)
{
    auto isBad = [](STAmount const& pays, STAmount const& gets)
    {
        ...
        // Can't have an BMC to BMC offer:
...
bool
NoBadOffers::finalize(STTx const& tx, TER const, BMCAmount const, beast::Journal const& j)
{
    if (bad_)
    {
    ...
    std::shared_ptr <SLE const> const& after)
{
    auto isBad = [](STAmount const& amount)
    {
        ...
        if (amount.bmc().drops() <= 0)
            return true;

        if (amount.bmc().drops() >= SYSTEM_CURRENCY_START)
            return true;
...
bool
NoZeroEscrow::finalize(STTx const& tx, TER const, BMCAmount const, beast::Journal const& j)
{
    if (bad_)
    {
...
bool
AccountRootsNotDeleted::finalize(STTx const&, TER const, BMCAmount const, beast::Journal const& j)
{
    if (! accountDeleted_)
        return true;
...
bool
LedgerEntryTypesMatch::finalize(STTx const&, TER const, BMCAmount const, beast::Journal const& j)
{
...
void
NoBMCTrustLines::visitEntry(
    uint256 const&,
    bool,
    std::shared_ptr <SLE const> const&,
    std::shared_ptr <SLE const> const& after)
{
    if (after && after->getType() == ltBLOCMATRIX_STATE)
    {
        // checking the issue directly here instead of
        // relying on .native() just in case native somehow
        // were systematically incorrect
        bmcTrustLine_ =
            after->getFieldAmount (sfLowLimit).issue() == bmcIssue() ||
            after->getFieldAmount (sfHighLimit).issue() == bmcIssue();
    }
}

bool
NoBMCTrustLines::finalize(STTx const&, TER const, BMCAmount const, beast::Journal const& j)
{
    if (! bmcTrustLine_)
        return true;

    JLOG(j.fatal()) << "Invariant failed: an BMC trust line was created";
    return false;
}

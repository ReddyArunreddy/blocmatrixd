...
#include <blocmatrix/app/tx/impl/Payment.h>
...
Payment::calculateMaxSpend(STTx const& tx)
{
    if (tx.isFieldPresent(sfSendMax))
    {
        ...
        return sendMax.native() ? sendMax.bmc() : beast::zero;
    }
    /* If there's no sfSendMax in BMC, and the sfAmount isn't
    in BMC, then the transaction can not send BMC. */
    auto const& saDstAmount = tx.getFieldAmount(sfAmount);
    return saDstAmount.native() ? saDstAmount.bmc() : beast::zero;
}
...
Payment::preflight (PreflightContext const& ctx)
{
    ...
    bool const defaultPathsAllowed = !(uTxFlags & tfNoBlocmatrixDirect);
    ...
    // isZero() is BMC.  FIX!
    bool const bBMCDirect = uSrcCurrency.isZero () && uDstCurrency.isZero ();
    ...
    if (bBMCDirect && bMax)
    {
        ...
        JLOG(j.trace()) << "Malformed transaction: " <<
            "SendMax specified for BMC to BMC.";
        return temBAD_SEND_BMC_MAX;
    }
    if (bBMCDirect && bPaths)
    {
        // BMC is sent without paths.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Paths specified for BMC to BMC.";
        return temBAD_SEND_BMC_PATHS;
    }
    if (bBMCDirect && partialPaymentAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Partial payment specified for BMC to BMC.";
        return temBAD_SEND_BMC_PARTIAL;
    }
    if (bBMCDirect && limitQuality)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Limit quality specified for BMC to BMC.";
        return temBAD_SEND_BMC_LIMIT;
    }
    if (bBMCDirect && !defaultPathsAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "No blocmatrix direct specified for BMC to BMC.";
        return temBAD_SEND_BMC_NO_DIRECT;
    }
...
{
    // Blocmatrix if source or destination is non-native or if there are paths.
    ...
    {
        // Blocmatrix payment with at least one intermediate step and uses
        // transitive balances.
...
Payment::doApply ()
{
    ...
    bool const bBlocmatrix = paths || sendMax || !saDstAmount.native ();

    // If the destination has lsfDepositAuth set, then only direct BMC
    // payments (no intermediate steps) are allowed to the destination.
    if (!depositPreauth && bBlocmatrix && reqDepositAuth)
    ...
    if (bBlocmatrix)
    {
        ...
        path::BlocmatrixCalc::Output rc;
        {
            PaymentSandbox pv(&view());
            JLOG(j_.debug())
                << "Entering BlocmatrixCalc in payment: "
        ...
        // Because of its overhead, if BlocmatrixCalc
    ...
    auto const mmm = std::max(reserve,
        ctx_.tx.getFieldAmount (sfFee).bmc ());

    if (mPriorBalance < saDstAmount.bmc () + mmm)
    {
        // Vote no. However the transaction might succeed, if applied in
        // a different order.
        JLOG(j_.trace()) << "Delay transaction: Insufficient funds: " <<
            " " << to_string (mPriorBalance) <<
            " / " << to_string (saDstAmount.bmc () + mmm) <<
            " (" << to_string (reserve) << ")";
    ...
    {
        // If depositPreauth is enabled, then an account that requires
        // authorization has three ways to get an BMC Payment in:
        //  1. If Account == Destination, or
        //  2. If Account is deposit preauthorized by destination, or
        //  3. If the destination's BMC balance is
        ...
        // then consumes all of its BMC.  Without the rule if an
        // account with lsfDepositAuth set spent all of its BMC, it
        // would be unable to acquire more BMC required to pay fees.
        //
        ...
        if (uDstAccountID != account_)
        {
                ...
                keylet::depositPreauth (uDstAccountID, account_)))
            {
                ...
                BMCAmount const dstReserve {view().fees().accountReserve (0)};

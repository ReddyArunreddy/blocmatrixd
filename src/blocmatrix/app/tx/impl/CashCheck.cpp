...
#include <blocmatrix/protocol/STAccount.h>
...
CashCheck::preclaim (PreclaimContext const& ctx)
{
    ...
    }
    {
        ...
        // less than value the check cannot be cashed.
        {
            ...
            // Note that src will have one reserve's worth of additional BMC
            // once the check is cashed, since the check's reserve will no
            // longer be required.  So, if we're dealing in BMC, we add one
            // reserve's worth to the available funds.
            if (value.native())
                availableFunds += BMCAmount (ctx.view.fees().increment);
...
CashCheck::doApply ()
{
    ...
    if (srcId != account_)
    {
        ...
        // Flow() doesn't do BMC to BMC transfers.
        if (sendMax.native())
        {
            // Here we need to calculate the amount of BMC sleSrc can send.
            ...
            // argument.
            STAmount const srcLiquid {bmcLiquid (psb, srcId, -1, viewJ)};

            // Now, how much do they need in order to be successful?
            STAmount const bmcDeliver {optDeliverMin ?
                std::max (*optDeliverMin, std::min (sendMax, srcLiquid)) :
                ctx_.tx.getFieldAmount (sfAmount)};

            if (srcLiquid < bmcDeliver)
            {
                // Vote no. However the transaction might succeed if applied
                // in a different order.
                JLOG(j_.trace()) << "Cash Check: Insufficient BMC: "
                    << srcLiquid.getFullText()
                    << " < " << bmcDeliver.getFullText();
                return tecUNFUNDED_PAYMENT;
            }

            if (optDeliverMin && doFix1623)
                // Set the DeliveredAmount metadata.
                ctx_.deliver (bmcDeliver);

            // The source account has enough BMC so make the ledger change.
            transferBMC (psb, srcId, account_, bmcDeliver, viewJ);
        }
        else
        {

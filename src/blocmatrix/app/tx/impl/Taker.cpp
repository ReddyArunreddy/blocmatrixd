    ...
    , journal_ (journal)
{
    ...
    assert (cross_type != CrossType::XrpToIou ||
        (isBMC (issue_in ()) && !isBMC (issue_out ())));

    assert (cross_type != CrossType::IouToXrp ||
        (!isBMC (issue_in ()) && isBMC (issue_out ())));

    // And make sure we're not crossing BMC for BMC
    assert (!isBMC (issue_in ()) || !isBMC (issue_out ()));
...
BasicTaker::do_cross (Amounts offer, Quality quality, AccountID const& owner)
{
    ...
    {
        result = flow_bmc_to_iou (offer, quality, owner_funds, taker_funds,
            out_rate (owner, account ()));
    }
    else if (cross_type_ == CrossType::IouToXrp)
    {
        result = flow_iou_to_bmc (offer, quality, owner_funds, taker_funds,
    ...
    Amounts offer2, Quality quality2, AccountID const& owner2)
{
    ...
    // The amount available to flow via BMC is the amount that the owner of the
    ...
    // of BMC that can flow between the two legs is, essentially, infinite
    // since all the owner is doing is taking out BMC of his left pocket
    // and putting it in his right pocket. In that case, we set the available
    // BMC to the largest of the two offers.
    auto bmc_funds = get_funds (owner1, offer1.out);

    if (owner1 == owner2)
    {
        JLOG(journal_.trace()) <<
            "The bridge endpoints are owned by the same account.";
        bmc_funds = std::max (offer1.out, offer2.in);
    }
    ...
    if (auto stream = journal_.debug())
    {
        ...
        stream << "      bmc: " << format_amount (bmc_funds);
    }
    ...
    auto flow1 = flow_iou_to_bmc (offer1, quality1, bmc_funds, leg1_in_funds, leg1_rate);

    if (!flow1.sanity_check ())
        Throw<std::logic_error> ("Computed flow1 fails sanity check.");

    auto flow2 = flow_bmc_to_iou (offer2, quality2, leg2_out_funds, bmc_funds, leg2_rate);
    ...
    // equalize them, so that the amount of BMC that flows out of the first leg
    // is the same as the amount of BMC that flows into the second leg. We take
...
TER Taker::transferBMC (
    AccountID const& from,
    AccountID const& to,
    STAmount const& amount)
{
    if (!isBMC (amount))
        Throw<std::logic_error> ("Using transferBMC with IOU");
    ...
    return blocmatrix::transferBMC (view_, from, to, amount, journal_);
}
...
{
    if (isBMC (amount))
        Throw<std::logic_error> ("Using redeemIOU with BMC");
    ...
    auto ret = blocmatrix::redeemIOU (view_, account, amount, issue, journal_);
...
{
    if (isBMC (amount))
        Throw<std::logic_error> ("Using issueIOU with BMC");
    ...
    return blocmatrix::issueIOU (view_, account, amount, issue, journal_);
}
...
Taker::fill (BasicTaker::Flow const& flow, Offer& offer)
{
    ...
    else
    {
        ...
        if (result == tesSUCCESS)
            result = transferBMC (offer.owner (), account (), flow.order.out);
    }
    ...
    BasicTaker::Flow const& flow2, Offer& leg2)
{
    ...
    // leg1 to leg2: bridging over BMC
    if (result == tesSUCCESS)
        result = transferBMC (leg1.owner (), leg2.owner (), flow1.order.out);
...
{
    // In direct crossings, at least one leg must not be BMC.
    if (isBMC (offer.amount ().in) && isBMC (offer.amount ().out))
        return tefINTERNAL;
...
{
    // In bridged crossings, BMC must can't be the input to the first leg
    // or the output of the second leg.
    if (isBMC (leg1.amount ().in) || isBMC (leg2.amount ().out))
        return tefINTERNAL;
...
Taker::calculateRate (
    ApplyView const& view,
        AccountID const& issuer,
            AccountID const& account)
{
    return isBMC (issuer) || (account == issuer)
        ? parityRate

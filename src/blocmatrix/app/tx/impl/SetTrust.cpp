...
#include <blocmatrix/app/tx/impl/SetTrust.h>
...
SetTrust::preflight (PreflightContext const& ctx)
{
    ...
    {
        JLOG(j.trace()) <<
            "Malformed transaction: specifies BMC as IOU";
...
SetTrust::doApply ()
{
    ...
    // which needs trust lines, is a powerful Blocmatrix feature.
    ...
    // user enough BMC to cover not only the account reserve
    // but the incremental reserve for the trust line as
    // well. A person with no intention of using the gateway
    // could use the extra BMC for their own purposes.

    BMCAmount const reserveCreate ((uOwnerCount < 2)
        ? BMCAmount (beast::zero)
    ...
    bool const bSetAuth = (uTxFlags & tfSetfAuth);
    bool const bSetNoBlocmatrix = (uTxFlags & tfSetNoBlocmatrix);
    bool const bClearNoBlocmatrix  = (uTxFlags & tfClearNoBlocmatrix);
    ...
    SLE::pointer sleBlocmatrixState = view().peek (
        keylet::line(account_, uDstAccountID, currency));
    ...
    if (sleBlocmatrixState)
    {
        ...
        saLowBalance    = sleBlocmatrixState->getFieldAmount (sfBalance);
        ...
        sleBlocmatrixState->setFieldAmount (!bHigh ? sfLowLimit : sfHighLimit, saLimitAllow);

        saLowLimit  = !bHigh ? saLimitAllow : sleBlocmatrixState->getFieldAmount (sfLowLimit);
        saHighLimit =  bHigh ? saLimitAllow : sleBlocmatrixState->getFieldAmount (sfHighLimit);
        ...
        if (!bQualityIn)
        {
            ...
            uLowQualityIn   = sleBlocmatrixState->getFieldU32 (sfLowQualityIn);
            uHighQualityIn  = sleBlocmatrixState->getFieldU32 (sfHighQualityIn);
        }
        else if (uQualityIn)
        {
            // Setting.

            sleBlocmatrixState->setFieldU32 (!bHigh ? sfLowQualityIn : sfHighQualityIn, uQualityIn);

            uLowQualityIn   = !bHigh ? uQualityIn : sleBlocmatrixState->getFieldU32 (sfLowQualityIn);
            uHighQualityIn  =  bHigh ? uQualityIn : sleBlocmatrixState->getFieldU32 (sfHighQualityIn);
        }
        else
        {
            // Clearing.

            sleBlocmatrixState->makeFieldAbsent (!bHigh ? sfLowQualityIn : sfHighQualityIn);

            uLowQualityIn   = !bHigh ? 0 : sleBlocmatrixState->getFieldU32 (sfLowQualityIn);
            uHighQualityIn  =  bHigh ? 0 : sleBlocmatrixState->getFieldU32 (sfHighQualityIn);
        }
        ...
        if (!bQualityOut)
        {
            ...
            uLowQualityOut  = sleBlocmatrixState->getFieldU32 (sfLowQualityOut);
            uHighQualityOut = sleBlocmatrixState->getFieldU32 (sfHighQualityOut);
        }
        else if (uQualityOut)
        {
            // Setting.

            sleBlocmatrixState->setFieldU32 (!bHigh ? sfLowQualityOut : sfHighQualityOut, uQualityOut);

            uLowQualityOut  = !bHigh ? uQualityOut : sleBlocmatrixState->getFieldU32 (sfLowQualityOut);
            uHighQualityOut =  bHigh ? uQualityOut : sleBlocmatrixState->getFieldU32 (sfHighQualityOut);
        }
        else
        {
            // Clearing.

            sleBlocmatrixState->makeFieldAbsent (!bHigh ? sfLowQualityOut : sfHighQualityOut);

            uLowQualityOut  = !bHigh ? 0 : sleBlocmatrixState->getFieldU32 (sfLowQualityOut);
            uHighQualityOut =  bHigh ? 0 : sleBlocmatrixState->getFieldU32 (sfHighQualityOut);
        }

        std::uint32_t const uFlagsIn (sleBlocmatrixState->getFieldU32 (sfFlags));
        std::uint32_t uFlagsOut (uFlagsIn);

        if (bSetNoBlocmatrix && !bClearNoBlocmatrix)
        {
            if ((bHigh ? saHighBalance : saLowBalance) >= beast::zero)
                uFlagsOut |= (bHigh ? lsfHighNoBlocmatrix : lsfLowNoBlocmatrix);

            else if (view().rules().enabled(fix1578))
                // Cannot set noBlocmatrix on a negative balance.
                return tecNO_PERMISSION;
        }
        else if (bClearNoBlocmatrix && !bSetNoBlocmatrix)
        {
            uFlagsOut &= ~(bHigh ? lsfHighNoBlocmatrix : lsfLowNoBlocmatrix);
        }
        ...
        bool const bLowDefBlocmatrix        = sleLowAccount->getFlags() & lsfDefaultBlocmatrix;
        bool const bHighDefBlocmatrix       = sleHighAccount->getFlags() & lsfDefaultBlocmatrix;

        bool const  bLowReserveSet      = uLowQualityIn || uLowQualityOut ||
                                            ((uFlagsOut & lsfLowNoBlocmatrix) == 0) != bLowDefBlocmatrix ||
                                            (uFlagsOut & lsfLowFreeze) ||
                                            saLowLimit || saLowBalance > beast::zero;
        bool const  bLowReserveClear    = !bLowReserveSet;

        bool const  bHighReserveSet     = uHighQualityIn || uHighQualityOut ||
                                            ((uFlagsOut & lsfHighNoBlocmatrix) == 0) != bHighDefBlocmatrix ||
        ...
        if (uFlagsIn != uFlagsOut)
            sleBlocmatrixState->setFieldU32 (sfFlags, uFlagsOut);
        ...
        if (bDefault || badCurrency() == currency)
        {
            ...
            terResult = trustDelete (view(),
                sleBlocmatrixState, uLowAccountID, uHighAccountID, viewJ);
        }
        ...
        else if (bReserveIncrease && mPriorBalance < reserveCreate)
        {
            ...
            // Another transaction could provide BMC to the account and then
        ...
        else
        {
            ...
            JLOG(j_.trace()) << "Modify blocmatrix line";
        }
    }
    // Line does not exist.
    ...
    {
        JLOG(j_.trace()) <<
            "Redundant: Setting non-existent blocmatrix line to defaults.";
    ...
    else
    {
        ...
        uint256 index (getBlocmatrixStateIndex (
            account_, uDstAccountID, currency));

        JLOG(j_.trace()) <<
            "doTrustSet: Creating blocmatrix line: " <<
            to_string (index);

        // Create a new blocmatrix line.
        terResult = trustCreate (view(),
            ...
            sle,
            bSetAuth,
            bSetNoBlocmatrix && !bClearNoBlocmatrix,

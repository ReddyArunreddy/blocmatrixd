...
SetAccount::preflight (PreflightContext const& ctx)
{
    ...
    // DisallowBMC
    //
    bool bSetDisallowBMC   = (uTxFlags & tfDisallowBMC) || (uSetFlag == asfDisallowBMC);
    bool bClearDisallowBMC = (uTxFlags & tfAllowBMC) || (uClearFlag == asfDisallowBMC);

    if (bSetDisallowBMC && bClearDisallowBMC)
    {
        JLOG(j.trace()) << "Malformed transaction: Contradictory flags set.";
        return temINVALID_FLAG;
    }
...
SetAccount::doApply ()
{
    ...
    bool const bSetDisallowBMC   {(uTxFlags & tfDisallowBMC) || (uSetFlag == asfDisallowBMC)};
    bool const bClearDisallowBMC {(uTxFlags & tfAllowBMC) || (uClearFlag == asfDisallowBMC)};
    ...
    // DisallowBMC
    //
    if (bSetDisallowBMC && !(uFlagsIn & lsfDisallowBMC))
    {
        JLOG(j_.trace()) << "Set lsfDisallowBMC.";
        uFlagsOut |= lsfDisallowBMC;
    }

    if (bClearDisallowBMC && (uFlagsIn & lsfDisallowBMC))
    {
        JLOG(j_.trace()) << "Clear lsfDisallowBMC.";
        uFlagsOut &= ~lsfDisallowBMC;
    }
    ...
    // DefaultBlocmatrix
    //
    if (uSetFlag == asfDefaultBlocmatrix)
    {
        JLOG(j_.trace()) << "Set lsfDefaultBlocmatrix.";
        uFlagsOut |= lsfDefaultBlocmatrix;
    }
    else if (uClearFlag == asfDefaultBlocmatrix)
    {
        JLOG(j_.trace()) << "Clear lsfDefaultBlocmatrix.";
        uFlagsOut &= ~lsfDefaultBlocmatrix;
    }

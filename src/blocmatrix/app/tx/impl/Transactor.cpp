...
#include <blocmatrix/app/tx/impl/SignerEntries.h>
...
preflight1 (PreflightContext const& ctx)
{
    ...
    if (!fee.native () || fee.negative () || !isLegalAmount (fee.bmc ()))
    {
...
TER Transactor::payFee ()
{
    ...
    // VFALCO Should we call view().rawDestroyBMC() here as well?
...
TER Transactor::apply ()
{
    ...
    if (sle)
    {
        mPriorBalance   = STAmount ((*sle)[sfBalance]).bmc ();
...
BMCAmount
Transactor::reset(BMCAmount fee)
{
    ctx_.discard();
    ...
    auto const balance = txnAcct->getFieldAmount (sfBalance).bmc ();
...
Transactor::operator()()
{
    ...
    auto fee = ctx_.tx.getFieldAmount(sfFee).bmc ();

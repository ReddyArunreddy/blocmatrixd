...
#include <blocmatrix/app/tx/impl/PayChan.h>
#include <blocmatrix/basics/chrono.h>
#include <blocmatrix/basics/Log.h>
#include <blocmatrix/ledger/ApplyView.h>
...
/*
    PaymentChannel
        ...
        Payment channels permit off-ledger checkpoints of BMC payments flowing
        in a single direction. A channel sequesters the owner's BMC in its own
    ...
    PaymentChannelCreate
        ...
        Amount
            The amount of BMC to deposit in the channel immediately.
        SettleDelay
        ...
        DestinationTag (optional)
            ...
            Wallet to be mapped back onto the Blocmatrix ledger. The destination tag
        ...
        SourceTag (optional)
            ...
            to be mapped back onto the Blocmatrix ledger. Source tags are similar to
    ...
    PaymentChannelClaim
        ...
        Channel
            The 256-bit ID of the channel.
        Balance (optional)
            The total amount of BMC delivered after this claim is processed (optional, not
            needed if just closing).
        Amount (optional)
            The amount of BMC the signature is for (not needed if equal to Balance or just
            closing the line).
        Signature (optional)
...
PayChanCreate::preflight (PreflightContext const& ctx)
{
    ...
    if (!isBMC (ctx.tx[sfAmount]) || (ctx.tx[sfAmount] <= beast::zero))
...
PayChanCreate::preclaim(PreclaimContext const &ctx)
{
...

    {
        ...
        // Obeying the lsfDisallowBMC flag was a bug.  Piggyback on
        // featureDepositAuth to remove the bug.
        if (!ctx.view.rules().enabled(featureDepositAuth) &&
            ((*sled)[sfFlags] & lsfDisallowBMC))
            return tecNO_TARGET;
    }
...
//  bool const noTecs = ctx.rules.enabled(fix1512);
    ...
    if (bal && (!isBMC (*bal) || *bal <= beast::zero))
        return temBAD_AMOUNT;

    auto const amt = ctx.tx[~sfAmount];
    if (amt && (!isBMC (*amt) || *amt <= beast::zero))
        return temBAD_AMOUNT;
    ...
    if (auto const sig = ctx.tx[~sfSignature])
    {
        ...
        auto const reqBalance = bal->bmc ();
        auto const authAmt = amt ? amt->bmc() : reqBalance;
...
PayChanClaim::doApply()
{
    ...
    {
        auto const chanBalance = slep->getFieldAmount (sfBalance).bmc ();
        auto const chanFunds = slep->getFieldAmount (sfAmount).bmc ();
        auto const reqBalance = ctx_.tx[sfBalance].bmc ();
        ...
        if (!depositAuth &&
            (txAccount == src && (sled->getFlags() & lsfDisallowBMC)))
            return tecNO_TARGET;
        ...
        BMCAmount const reqDelta = reqBalance - chanBalance;

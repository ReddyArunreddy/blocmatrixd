...
#include <blocmatrix/app/tx/impl/Escrow.h>
...
#include <blocmatrix/conditions/Condition.h>
#include <blocmatrix/conditions/Fulfillment.h>
...
/*
    ...
    Escrow is a feature of the BMC Ledger that allows you to send conditional
    BMC payments. These conditional payments, called escrows, set aside BMC and
    ...
    The BMC set aside in an escrow is locked up. No one can use or destroy the
    BMC until the escrow has been successfully finished or canceled. Before the
    expiration time, only the intended receiver can get the BMC. After the
    expiration time, the BMC can only be returned to the sender.
...
EscrowCreate::preflight (PreflightContext const& ctx)
{
    ...
    {
        using namespace blocmatrix::cryptoconditions;
...
EscrowCreate::doApply()
{
    ...
    {
        auto const balance = STAmount((*sle)[sfBalance]).bmc();
        ...
        if (balance < reserve + STAmount(ctx_.tx[sfAmount]).bmc())
            return tecUNFUNDED;
    }
    ...
    // Check destination account
    {
        ...
        // Obeying the lsfDissalowBMC flag was a bug.  Piggyback on

...
static
BMCAmount accountFundsHelper (ReadView const& view,
    AccountID const& id,
    BMCAmount const& amtDefault,
    Issue const& issue,
    FreezeHandling freezeHandling,
    beast::Journal j)
{
    return toAmount<BMCAmount> (
        accountHolds (view, id, issue.currency, issue.account, freezeHandling, j));
}
...
template class FlowOfferStream<BMCAmount, IOUAmount>;
template class FlowOfferStream<IOUAmount, BMCAmount>;

template class TOfferStreamBase<STAmount, STAmount>;
template class TOfferStreamBase<IOUAmount, IOUAmount>;
template class TOfferStreamBase<BMCAmount, IOUAmount>;
template class TOfferStreamBase<IOUAmount, BMCAmount>;
}

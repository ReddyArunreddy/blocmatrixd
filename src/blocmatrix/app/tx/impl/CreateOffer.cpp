...
#include <blocmatrix/app/tx/impl/CreateOffer.h>
#include <blocmatrix/app/ledger/OrderBookDB.h>
#include <blocmatrix/app/paths/Flow.h>
#include <blocmatrix/ledger/CashDiff.h>
...
#include <blocmatrix/beast/utility/WrappedSink.h>
...
CreateOffer::preflight (PreflightContext const& ctx)
{
    ...
    {
        JLOG(j.debug()) <<
            "Malformed offer: redundant (BMC for BMC)";
        return temBAD_OFFER;
    }
    ...
    // We don't allow a non-native currency to use the currency code BMC.
...
CreateOffer::reachedOfferCrossingLimit (Taker const& taker) const
{
    ...
    // The crossing limit is part of the Blocmatrix protocol and
    ...
    NetClock::time_point const when)
{
    ...
    assert (!isBMC (takerAmount.in) && !isBMC (takerAmount.out));

    if (isBMC (takerAmount.in) || isBMC (takerAmount.out))
        Throw<std::logic_error> ("Bridging with BMC and an endpoint.");
    ...
    OfferStream offers_leg1 (view, view_cancel,
        Book (taker.issue_in (), bmcIssue ()),
        when, stepCounter_, j_);

    OfferStream offers_leg2 (view, view_cancel,
        Book (bmcIssue (), taker.issue_out ()),
        when, stepCounter_, j_);
    ...
    Amounts const& takerAmount)
{
    ...
    //
    // We check this in preclaim, but when selling BMC
    ...
    Amounts const& takerAmount)
{
    try
    {
        ...
        //
        // We check this in preclaim, but when selling BMC charged fees can
        ...
        // of the takerAmount currencies are BMC then we cross through an
        // additional path with BMC as the intermediate between two books.
...
static std::string to_string (SBoxCmp c)
{
    ...
    case SBoxCmp::bmcRound:
        return "BMC round to zero";
    case SBoxCmp::diff:
        return "different";
    }
    return {};
    ...
    beast::Journal j)
{
    ...
    if (diff.hasDiff())
    {
        ...
        // There is a special case of an offer with BMC on one side where
        // the BMC gets rounded to zero.  It mostly looks like dust-level
        // differences.  It is easier to detect if we look for it before
        // removing the dust differences.
        if (int const side = diff.bmcRoundToZero())
        {
            char const* const whichSide = side > 0 ? "; Flow" : "; Taker";
            j.stream (kWarning) << "FlowCross: " << name << " different" <<
                whichSide << " BMC rounded to zero.  tx: " <<
                ctx.tx.getTransactionID();
            return SBoxCmp::bmcRound;
        }
...
CreateOffer::preCompute()
{
    ...
    if (pays_bmc && !gets_bmc)
        cross_type_ = CrossType::IouToXrp;
    else if (gets_bmc && !pays_bmc)
...
CreateOffer::applyGuts (Sandbox& sb, Sandbox& sbCancel)
{
    ...
    {
        BMCAmount reserve = ctx_.view().fees().accountReserve(

...
#include <blocmatrix/app/paths/Pathfinder.h>
#include <blocmatrix/app/paths/BlocmatrixCalc.h>
#include <blocmatrix/app/paths/BlocmatrixLineCache.h>
#include <blocmatrix/ledger/PaymentSandbox.h>
#include <blocmatrix/app/ledger/OrderBookDB.h>
...
The pathfinding request is identified by category, BMC to BMC, BMC to
non-BMC, non-BMC to BMC, same currency non-BMC to non-BMC, cross-currency
non-BMC to non-BMC.  For each category, there is a table of paths that the
...
Pathfinder::Pathfinder (
    std::shared_ptr<BlocmatrixLineCache> const& cache,
    ...
    :   mSrcAccount (uSrcAccount),
        ...
        mEffectiveDst (isBMC(saDstAmount.getIssuer ()) ?
        ...
        mSrcAmount (srcAmount.value_or(STAmount({uSrcCurrency,
            uSrcIssuer.value_or(isBMC(uSrcCurrency) ?
                bmcAccount() : uSrcAccount)}, 1u, 0, true))),
...
{
    assert (! uSrcIssuer || isBMC(uSrcCurrency) == isBMC(uSrcIssuer.get()));
}
...
bool Pathfinder::findPaths (int searchLevel)
{
    ...
    auto currencyIsBMC = isBMC (mSrcCurrency);

    bool useIssuerAccount
            = mSrcIssuer && !currencyIsBMC && !isBMC (*mSrcIssuer);
    auto& account = useIssuerAccount ? *mSrcIssuer : mSrcAccount;
    auto issuer = currencyIsBMC ? AccountID() : account;
    ...
    bool bSrcXrp = isBMC (mSrcCurrency);
    bool bDstXrp = isBMC (mDstAmount.getCurrency());
    ...
    if (! mLedger->exists (keylet::account (mDstAccount)))
    {
        ...
        {
            JLOG (j_.debug())
                    << "New account not being funded in BMC ";
            return false;
        }
    ...
    {
        // BMC -> BMC
        JLOG (j_.debug()) << "BMC to BMC payment";
        paymentType = pt_BMC_to_BMC;
    }
    else if (bSrcXrp)
    {
        // BMC -> non-BMC
        JLOG (j_.debug()) << "BMC to non-BMC payment";
        paymentType = pt_BMC_to_nonBMC;
    }
    else if (bDstXrp)
    {
        // non-BMC -> BMC
        JLOG (j_.debug()) << "non-BMC to BMC payment";
        paymentType = pt_nonBMC_to_BMC;
    }
    else if (mSrcCurrency == mDstAmount.getCurrency ())
    {
        // non-BMC -> non-BMC - Same currency
        JLOG (j_.debug()) << "non-BMC to non-BMC - same currency";
        paymentType = pt_nonBMC_to_same;
    }
    else
    {
        // non-BMC to non-BMC - Different currency
        JLOG (j_.debug()) << "non-BMC to non-BMC - cross currency";
        paymentType = pt_nonBMC_to_nonBMC;
    }
    ...
    uint64_t& qualityOut) const    // OUT: The returned initial quality
{
    ...
    path::BlocmatrixCalc::Input rcInput;
    ...
    try
    {
        ...
        auto rc = path::BlocmatrixCalc::blocmatrixCalculate (
            sandbox,
            mSrcAmount,
            minDstAmount,
            mDstAccount,
        ...
        if (! convert_all_)
        {
            ...
            rcInput.partialPaymentAllowed = true;
            rc = path::BlocmatrixCalc::blocmatrixCalculate (
                sandbox,
                mSrcAmount,
    ...
    AccountID const& srcIssuer)
{
    ...
    const bool issuerIsSender = isBMC (mSrcCurrency) || (srcIssuer == mSrcAccount);
    ...
    AccountID const& dstAccount)
{
    ...
    if (!bFrozen)
    {
        ...
        for (auto const& item : mRLCache->getBlocmatrixLines (account))
        {
            BlocmatrixState* rspEntry = (BlocmatrixState*) item.get ();
            ...
            else if (rspEntry->getNoBlocmatrixPeer ())
            {
...
STPathSet& Pathfinder::addPathsForType (PathType const& pathType)
{
    ...
    case nt_BMC_BOOK:
        addLinks (parentPaths, pathsOut, afADD_BOOKS | afOB_BMC);
        break;
...
bool Pathfinder::isNoBlocmatrix (
    AccountID const& fromAccount,
    AccountID const& toAccount,
    Currency const& currency)
{
    auto sleBlocmatrix = mLedger->read(keylet::line(
        toAccount, fromAccount, currency));

    auto const flag ((toAccount > fromAccount)
                     ? lsfHighNoBlocmatrix : lsfLowNoBlocmatrix);

    return sleBlocmatrix && (sleBlocmatrix->getFieldU32 (sfFlags) & flag);
}

// Does this path end on an account-to-account link whose last account has
// set "no blocmatrix" on the link?
bool Pathfinder::isNoBlocmatrixOut (STPath const& currentPath)
{
    ...
    // no blocmatrix on the output. A path with no blocmatrix on its output can't be
    // followed by a link with no blocmatrix on its input.
    auto const& fromAccount = (currentPath.size () == 1)
        ? mSrcAccount
        : (currentPath.end () - 2)->getAccountID ();
    auto const& toAccount = endElement.getAccountID ();
    return isNoBlocmatrix (fromAccount, toAccount, endElement.getCurrency ());
}
    ...
    int addFlags)
{
    ...
    bool const bOnBMC = uEndCurrency.isZero ();
    ...
    if (addFlags & afADD_ACCOUNTS)
    {
        ...
        if (bOnBMC)
        {
            ...
            { // non-default path to BMC destination
                JLOG (j_.trace())
        ...
        else
        {
            ...
            if (sleEnd)
            {
                ...
                bool const bIsNoBlocmatrixOut (
                    isNoBlocmatrixOut (currentPath));
                bool const bDestOnly (
                    addFlags & afAC_LAST);

                auto& blocmatrixLines (mRLCache->getBlocmatrixLines (uEndAccount));

                AccountCandidates candidates;
                candidates.reserve (blocmatrixLines.size ());

                for (auto const& item : blocmatrixLines)
                {
                    auto* rs = dynamic_cast<BlocmatrixState const *> (item.get ());
                    if (!rs)
                    {
                        JLOG (j_.error())
                                << "Couldn't decipher BlocmatrixState";
                        continue;
                    }
                        ...
                        !currentPath.hasSeen (acct, uEndCurrency, acct))
                    {
                        ...
                        else if (bIsNoBlocmatrixOut && rs->getNoBlocmatrix ())
                        {
    ...
    {
        // add order books
        if (addFlags & afOB_BMC)
        {
            // to BMC only
            if (!bOnBMC && app_.getOrderBookDB ().isBookToBMC (
            ...
            for (auto const& book : books)
            {
                     ...
                     (book->getCurrencyOut () == mDstAmount.getCurrency ())))
                {
                    ...
                    { // to BMC
                        ...
                        {
                            // destination is BMC, add account and path is
                            // complete
                            JLOG (j_.trace())
                        ...
                        book->getIssuerOut ()))
                    {
                            ...
                            (newPath[newPath.size() - 2].isOffer ()))
                        {
                            ...
                            newPath[newPath.size() - 1] = STPathElement (
                                ...
                                bmcAccount(), book->getCurrencyOut(),
                                book->getIssuerOut());
                        }
                        else
                        {
...
Pathfinder::PathType makePath (char const *string)
{
    ...
    while (true)
    {
        switch (*string++)
        {
            ...
            case 'x': // bmc book
                ret.push_back (Pathfinder::nt_BMC_BOOK);
                break;
...
void Pathfinder::initPathTable ()
{
    ...
    fillPaths(
        pt_nonBMC_to_BMC, {
            {1, "sxd"},       // gateway buys BMC
            {2, "saxd"},      // source -> gateway -> book(BMC) -> dest
            {6, "saaxd"},
            {7, "sbxd"},
            {8, "sabxd"},
            {9, "sabaxd"}
    ...
    // non-BMC to non-BMC (same currency)
    fillPaths(
        pt_nonBMC_to_same,  {
            ...
            {6, "sxfad"},
            {6, "safad"},
            {6, "saxfd"},   // source -> gateway -> book to BMC -> book ->
                            // destination
            {6, "saxfad"},

...
#ifndef BLOCMATRIX_APP_PATHS_BLOCMATRIXSTATE_H_INCLUDED
#define BLOCMATRIX_APP_PATHS_BLOCMATRIXSTATE_H_INCLUDED
...
class BlocmatrixState
{
public:
    // VFALCO Why is this shared_ptr?
    using pointer = std::shared_ptr <BlocmatrixState>;

public:
    BlocmatrixState () = delete;

    virtual ~BlocmatrixState() = default;

    static BlocmatrixState::pointer makeItem(
        AccountID const& accountID,
        std::shared_ptr<SLE const> sle);

    // Must be public, for make_shared
    BlocmatrixState (std::shared_ptr<SLE const>&& sle,
    ...
    bool getNoBlocmatrix () const
    {
        return mFlags & (mViewLowest ? lsfLowNoBlocmatrix : lsfHighNoBlocmatrix);
    }

    bool getNoBlocmatrixPeer () const
    {
        return mFlags & (!mViewLowest ? lsfLowNoBlocmatrix : lsfHighNoBlocmatrix);
    }
...
getBlocmatrixStateItems (AccountID const& accountID,
    ReadView const& view);

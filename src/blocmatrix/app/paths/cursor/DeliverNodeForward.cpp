...
#include <blocmatrix/app/paths/cursor/EffectiveRate.h>
    ...
    bool callerHasLiquidity) const
{
    ...
    while (resultCode == tesSUCCESS && saInAct + saInFees < saInReq)
    {
        ...
        else if (resultCode == tesSUCCESS)
        {
            ...
            if (!isBMC(nextNode().account_))
            {
                // ? --> OFFER --> account
                // Input fees: vary based upon the consumed offer's owner.
                // Output fees: none as BMC or the destination account is the
                // issuer.
                ...
                // Output: Debit offer owner, send BMC or non-XPR to next
                // account.
            ...
            else
            {
                ...
                // fees).
                auto const& id = isBMC(node().issue_) ?
                        bmcAccount() : node().issue_.account;
            ...
            // is redundant.
            if (isBMC (previousNode().issue_.currency)
                || uInAccountID != node().offerOwnerAccount_)
            {
                auto id = !isBMC(previousNode().issue_.currency) ?
                        uInAccountID : bmcAccount();
                resultCode = accountSend(view(),
                    id,
                    node().offerOwnerAccount_,
                    saInPassAct,

...
#include <blocmatrix/app/paths/cursor/BlocmatrixLiquidity.h>
...
// - If next node is an offer and output is non-BMC then we are the issuer and
//   do not need to push funds.
// - If next node is an offer and output is BMC then we need to deliver funds to
//   limbo.
TER PathCursor::forwardLiquidityForAccount () const
{
    TER resultCode   = tesSUCCESS;
    auto const lastNodeIndex       = pathState_.nodes().size () - 1;
    auto viewJ = blocmatrixCalc_.logs_.journal ("View");
    ...
    if (previousNode().isAccount() && nextNode().isAccount())
    {
        ...
        if (!nodeIndex_)
        {
            ...
            // For the first node, calculate amount to blocmatrix based on what is
            // available.
        ...
        else if (nodeIndex_ == lastNodeIndex)
        {
            ...
            {
                // Actually receive.
                resultCode = blocmatrixCredit(view(),
                    previousAccountID,
                    node().account_,
    ...
    else if (previousNode().isAccount() && !nextNode().isAccount())
    {
        ...
        if (nodeIndex_)
        {
            // Non-BMC, current node is the issuer.
            JLOG (j_.trace())
                << "forwardLiquidityForAccount: account --> "
                << "ACCOUNT --> offer";
        ...
        else
        {
            ...
            if (pathState_.inReq() >= beast::zero)
            {
                ...
                // Limit BMC by available. No limit for non-BMC as issuer.
                if (isBMC (node().issue_))
                    node().saFwdDeliver = std::min (
                        node().saFwdDeliver,
                        accountHolds(view(),
                            node().account_,
                            bmcCurrency(),
                            bmcAccount(),
                            fhIGNORE_FREEZE, viewJ)); // BMC can't be frozen
            ...
            else if (!isBMC (node().issue_))
            {
                // Non-BMC, current node is the issuer.
                // We could be delivering to multiple accounts, so we don't know
                // which blocmatrix balance will be adjusted.  Assume just issuing.
            ...
            else
            {
                ...
                // Deliver BMC to limbo.
                resultCode = accountSend(view(),
                    node().account_, bmcAccount(), node().saFwdDeliver, viewJ);
            }
        }
    }
    else if (!previousNode().isAccount() && nextNode().isAccount())

...
TER PathCursor::advanceNode (STAmount const& amount, bool reverse, bool callerHasLiquidity) const
{
    ...
    PathCursor withMultiQuality {blocmatrixCalc_, pathState_, multi, j_, nodeIndex_};
        ...
        else
        {
            ...
            else
            {
                            ...
                            view().parentCloseTime().time_since_epoch().count()))
                {
                    ...
                    blocmatrixCalc_.permanentlyUnfundedOffers_.insert(
                        node().offerIndex_);
                    continue;
                }
                ...
                if (node().saTakerPays <= beast::zero || node().saTakerGets <= beast::zero)
                {
                    ...
                    else if (blocmatrixCalc_.permanentlyUnfundedOffers_.find (index)
                             != blocmatrixCalc_.permanentlyUnfundedOffers_.end ())
                    {
                ...
                auto itPast = blocmatrixCalc_.mumSource_.find (accountIssue);
                bool bFoundPast = (itPast != blocmatrixCalc_.mumSource_.end ());
                ...
                if (node().saOfferFunds <= beast::zero)
                {
                    ...
                    if (bReverse && !bFoundReverse && !bFoundPast)
                    {
                        ...
                        // still do deletions.
                        // Mark offer for always deletion.
                        blocmatrixCalc_.permanentlyUnfundedOffers_.insert (node().offerIndex_);
                    }
                    else
                    {

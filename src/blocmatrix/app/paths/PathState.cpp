...
#include <blocmatrix/app/paths/PathState.h>
...
class BlocmatrixCalc; // for logging
...
// - A blocmatrix nodes output issuer must be the node's account or the next node's
//   account.
// - Offers can only go directly to another offer if the currency and issuer are
//   an exact match.
// - Real issuers must be specified for non-BMC.
    ...
    AccountID const& issuer)    // --> Delivering this issuer.
{
    ...
    if (nodes_.back ().issue_.currency != currency)
    {
        ...
        // book.  bmcAccount() does double duty as signaling "this is an order
        // book".

        // Corresponds to "Implies an offer directory" in the diagram, currently
        // at http://goo.gl/Uj3HAB.

        auto type = isBMC(currency) ? STPathElement::typeCurrency
            : STPathElement::typeCurrency | STPathElement::typeIssuer;

        // The offer's output is what is now wanted.
        // bmcAccount() is a placeholder for offers.
        resultCode = pushNode (type, bmcAccount(), currency, issuer);
    }


    // For blocmatrix, non-BMC, ensure the issuer is on at least one side of the
    // transaction.
    ...
    {
        // Need to blocmatrix through issuer's account.
    ...
    AccountID const& issuer)     // If not specified, default to previous.
{
    ...
    // true, iff node is a blocmatrix account. false, iff node is an offer node.
    ...
    else if (hasIssuer && isBMC (node.issue_))
    {
        JLOG (j_.debug()) << "pushNode: issuer specified for BMC.";
    ...
    else if (hasAccount)
    {
        ...
        node.issue_.account = hasIssuer ? issuer :
                (isBMC (node.issue_) ? bmcAccount() : account);
        ...
        else
        {
            ...
            resultCode = pushImpliedNodes (
                ...
                isBMC(node.issue_.currency) ? bmcAccount() : account);
        ...
        if (resultCode == tesSUCCESS && !nodes_.empty ())
        {
            ...
            {
                auto sleBlocmatrixState = view().peek(
                    keylet::line(backNode.account_, node.account_, backNode.issue_.currency));

                // A "BlocmatrixState" means a balance betweeen two accounts for a
                // specific currency.
                ...
                else
                {
                    ...
                    if (!sleBck)
                    {
                        ...
                        resultCode   = terNO_ACCOUNT;
                             ...
                             !(sleBlocmatrixState->getFieldU32 (sfFlags) &
                                  (bHigh ? lsfHighAuth : lsfLowAuth)) &&
                             sleBlocmatrixState->getFieldAmount(sfBalance) == beast::zero)
                    {
                        JLOG (j_.warn())
                                << "pushNode: delay: can't receive IOUs from "
                                << "issuer without auth.";
    ...
    else
    {
        ...
        else if (isBMC (node.issue_.currency))
            node.issue_.account = bmcAccount();
        else if (isBMC (backNode.issue_.account))
        ...
        else if (backNode.issue_ == node.issue_)
        {
            ...
            resultCode   = pushImpliedNodes (
                bmcAccount(), // Rippling, but offers don't have an account.
    ...
    AccountID const& uSenderID)
{
    ...
    AccountID const& uSenderIssuerID
        = isBMC(uMaxCurrencyID) ? bmcAccount() : uSenderID;
    // Sender is always issuer for non-BMC.
    ...
    // BMC with issuer is malformed.
    if ((isBMC (uMaxCurrencyID) && !isBMC (uMaxIssuerID))
        || (isBMC (currencyOutID) && !isBMC (issuerOutID)))
    {
        JLOG (j_.debug())
            << "expandPath> issuer with BMC";
        terStatus   = temBAD_PATH;
    }

    // Push sending node.
    // For non-BMC, issuer is always sending account.
    // - Trying to expand, not-compact.
    // - Every issuer will be traversed through.
    if (terStatus == tesSUCCESS)
    {
        terStatus   = pushNode (
            !isBMC(uMaxCurrencyID)
    ...
    if (tesSUCCESS == terStatus && uMaxIssuerID != uSenderIssuerID)
    {
        ...
        // - If it was BMC, then issuers would have matched.
        ...
        // account.
        if (!uNxtCurrencyID
            // Next is BMC, offer next. Must go through issuer.
    ...
    if (terStatus == tesSUCCESS
        && !isBMC(currencyOutID)               // Next is not BMC
...
/** Check if a sequence of three accounts violates the no blocmatrix constrains
    [first] -> [second] -> [third]
    Disallowed if 'second' set no blocmatrix on [first]->[second] and
    [second]->[third]
*/
TER PathState::checkNoBlocmatrix (
    ...
    Currency const& currency)
{
    ...
    if (!sleIn || !sleOut)
    {
        ...
        sleIn->getFieldU32 (sfFlags) &
            ((secondAccount > firstAccount) ? lsfHighNoBlocmatrix : lsfLowNoBlocmatrix) &&
        sleOut->getFieldU32 (sfFlags) &
            ((secondAccount > thirdAccount) ? lsfHighNoBlocmatrix : lsfLowNoBlocmatrix))
    {
        JLOG (j_.info())
            << "Path violates noBlocmatrix constraint between "
            << firstAccount << ", "
            << secondAccount << " and "
            << thirdAccount;

        terStatus = terNO_BLOCMATRIX;
    }
    return terStatus;
}

// Check a fully-expanded path to make sure it doesn't violate no-Blocmatrix
// settings.
TER PathState::checkNoBlocmatrix (
    AccountID const& uDstAccountID,
    AccountID const& uSrcAccountID)
{
    // There must be at least one node for there to be two consecutive blocmatrix
    // lines.
    if (nodes_.size() == 0)
       return terStatus;
    ...
    // These are the nodes whose no blocmatrix constraints could be violated

 ...
 * By using BMC for bridging the most complicated path expected is usually:
 * source -> gateway -> BMC -> gateway -> destination
3. Pareto principle.
 * Good liquidity is often reusable.
...
When someone accepts payment they list their specific payment terms, "what must happen before the payment goes off."  That can be done completely in the Blocmatrix Network.  Normally a payment on the Blocmatrix net can be completely settled there.  When the ledger closes, the terms are met, or they are not met.
...
In contrast, a gateway is in the business of redeeming value on the Blocmatrix Network for value off the Ripnet.  A gateway is in the business of issuing balances.
...
Suppose I'm on the Blocmatrix network and want to send value to the bitcoin network.  See:  https://ripple.com/wiki/Outbound_Bridge_Payments
...
If you're sending and receiving BMC you don't need a path.
...
1. Finding liquidity - "path finding"
  ...
  * filtering by liquidity.
    ...
    * This routine is called BlocmatrixCalc::rippleCalc.
...
2. Build a payment transaction containing the path set.
  ...
  * A payment transaction includes the following fields
    ...
    * destination amount
      ...
      * if currency not BMC, must have a specified issuer.
 ...
 * The routine used to compute liquidity and ledger change is also called BlocmatrixCalc::rippleCalc.

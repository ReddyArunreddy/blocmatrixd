...
BlocmatrixState::pointer
BlocmatrixState::makeItem (
    AccountID const& accountID,
        std::shared_ptr<SLE const> sle)
{
    // VFALCO Does this ever happen in practice?
    if (! sle || sle->getType () != ltBLOCMATRIX_STATE)
        return {};
    return std::make_shared<BlocmatrixState>(
        std::move(sle), accountID);
}

BlocmatrixState::BlocmatrixState (
    std::shared_ptr<SLE const>&& sle,
...
Json::Value BlocmatrixState::getJson (int)
{
    ...
    ReadView const& view)
{
    ...
    forEachItem(view, accountID,
        ...
        {
             auto ret = BlocmatrixState::makeItem (accountID, sleCur);
             if (ret)
                items.push_back (ret);
        });

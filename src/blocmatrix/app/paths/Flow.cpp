...
#include <blocmatrix/app/paths/impl/StrandFlow.h>
...
auto finishFlow (PaymentSandbox& sb,
    ...
    FlowResult&& f)
{
    path::BlocmatrixCalc::Output result;
    if (f.ter == tesSUCCESS)
        f.sandbox->apply (sb);
    else
    ...
    path::detail::FlowDebugInfo* flowDebugInfo)
{
    Issue const srcIssue = [&] {
        ...
        if (!isBMC (deliver.issue ().currency))
    ...
    const bool srcIsBMC = isBMC (srcIssue.currency);
    const bool dstIsBMC = isBMC (dstIssue.currency);

    auto const asDeliver = toAmountSpec (deliver);

    // The src account may send either bmc or iou. The dst account may receive
    // either bmc or iou. Since BMC and IOU amounts are represented by different
    // types, use templates to tell `flow` about the amount types.
    if (srcIsBMC && dstIsBMC)
    {
        return finishFlow (sb, srcIssue, dstIssue,
            flow<BMCAmount, BMCAmount> (
                sb, strands, asDeliver.bmc, partialPayment, offerCrossing,
    ...
    {
        return finishFlow (sb, srcIssue, dstIssue,
            flow<IOUAmount, BMCAmount> (
                sb, strands, asDeliver.bmc, partialPayment, offerCrossing,
                limitQuality, sendMax, j, flowDebugInfo));
    }

    assert (!srcIsBMC && !dstIsBMC);
    return finishFlow (sb, srcIssue, dstIssue,

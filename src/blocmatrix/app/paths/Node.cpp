...
Json::Value Node::getJson () const
{
    ...
    bool const hasCurrency = !isBMC (issue_.currency);
    bool const hasAccount = !isBMC (account_);
    bool const hasIssuer = !isBMC (issue_.account);
    ...
    if (!isBMC (issue_.currency))
        jvNode[jss::currency] = to_string (issue_.currency);

    if (!isBMC (issue_.account))

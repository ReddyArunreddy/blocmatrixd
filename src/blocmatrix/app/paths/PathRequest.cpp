...
#include <blocmatrix/app/paths/PathRequest.h>
...
bool PathRequest::isValid (std::shared_ptr<BlocmatrixLineCache> const& crCache)
{
    ...
    if (! sleDest)
    {
        ...
        {
            // Only BMC can be send to a non-existent account.
    ...
    {
        bool const disallowBMC (
            sleDest->getFlags() & lsfDisallowBMC);

        auto usDestCurrID = accountDestCurrencies (
                *raDstAccount, crCache, ! disallowBMC);
...
PathRequest::getPathFinder(std::shared_ptr<BlocmatrixLineCache> const& cache,
...
bool
PathRequest::findPaths (std::shared_ptr<BlocmatrixLineCache> const& cache,
    int const level, Json::Value& jvArray)
{
        ...
        for (auto const& c : currencies)
        {
            if (! sameAccount || c != saDstAmount.getCurrency())
            {
                ...
                sourceCurrencies.insert(
                    {c, c.isZero() ? bmcAccount() : *raSrcAccount});
            }
        }
    }
    ...
    for (auto const& issue : sourceCurrencies)
    {
        ...
        auto& sourceAccount = ! isBMC(issue.account)
            ? issue.account
        ...
        JLOG(m_journal.debug()) << iIdentifier
            << " Paths found, calling blocmatrixCalc";
        ...
        if (rc.result () == tesSUCCESS)
        {
            ...
            if (hasCompletion ())
            {
                // Old blocmatrix_path_find API requires this
        ...
        {
            JLOG(m_journal.debug()) << iIdentifier << " blocmatrixCalc returns "
                << transHuman(rc.result());
        }
    }
...
Json::Value PathRequest::doUpdate(
    std::shared_ptr<BlocmatrixLineCache> const& cache, bool fast)
{
    using namespace std::chrono;
    JLOG(m_journal.debug()) << iIdentifier
    ...
    if (hasCompletion ())
    {
        // Old blocmatrix_path_find API gives destination_currencies

...
#include <blocmatrix/json/json_writer.h>
...
bool checkNear (BMCAmount const& expected, BMCAmount const& actual)
{
    return expected == actual;
};

static
bool isBMCAccount (STPathElement const& pe)
{
    if (pe.getNodeType () != STPathElement::typeAccount)
        return false;
    return isBMC (pe.getAccountID ());
};
    ...
    Issue const& curIssue)
{
    ...
    if (ctx.isFirst && e1->isAccount () &&
        ...
        isBMC (e1->getCurrency ()))
    {
        return make_BMCEndpointStep (ctx, e1->getAccountID ());
    }

    if (ctx.isLast && isBMCAccount (*e1) && e2->isAccount())
        return make_BMCEndpointStep (ctx, e2->getAccountID());
    ...
    if (isBMC (curIssue.currency) && isBMC (outCurrency))
    {
        JLOG (j.warn()) << "Found bmc/bmc offer payment step";
        return {temBAD_PATH, std::unique_ptr<Step>{}};
    }

    assert (e2->isOffer ());

    if (isBMC (outCurrency))
        return make_BookStepIX (ctx, curIssue);

    if (isBMC (curIssue.currency))
    ...
    beast::Journal j)
{
    if (isBMC (src))
    {
        JLOG (j.debug()) << "toStrand with bmcAccount as src";
        return {temBAD_PATH, Strand{}};
    }
    if (isBMC (dst))
    {
        JLOG (j.debug()) << "toStrand with bmcAccount as dst";
        return {temBAD_PATH, Strand{}};
    }
    ...
    beast::Journal j)
{
    ...
    for (auto const& pe : path)
    {
        ...
        if (hasIssuer && isBMC(pe.getIssuerID()))
            return {temBAD_PATH, Strand{}};

        if (hasAccount && isBMC(pe.getAccountID()))
            return {temBAD_PATH, Strand{}};

        if (hasCurrency && hasIssuer &&
            isBMC(pe.getCurrency()) != isBMC(pe.getIssuerID()))
            return {temBAD_PATH, Strand{}};
    ...
    */
        ...
        */
        ...
        if (cur->isAccount() && next->isAccount())
        {
                ...
                curIssue.account != next->getAccountID ())
            {
                ...
                result.push_back (std::move (msr.second));
                impliedPE.emplace(STPathElement::typeAccount,
                    curIssue.account, bmcCurrency(), bmcAccount());
                cur = &*impliedPE;
            }
        }
        else if (cur->isAccount() && next->isOffer())
        ...
        else if (cur->isOffer() && next->isAccount())
        {
                ...
                !isBMC (next->getAccountID ()))
            {
                if (isBMC(curIssue))
                {
                    ...
                    {
                        // Last step. insert bmc endpoint step
                        auto msr = make_BMCEndpointStep (ctx(), next->getAccountID());
                        if (msr.first != tesSUCCESS)
                            return {msr.first, Strand{}};
                        result.push_back(std::move(msr.second));
                    }
    ...
    auto checkStrand = [&]() -> bool {
        auto stepAccts = [](Step const& s) -> std::pair<AccountID, AccountID> {
            ...
            return std::make_pair(bmcAccount(), bmcAccount());
        };
    ...
    beast::Journal j)
{
    ...
    {
        JLOG (j.debug())
            << "Flow: Invalid transaction: No paths and direct blocmatrix not allowed.";
        return {temBLOCMATRIX_EMPTY, std::vector<Strand>{}};
    }
...
template<>
bool
isDirectXrpToXrp<BMCAmount, BMCAmount> (Strand const& strand)
{
...
template
bool
isDirectXrpToXrp<BMCAmount, IOUAmount> (Strand const& strand);
template
bool
isDirectXrpToXrp<IOUAmount, BMCAmount> (Strand const& strand);
template
bool

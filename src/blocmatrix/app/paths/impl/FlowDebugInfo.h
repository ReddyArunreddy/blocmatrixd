...
#ifndef BLOCMATRIX_PATH_IMPL_FLOWDEBUGINFO_H_INCLUDED
#define BLOCMATRIX_PATH_IMPL_FLOWDEBUGINFO_H_INCLUDED
        ...
        if (writePassInfo)
        {
            ...
            auto writeXrpAmtList = [&write_list](
                ...
                auto get_val = [](EitherAmount const& a) -> std::string {
                    return blocmatrix::to_string (a.bmc);
                };
            ...
            auto writeIouAmtList = [&write_list](
                std::vector<EitherAmount> const& amts, char delim=';') {
                auto get_val = [](EitherAmount const& a) -> std::string {
                    return blocmatrix::to_string (a.iou);
                };
                write_list (amts, get_val, delim);
            };
...
{
    return {sb.balanceChanges (rv), sb.bmcDestroyed ()};
}
...
balanceDiffsToString (boost::optional<BalanceDiffs> const& bd)
{
    ...
    auto const& bmcDestroyed = bd->second;
    std::ostringstream ostr;
    ostr << ", bmcDestroyed: " << to_string (bmcDestroyed);
    ostr << ", balanceDiffs: ";

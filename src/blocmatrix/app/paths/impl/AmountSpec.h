...
#ifndef BLOCMATRIX_PATH_IMPL_AMOUNTSPEC_H_INCLUDED
#define BLOCMATRIX_PATH_IMPL_AMOUNTSPEC_H_INCLUDED
...
#endif
    explicit
    EitherAmount (BMCAmount const& a)
            :bmc(a)
    {
#ifndef NDEBUG
        native = true;
...
inline
BMCAmount&
get<BMCAmount> (EitherAmount& amt)
{
    assert (amt.native);
    return amt.bmc;
}
...
inline
BMCAmount const&
get<BMCAmount> (EitherAmount const& amt)
{
    assert (amt.native);
    return amt.bmc;
}
...
toAmountSpec (STAmount const& amt)
{
    ...
    result.native = isBMC (amt);
    if (result.native)
    {
        result.bmc = BMCAmount (sMant);
    }
    else
    {

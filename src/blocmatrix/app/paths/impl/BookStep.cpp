...
#include <blocmatrix/app/paths/impl/FlatSets.h>
#include <blocmatrix/app/paths/impl/Steps.h>
#include <blocmatrix/app/paths/Credit.h>
#include <blocmatrix/app/paths/NodeDirectory.h>
#include <blocmatrix/app/tx/impl/OfferStream.h>
...
#include <blocmatrix/protocol/Book.h>
...
public:
        ...
        bool prevStepRedeems) const
    {
        ...
        auto rate = [&](AccountID const& id) {
            if (isBMC(id) || id == this->strandDst_)
                return parityRate;
    ...
    Callback& callback) const
{
    ...
    while (offers.step ())
    {
        ...
        // An account can always own BMC or their own IOUs.
        if (flowCross &&
            (!isBMC (offer.issueIn().currency)) &&
...
template <class TIn, class TOut, class TDerived>
static
bool equalHelper (Step const& step, blocmatrix::Book const& book)
{
...
bool bookStepEqual (Step const& step, blocmatrix::Book const& book)
{
    bool const inBMC = isBMC (book.in.currency);
    bool const outBMC = isBMC (book.out.currency);
    if (inBMC && outBMC)
        return equalHelper<BMCAmount, BMCAmount,
            BookPaymentStep<BMCAmount, BMCAmount>> (step, book);
    if (inBMC && !outBMC)
        return equalHelper<BMCAmount, IOUAmount,
            BookPaymentStep<BMCAmount, IOUAmount>> (step, book);
    if (!inBMC && outBMC)
        return equalHelper<IOUAmount, BMCAmount,
            BookPaymentStep<IOUAmount, BMCAmount>> (step, book);
    if (!inBMC && !outBMC)
...
make_BookStepIX (
    ...
    Issue const& in)
{
    return make_BookStepHelper<IOUAmount, BMCAmount> (ctx, in, bmcIssue());
}
...
make_BookStepXI (
    ...
    Issue const& out)
{
    return make_BookStepHelper<BMCAmount, IOUAmount> (ctx, bmcIssue(), out);
}

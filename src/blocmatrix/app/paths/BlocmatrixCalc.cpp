...
/*
    ...
    Copyright (c) 2012, 2013 Blocmatrix Labs Inc.
...
#include <blocmatrix/app/paths/impl/FlowDebugInfo.h>
...
BlocmatrixCalc::Output BlocmatrixCalc::blocmatrixCalculate (
    PaymentSandbox& view,
    ...
    // Issuer:
    //      BMC: bmcAccount()
    //  non-BMC: uSrcAccountID (for any issuer) or another account with
    //           trust node.
    STAmount const& saMaxAmountReq,             // --> -1 = no limit.

    // Issuer:
    //      BMC: bmcAccount()
    //  non-BMC: uDstAccountID (for any issuer) or another account with
    //           trust node.
    ...
    Input const* const pInputs)
{
    ...
    if (callFlowV1)
    {
        ...
        auto result = rc.blocmatrixCalculate (compareFlowV1V2 ? &flowV1FlowDebugInfo : nullptr);
    ...
    if (callFlowV2)
    {
        ...
        catch (std::exception& e)
        {
            ...
            if (!useFlowV1Output)
            {
                ...
                path::BlocmatrixCalc::Output exceptResult;
...
bool BlocmatrixCalc::addPathState(STPath const& path, TER& resultCode)
{
    ...
    if (pathState->status() == tesSUCCESS)
        pathState->checkNoBlocmatrix (uDstAccountID_, uSrcAccountID_);
...
TER BlocmatrixCalc::blocmatrixCalculate (detail::FlowDebugInfo* flowDebugInfo)
{
    JLOG (j_.trace())
        << "blocmatrixCalc>"
    ...
    {
        JLOG (j_.debug())
            << "blocmatrixCalc: Invalid transaction:"
            << "No paths and direct blocmatrix not allowed.";

        return temBLOCMATRIX_EMPTY;
    }

    // Build a default path.  Use saDstAmountReq_ and saMaxAmountReq_ to imply
    // nodes.
    // XXX Might also make a BMC bridge by default.

    JLOG (j_.trace())
        << "blocmatrixCalc: Paths in set: " << spsPaths_.size ();
        ...
        for (auto pathState : pathStateList_)
        {
                ...
                // Only do active paths.
            {
                ...
                else if (pathState->outPass() == beast::zero)
                {
                    ...
                    JLOG (j_.warn())
                        << "blocmatrixCalc: Non-dry path moves no funds";

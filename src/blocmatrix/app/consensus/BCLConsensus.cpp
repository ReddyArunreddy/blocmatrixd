...
#include <blocmatrix/app/consensus/BCLConsensus.h>
#include <blocmatrix/app/consensus/BCLValidations.h>
#include <blocmatrix/app/ledger/BuildLedger.h>
#include <blocmatrix/app/ledger/InboundLedgers.h>
#include <blocmatrix/app/ledger/InboundTransactions.h>
#include <blocmatrix/app/ledger/LedgerMaster.h>
#include <blocmatrix/app/ledger/LocalTxs.h>
#include <blocmatrix/app/ledger/OpenLedger.h>
#include <blocmatrix/app/misc/AmendmentTable.h>
#include <blocmatrix/app/misc/HashRouter.h>
#include <blocmatrix/app/misc/LoadFeeTrack.h>
#include <blocmatrix/app/misc/NetworkOPs.h>
#include <blocmatrix/app/misc/TxQ.h>
#include <blocmatrix/app/misc/ValidatorKeys.h>
#include <blocmatrix/app/misc/ValidatorList.h>
#include <blocmatrix/basics/make_lock.h>
#include <blocmatrix/beast/core/LexicalCast.h>
#include <blocmatrix/consensus/LedgerTiming.h>
#include <blocmatrix/nodestore/DatabaseShard.h>
#include <blocmatrix/overlay/Overlay.h>
#include <blocmatrix/overlay/predicates.h>
...
boost::optional<BCLCxLedger>
BCLConsensus::Adaptor::acquireLedger(LedgerHash const& hash)
{
    ...
    return BCLCxLedger(built);
}

void
BCLConsensus::Adaptor::share(BCLCxPeerPos const& peerPos)
{
...
void
BCLConsensus::Adaptor::share(BCLCxTx const& tx)
{
...
void
BCLConsensus::Adaptor::propose(BCLCxPeerPos::Proposal const& proposal)
{
    JLOG(j_.trace()) << "We propose: "
                     << (proposal.isBowOut()
                             ? std::string("bowOut")
                             : blocmatrix::to_string(proposal.position()));
...
void
BCLConsensus::Adaptor::share(BCLTxSet const& txns)
{
    inboundTransactions_.giveSet(txns.id(), txns.map_, false);
}

boost::optional<BCLTxSet>
BCLConsensus::Adaptor::acquireTxSet(BCLTxSet::ID const& setId)
{
    if (auto txns = inboundTransactions_.getSet(setId, true))
    {
        return BCLTxSet{std::move(txns)};
    }
    return boost::none;
}

bool
BCLConsensus::Adaptor::hasOpenTransactions() const
{
    return !app_.openLedger().empty();
}

std::size_t
BCLConsensus::Adaptor::proposersValidated(LedgerHash const& h) const
{
    return app_.getValidations().numTrustedForLedger(h);
}

std::size_t
BCLConsensus::Adaptor::proposersFinished(
    BCLCxLedger const& ledger,
    LedgerHash const& h) const
{
    BCLValidations& vals = app_.getValidations();
    return vals.getNodesAfter(
        BCLValidatedLedger(ledger.ledger_, vals.adaptor().journal()), h);
}

uint256
BCLConsensus::Adaptor::getPrevLedger(
    uint256 ledgerID,
    BCLCxLedger const& ledger,
    ConsensusMode mode)
{
    ...
    uint256 netLgr = vals.getPreferred(
        BCLValidatedLedger{ledger.ledger_, vals.adaptor().journal()},
    ...
    ConsensusMode mode) -> Result
{
    ...
    if (!wrongLCL)
    {
        ...
        BCLCensorshipDetector<TxID, LedgerIndex>::TxIDSeqVec proposed;
    ...
    return Result{
        std::move(initialSet),
        BCLCxPeerPos::Proposal{
            initialLedger->info().parentHash,
            BCLCxPeerPos::Proposal::seqJoin,
            setHash,
            closeTime,
...
void
BCLConsensus::Adaptor::onForceAccept(
    Result const& result,
    BCLCxLedger const& prevLedger,
...
BCLCxLedger
BCLConsensus::Adaptor::buildLCL(
    BCLCxLedger const& previousLedger,
    ...
    std::set<TxID>& failedTxs)
{
    ...
    return BCLCxLedger{std::move(built)};
}

void
BCLConsensus::Adaptor::validate(BCLCxLedger const& ledger,
    BCLTxSet const& txns,
    bool proposing)
{
    using namespace std::chrono_literals;
...
void
BCLConsensus::Adaptor::onModeChange(
...
Json::Value
BCLConsensus::getJson(bool full) const
{
    Json::Value ret;
    {
...
void
BCLConsensus::timerEntry(NetClock::time_point const& now)
{
    try
    {
        ScopedLockType _{mutex_};
...
void
BCLConsensus::gotTxSet(NetClock::time_point const& now, BCLTxSet const& txSet)
{
    try
    {
        ScopedLockType _{mutex_};
...
bool
BCLConsensus::peerProposal(
    NetClock::time_point const& now,
    BCLCxPeerPos const& newProposal)
{
    ScopedLockType _{mutex_};
...
bool
BCLConsensus::Adaptor::preStartRound(BCLCxLedger const & prevLgr)
{
...
bool
BCLConsensus::Adaptor::haveValidated() const
{
    return ledgerMaster_.haveValidated();
}

LedgerIndex
BCLConsensus::Adaptor::getValidLedgerIndex() const
{
    return ledgerMaster_.getValidLedgerIndex();
}

std::pair<std::size_t, hash_set<BCLConsensus::Adaptor::NodeKey_t>>
BCLConsensus::Adaptor::getQuorumKeys() const
{
    return app_.validators().getQuorumKeys();
}

std::size_t
BCLConsensus::Adaptor::laggards(Ledger_t::Seq const seq,
    hash_set<BCLConsensus::Adaptor::NodeKey_t>& trustedKeys) const
{
    return app_.getValidations().laggards(seq, trustedKeys);
}

bool
BCLConsensus::Adaptor::validator() const
{
    return !valPublic_.empty();
}

void
BCLConsensus::startRound(
    NetClock::time_point const& now,
    BCLCxLedger::ID const& prevLgrId,
    BCLCxLedger const& prevLgr,

...
#include <blocmatrix/app/ledger/InboundLedger.h>
...
#include <blocmatrix/core/TimeKeeper.h>
#include <memory>
#include <mutex>
#include <thread>
...
BCLValidatedLedger::BCLValidatedLedger(MakeGenesis)
    : ledgerID_{0}, ledgerSeq_{0}, j_{beast::Journal::getNullSink()}
{
}

BCLValidatedLedger::BCLValidatedLedger(
...
auto
BCLValidatedLedger::minSeq() const -> Seq
{
    return seq() - std::min(seq(), static_cast<Seq>(ancestors_.size()));
}

auto
BCLValidatedLedger::seq() const -> Seq
{
    return ledgerSeq_;
}
auto
BCLValidatedLedger::id() const -> ID
{
    return ledgerID_;
}

auto BCLValidatedLedger::operator[](Seq const& s) const -> ID
{
    if (s >= minSeq() && s <= seq())
    {
        if (s == seq())
...
BCLValidatedLedger::Seq
mismatch(BCLValidatedLedger const& a, BCLValidatedLedger const& b)
{
    using Seq = BCLValidatedLedger::Seq;
...
BCLValidationsAdaptor::BCLValidationsAdaptor(Application& app, beast::Journal j)
    : app_(app),  j_(j)
{
...
NetClock::time_point
BCLValidationsAdaptor::now() const
{
    return app_.timeKeeper().closeTime();
}

boost::optional<BCLValidatedLedger>
BCLValidationsAdaptor::acquire(LedgerHash const & hash)
{
    ...
    return BCLValidatedLedger(std::move(ledger), j_);
}

void
BCLValidationsAdaptor::onStale(BCLValidation&& v)
{
...
void
BCLValidationsAdaptor::flush(hash_map<NodeID, BCLValidation>&& remaining)
{
    bool anyNew = false;
    {
        ScopedLockType sl(staleLock_);
...
void
BCLValidationsAdaptor::doStaleWrite(ScopedLockType&)
{
    ...
    {
        std::vector<BCLValidation> currentStale;
...

        {
            ScopedUnlockType sul(staleLock_);
            {
                ...
                for (BCLValidation const& wValidation : currentStale)
                {
    ...
    std::string const& source)
{
    ...
    BCLValidations& validations = app.getValidations();

...
#ifndef BLOCMATRIX_APP_CONSENSUSS_VALIDATIONS_H_INCLUDED
#define BLOCMATRIX_APP_CONSENSUSS_VALIDATIONS_H_INCLUDED

#include <blocmatrix/app/ledger/Ledger.h>
#include <blocmatrix/basics/ScopedLock.h>
#include <blocmatrix/consensus/Validations.h>
...
public:
    using NodeKey = blocmatrix::PublicKey;
    using NodeID = blocmatrix::NodeID;

    /** Constructor

        @param v The validation to wrap.
    */
    BCLValidation(STValidation::pointer const& v) : val_{v}
    {
    }
...
/** Wraps a ledger instance for use in generic Validations LedgerTrie.
    ...
    BCLValidatedLedger, we treat any ledgers separated by more than 256 Seq as
    distinct.
*/
class BCLValidatedLedger
{
public:
    using ID = LedgerHash;
    ...
    BCLValidatedLedger(MakeGenesis);
...
/** Generic validations adaptor class for BCL

    Manages storing and writing stale BCLValidations to the sqlite DB and
    acquiring validated ledgers from the network.
*/
class BCLValidationsAdaptor
{
public:
    // Type definitions for generic Validation
    using Mutex = std::mutex;
    using Validation = BCLValidation;
    using Ledger = BCLValidatedLedger;

    BCLValidationsAdaptor(Application& app, beast::Journal j);
    ...
    */
    void
    flush(hash_map<NodeID, BCLValidation>&& remaining);
...
private:
    ...
    std::vector<BCLValidation> staleValidations_;
...
/// Alias for BCL-specific instantiation of generic Validations
using BCLValidations = Validations<BCLValidationsAdaptor>;

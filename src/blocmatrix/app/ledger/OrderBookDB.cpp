...
{
    bool toBMC = isBMC (book.out);
    std::lock_guard <std::recursive_mutex> sl (mLock);

    if (toBMC)
    {
        // We don't want to search through all the to-BMC or from-BMC order
        // books!
        for (auto ob: mSourceMap[book.in])
        {
            if (isBMC (ob->getCurrencyOut ())) // also to BMC
                return;
        }
    }
    else
...
bool OrderBookDB::isBookToBMC(Issue const& issue)
{
    std::lock_guard <std::recursive_mutex> sl (mLock);
    return mBMCBooks.count(issue) > 0;
}

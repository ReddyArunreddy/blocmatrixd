    ...
    TxQ::Setup const& setup)
{
    ...
    // happen.
    return mulDiv(tx[sfFee].bmc().drops(),
        baseRefLevel,
...
/*
    ...
    0. Is `featureFeeEscalation` enabled?
        ...
        No: Fallback to `blocmatrix::apply`. Stop.
        ...
        ApplyFlags flags, beast::Journal j)
{
    ...
    struct MultiTxn
    {
        ...
        BMCAmount fee = beast::zero;
        BMCAmount potentialSpend = beast::zero;
        ...
        if (sle)
        {
            ...
            if (multiTxn)
            {
                /* Check if the total fees in flight are greater
                    ...
                    Assume: Minimum account reserve is 20 BMC.
                    Example 1: If I have 1,000,000 BMC, I can queue
                        a transaction with a 1,000,000 BMC fee. In
                        the meantime, some other transaction may
                        lower my balance (eg. taking an offer). When
                        the transaction executes, I will either
                        spend the 1,000,000 BMC, or the transaction
                        will get stuck in the queue with a
                        `terINSUF_FEE_B`.
                    Example 2: If I have 1,000,000 BMC, and I queue
                        10 transactions with 0.1 BMC fee, I have 1 BMC
                        in flight. I can now queue another tx with a
                        999,999 BMC fee. When the first 10 execute,
                        ...
                        999,999 BMC, or get stuck in the queue.
                    Example 3: If I have 1,000,000 BMC, and I queue
                        7 transactions with 3 BMC fee, I have 21 BMC
                ...
                */
                auto const balance = (*sle)[sfBalance].bmc();
